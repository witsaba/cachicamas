...
// set at v1 is 11 — five pre-existing + four CH-09 additions +
// two CH-10 additions. A future variant added without an `event:`
// mapping surfaces here as a missing entry AND in `wireFrameName`'s
// runtime panic.
//
...
var knownWireFrameNames = []string{
	...
	"tool.call.end",
	"tool.result",
	// CH-10 additions (D-3, R-CPM-003, NFR-CPM-003 wire-fragmentation guard).
	"permission.decision.required",
	"permission.decision.made",
}

// TestWire_FrameNameSet_IsClosed (S-CTS-024, S-CPM-022) — the chat
// package's wire-frame vocabulary is exactly the 11 names listed
// above. A future WireEvent variant added without a wireFrameName
// case is a runtime panic (verified by the test harness in
// projection_tool_test.go for the four CH-09 variants). The
// exhaustive coverage here guards against accidental REMOVAL of a
// frame name — a deleted entry would surface as a parseTranscript
// regression in the frontend (S-1.b silent drop).
func TestWire_FrameNameSet_IsClosed(t *testing.T) {
	t.Parallel()

	if len(knownWireFrameNames) != 11 {
		t.Errorf("wire frame name set has %d entries, want 11 (S-CTS-024 + S-CPM-022 — must match eventsource.go switch and parseTranscript)", len(knownWireFrameNames))
	}
