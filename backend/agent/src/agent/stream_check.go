...
// (VL2-SEAM-14).
//
// It reports the first violation in stream order. Rules, checked in this
// order for each event:
//
//  1. contiguity — the event's [Sequence] equals the running expectation,
//     1-based;
//  2. nothing follows the terminal — no event may appear once the run
//     bracket has closed;
...
func CheckStream(events []Event) StreamReport {
	...
	for _, e := range events {
		...
		if seq != seqExpect {
			return violation(ai.ErrOutOfRange, ai.AtIndex("event", int(seq)))
		}
		seqExpect++

		if run == runBracketClosed {
			return violation(ai.ErrMisplaced, ai.AtIndex("event", int(seq)))
		}
		...
		case BracketRoleOpensRun:
			if run != runBracketUnopened {
				return violation(ai.ErrDuplicate, ai.AtIndex("event", int(seq)))
			}
			run = runBracketOpen
		...
		case BracketRoleClosesRun:
			...
			if turnOpen {
				return violation(ai.ErrMalformed, ai.AtIndex("event", int(seq)))
			}
