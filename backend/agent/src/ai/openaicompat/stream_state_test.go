...
func TestMapperState_ThreeContentChunks_EmitsThreeOrderedDeltas(t *testing.T) {
	...
	events := driveChunks(t, state,
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"role":"assistant","content":"Hola"},"finish_reason":null}]}`),
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":", "},"finish_reason":null}]}`),
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":"mundo"},"finish_reason":null}]}`),
	)
...
func TestMapperState_TwoContentChunksAndTerminal_EmitsSixKindsInOrder(t *testing.T) {
	...
	state := &mapperState{}
	events := driveChunks(t, state,
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":"un"},"finish_reason":null}]}`),
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":" dos"},"finish_reason":null}]}`),
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`),
	)
...
func TestMapperState_RoleOnlyOpeningThenContent_BlockStartBetweenThem(t *testing.T) {
	...
	opening, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"role":"assistant"},"finish_reason":null}]}`))
	if err != nil {
	...
	content, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":"hola"},"finish_reason":null}]}`))
	if err != nil {
...
func TestMapperState_NoContentAnywhere_NoBlockMintedAtAll(t *testing.T) {
	...
	state := &mapperState{}
	events := driveChunks(t, state,
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"role":"assistant"},"finish_reason":null}]}`),
		mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`),
	)
	completion, err := state.buildCompletion()
	if err != nil {
...
func TestMapperState_TerminalThenDeltaLessAndUsageChunks_ClosesCleanly(t *testing.T) {
	...
	content, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":null}]}`))
	if err != nil {
		t.Fatalf("applyChunk(content) error = %v, want nil", err)
	}
	...
	terminal, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`))
	if err != nil {
	...
	deltaLess, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[{"index":0,"delta":{},"finish_reason":null}]}`))
	if err != nil {
	...
	usage, err := state.applyChunk(mustChunk(t, `{"id":"r1","model":"m","object":"chat.completion.chunk","created":1700000000,"choices":[],"usage":{"prompt_tokens":3,"completion_tokens":5,"total_tokens":8}}`))
	if err != nil {
