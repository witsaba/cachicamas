...
> **Status (updated 2026-08-20, count corrected by `sdd-verify` MINOR-4 — 24 total and the wave breakdown are read directly off this document's own Wave 0…6 table of contents above, not restated from memory):** Wave 0 complete, Wave 1 complete, Wave 2 complete (AG-07…AG-11), Wave 3 complete (AG-12…AG-16), Wave 4 complete (AG-17…AG-18), Wave 5 complete (AG-19…AG-20), Wave 6 complete (AG-21…AG-23) — **24 of 24** milestones shipped, Layer 2 complete. Wave 0 (AG-00 contract vocabulary, AG-01 event delivery, AG-02 v1 scope — all `[decision]` leaves) merged via PR #159 (`47813e6c`) on 2026-08-11. AG-03 (package scaffold and boundary guards) lands `backend/agent/src/agent/` — Layer 2's first code — with a machine-checked doc contract, a forward import guard over both closures, and a no-ambient-authority guard, all seven required bites recorded RED then GREEN; verified PASS WITH WARNINGS, 0 CRITICAL. AG-04 (event envelope and ordering invariants) lands Layer 2's first behavior — the event envelope, per-lane ordering, the run and turn lifecycle families, the stream-contract validator's two-level scope engine, the two invariant pins, and the every-kind-constructible guard — closing on its own recorded guard bites (S-AEV-082/083/090/092), the `L2C-04` doc-contract row, and the `S-AEV-102` documentation pin, all RED-first under strict TDD. Wave 1 complete (AG-05 event assembly, AG-06 codec family). Wave 2 complete (AG-07 walking skeleton, AG-08 hook seam, AG-09 tool scheduler, AG-10 permission protocol, AG-11 turn termination — five milestones, per this document's own Wave 2 table-of-contents entry above; corrected by `sdd-verify` MINOR-4 from a stale "5 of 7"); AG-10 closes G1's protocol half (R-10) and v2 § 6 seam 2, and AG-11 closes R-08's typed mid-stream path — the loop now maps every normalized finish reason to a distinct typed turn outcome under an exhaustive dispatch, and reports a terminal provider failure upward as a typed value carrying category, retryability and the partial-output discriminator, without ever retrying. AG-12 (history and the pairing invariant) opens Wave 3 — it stands only on AG-03 and runs beside all of wave 2 — and closes R-07's boundary enforcement: the transcript store now rejects an orphaned call/result pairing at the boundary rather than at call sites, and synthesizes results for calls an interruption left orphaned before the next turn begins. AG-13 (the multi-turn run driver) is Wave 3's second milestone and closes R-08's driving loop while consuming R-09's upward path: a value-form `Harness` drives `Turn` repeatedly to a run's terminal decision, appending the user-side messages, bracketing the whole run with one run bracket over N turn brackets on one contiguous lane, queueing steering input at turn boundaries with zero drops, resuming a paused turn verbatim, and surviving a permission suspension that spans a turn through an externally woken parked call. AG-14 (the cancellation tree) is Wave 3's third milestone and closes R-08's typed mid-stream path for the cancellation case while consuming R-09's upward path a second way: two named signals — interrupt and shutdown — propagate down through the loop, the provider and tools as one context-cancellation cause, an interrupted run winds down (orphan synthesis, a clean turn close, an interrupted run-end carrying no failure) and leaves the harness ready for the next prompt, a shutdown winds down identically and then terminally refuses further prompts, the two signals stay distinguishable through both the run-end outcome and the error chain, and a documented per-call bound reports any tool that outlives it typed by tool name and call identity rather than letting it hold the run hostage. AG-15 (retry policy and the failover seam) is Wave 3's fourth milestone and closes R-15's Layer 2 half (G8): a pure, ordered-gate predicate decides whether a failed turn surfaces, retries, or has exhausted its budget on the failed turn's own typed evidence — a retryable failure with no emitted output retries visibly over a byte-identical transcript, one turn bracket per attempt on the run's own lane; a failure after emitted output is never silently retried, even when it reports itself retryable, closing the G8 sentence the charter opened; a non-retryable failure surfaces immediately regardless of delivery position; the backoff wait runs through an injected timing seam, a reported retry-after overrides the computed exponential-with-jitter delay, and an interrupt or shutdown fired during the wait aborts it and winds the run down like any other signal; the harness's own attempt bound composes with Layer 1's own retry budget into a documented worst-case ceiling whose two-layer wording divergence is enforced by a test, not a comment; the exhausted-retry report now preserves the failed turn's true category, retryability, retry-after and delivery evidence by wrapping the identical typed failure rather than reconstructing it; and the failover seam is a named, nil-safe injection point consulted exactly once at exhaustion, shipping one declining implementation proven behaviorally identical to no policy at all, with the fallback route itself deferred past v1 (AGS-D). AG-16 (cost and usage events) is Wave 3's fifth milestone and closes G10's Layer 2 half: every non-aborted turn now emits a per-turn cost figure carrying Layer 1's own per-figure presence — absence read as absence, never an invented zero — and the harness folds every emitted per-turn figure into a run-scoped cumulative, reporting a running estimate between continued turns and exactly one final figure immediately before every run close (success, failure and cancellation alike, retries counted by construction), so a run's real token spend is observable on every outcome with no new event kind and no field that could carry money. AG-17 (the context strategy seam and token accounting) is Wave 4's opening milestone and closes R-18's seams 5 and 6: a nil-default context-strategy seam on the caller-owned harness is consulted exactly once per logical turn at the run driver's turn boundary — never per retry attempt — carrying the coming transcript, Layer 3's stated budget and a type-level token accounting whose figure is unreadable without its provenance; the accounting is discovered by type-asserting the shipped Layer 1 counting contract and falls back to a documented byte-based estimate only on a genuinely absent capability, never on an advertised counter that declines to answer; the shipped default never compacts, unconstructibly so at the type level — installing it changes nothing observable — and compaction itself, together with any threshold arithmetic over the budget and the accounting, remains wholly AG-18's. AG-18 (implement compaction) closes Wave 4 and discharges **G3** (R-11), the verdict AG-17 explicitly did not: the seam's verdict type gains the one field AG-17 forecast, re-homing the never-compact guarantee from the type onto the zero value — the strongest form still available once the extension point opens; a summarization call on an injected provider, injected options and an injected instruction distinct from the run's own replaces the resolved prefix of a transcript with one typed summary entry through history's single validating commit primitive, at a boundary that is both turn-mark-aligned and pairing-closed, with the protected tail preserved value-identical in Layer 1 values and origins and its entries renumbered — ordinal identity stays stable only within a transcript generation, never across a compaction, with the turn identifier as the durable cross-generation handle; the compaction event family emits for the first time after twelve milestones constructible-but-unemitted, inside its own dedicated turn bracket, with the stream validator, every Layer 1 boundary and the whole compaction payload vocabulary byte-unchanged; atomicity is a property of ordering, not a mechanism — the commit is unreachable until the call succeeded and the replacement validated, so an interrupted compaction leaves the pre-compaction transcript intact with no journal and no rollback log, and the run's own cancellation wind-down is never entered from inside compaction; and an on-demand entry point invokes the identical operation, refusing typed whenever a run is in flight — reusing the harness's existing signal state rather than a new flag — and never queueing. AG-19 (prove re-entrancy and delegation readiness) opens Wave 5 and discharges seam 12 (R-14, G7's structural half): a context-carried publishing seam, installed for exactly one tool call and revoked on every exit path — normal return, detach and re-panic alike — rides the scheduler's existing emission funnel with no new channel, loss rule or second stamping writer, and its admissibility rule, derived from the event registry itself, admits nineteen of the twenty-five registered kinds while refusing every bracket-opening, terminal, at-most-one and `cost_turn` kind by construction; a nested child harness's events resolve to their parent through one mirrored `subagent_started`/`subagent_ended` pair rather than a per-event stamp, sibling children interleave under `-race` with no cross-talk, nested cancellation is inherited through the existing signal tree and completes leaf-first, child cost crosses as a consumer-side reconstruction with the Layer-2 fold made unreachable rather than merely discouraged, and a derived permission scope composes the parent's policy, narrowing only and never widening, with the ask routed up and the decision routed down through the existing wake surface. No subagent tool, configuration or depth limit ships — every subagent concept lives in the external test package, unreachable from production by the compiler rather than by convention. AG-20 (complete the hook taxonomy) closes Wave 5 and discharges G11's Layer 2 half (R-17): one `Hooks` value type, held as a single field on the harness and transported into `Turn`'s own per-turn copy beside `Continuation`, carries all four hook points behind one registration surface — the shipped pre-request seam widened to chain composition with the singular field kept and running first, a pre-compact splice reaching both compaction doors through the one operation with the adjusted cut re-resolved unconditionally through the existing retraction-only surgery, a post-turn observation firing once per logical turn across fourteen enumerated loop exits collapsed to four call sites, and a session-start latch firing once per harness value; the two families are distinguished by type rather than by convention — an observing hook has no result parameters at all, so the wrong thing is unconstructible — and dispatch runs on a per-run observer lane whose enqueue never blocks and whose drain goroutine is never the run's own, with the observer's own context value-stripped so the delegation publishing seam stays unreachable from a hook; a deliberately stalled observer is proven, by a goroutine-placement assertion and by stream byte-identity with no wall clock anywhere, to delay no event delivery, and is reported through a typed value off the streaming path — never a twenty-sixth event kind — at the run's terminal boundary. Envelope invariant 3 (non-blocking observers) closes jointly with AG-01.1, and Wave 5 is complete. AG-21 (harden concurrency, backpressure and leaks) opens Wave 6 and discharges the combined-scenario matrix the charter names explicitly, the interactions no single milestone's own fixture exercised: twelve state-times-signal cells — a permission suspension, a queued steer, an in-flight compaction and a hosted child harness, each crossed with interrupt, shutdown and a provider failure — each reach their own terminal with a valid transcript and a contract-ordered, unmodified stream, with two cells (a failed compaction, and a child harness's contained provider failure, whose loop-level dispatch never surfaces as the parent's own Turn-level error) carrying a recorded, evidence-cited adjusted Then rather than the uniform one (sdd-verify round-2, MAJOR-2: a suspended call's provider failure is uniform in outcome, adjusted only in timing, and is not one of these two); a structurally stalled consumer over an unbuffered sink loses no event describing an already-committed fact, and cancellation still unblocks it within the documented wind-down bound; the harness itself is proven to retain nothing across two serial runs on one value beyond an enumerated inventory of caller-owned or already-shipped state, the transcript's own absence asserted against a minted needle with its own defeat test rather than a presence claim that would pass with the mechanism defeated; and one new non-parallel sweep proves the whole assembly leak-free, with the sole third-party goroutine class accounted for by a typed report rather than excluded from the count. No production code changed. AG-22 (add the observability boundary) is Wave 6's second milestone and discharges R-19: the AG-22.1 decision node records the Layer 2 attribute vocabulary as an extension of [ADR 0005 § D3](../../adr/0005-promote-agent-stack-to-own-module.md#d3--observability-boundary)'s table — § D3's own allowlist covers Layer 1 spans only — across four span families (`invoke_agent` for the run, `turn`, `execute_tool {name}` and `compact`), each attribute mapped key to Go type to the exact event accessor its value must equal; Layer 1's eleven request-span keys are deliberately not re-recorded here (deferred to AI-37, stated on the record so nobody re-adds them), and seven deliberate non-recordings each carry their own reason — per-delta events, hook timings, permission argument content, usage/cost, compaction instruction and summary content, `Failure` beyond its category, and the subagent/compaction window IDs — while § D3's denylist is restated verbatim and unweakened. Spans are acquired through an injected `Harness.TracerProvider` defaulting to a no-op, mirroring Layer 1's AI-37 precedent; the § D3-granted global getter was rejected on measurement — the root `go.opentelemetry.io/otel` package's own closure is 25 packages and contains `go.opentelemetry.io/auto/sdk`, which `S-AGP-024` forbids naming in an import allowlist — and the injected seam also keeps the package's pervasive `t.Parallel()` usable. The import-boundary allowlist gains five § D3-cited entries, two of which (`otel/semconv`, `github.com/cespare/xxhash/v2`) are forced-closure entries with no § D3 table row and cite the forced-closure clause plus the measurement instead; the semconv entry is admitted as a prefix because its measured path is version-suffixed `…/semconv/v1.41.0`, pinned by `otel/trace@v1.44.0`. Every span ends exactly once on every exit path — normal, error, cancellation and panic — through defer-registered finalizers, and the detached tool arm ends at the scheduler's own wind-down bound carrying `Error` status and `cachicamas.tool.detached=true`, so AG-21's leak discipline is not regressed in a new shape; delegation trees nest with zero fixture changes because `tracer.Start`'s parent resolution is entirely ctx-driven, so a child harness carrying its own provider still nests under whatever span the ctx already holds, proved on AG-19's existing delegating-tool fixture. The denylist is proven by absence, not asserted: uniquely minted, runtime-concatenated sentinels across nine vectors, a positive-control self-test proving the needle bites its own corpus, and non-vacuity floors on corpus size, unique attribute keys and event-kind coverage; and the untraced run's drained event sequence is compared to the traced run's by value, not merely by kind — no tracer, no difference. `S-AGS-066` in `agent-v1-scope` — "declares … no telemetry sink" — is falsified by this change with no test failing, because its enforcing test only ever checked `hooks.go` for wall clocks and zero-value `Hooks` inertness; it is amended, and `S-AGS-068` now checks the seam-not-sink claim against the shipped exported surface. `sdd-verify` round 1 returned FAIL (2 CRITICAL: a rewritten import guard that let `os/exec` into Layer 2 production, and run/turn/compaction spans leaking on panic); round 2 returned PASS WITH WARNINGS, 0 CRITICAL, after both were closed and re-proved by defeat tests in both directions. `backend/agent/go.mod`, `go.sum` and all of `backend/agent/src/ai/` are byte-unchanged — Layer 1 had already paid for the OTel API dependency at AI-37, and a shipped test enforces the freeze. Two follow-ups carry forward from round 2 rather than being dropped: `import_boundary_test.go` and `scheduler.go` are released from `hksScopeFenceByteUnchangedFiles()`'s byte-freeze permanently, for every future branch, with no restoration recorded anywhere (verify-report W-6) — once this milestone merges the freeze baseline moves to the new `main`, so AG-23 can and should restore both entries; and a genuinely pre-existing race between `Harness.Run`'s forwarder goroutine and its own deferred `close(sink)` on a panicking `Turn`, structurally identical on `main` and on this branch and so out of AG-22's scope, is disclosed only inside an Engram observation until carried here (verify-report W-5). Layer 2 code now exists and carries behavior; the seams named in [the v2 architecture reference § 4](../0001-cachicamas-agent-stack-v2.md#4-layer-2--the-portable-agent-runtime) are being built in as each milestone lands, per the entry-gate discipline below. AG-23 (publish the Layer 3 readiness contract) closes Wave 6 and discharges R-21, and is Layer 2's last milestone: the genuinely pre-existing forwarder race AG-22 carried forward (W-5) is fixed, RED-first — the RED watched failing before the fix existed, and both defeat directions planted and watched failing after the fix landed, each reverting one half of it before being reverted in turn — a dedicated abort channel plus a deferred join, registered so it always runs before the consumer sink closes on every exit path including a panic through the deliberately unrecovered pre-request-hook seam, so the harness's per-attempt forwarder can no longer crash a goroutine the caller does not own; the import-boundary guard gains a fifth check, a source-level scan over two new sibling package trees (test files included) denying process, network, system-call, filesystem and wall-clock imports by name, with its own per-pattern vacuity floor proven non-vacuous by a planted bite; a new importable, scriptable kit ships at `backend/agent/src/apptest` — never inside `backend/agent/src/agenttest`, which stays byte-unchanged, as a genuinely third-party consumer package cannot live inside a tree Layer 1's own import guard sweeps under Layer 1's rules — providing a scripted permission policy, a scripted tool, and a drain helper delegating wholesale to the already-exported stream validator; a new external consumer proof at `backend/agent/src/layer3handoff` builds a harness from injected fakes alone and drives all seven acceptance capabilities — construction, a multi-turn tool-calling conversation, a scripted permission suspension and its resolution, an interrupt, a resumed prompt, a second harness seeded from the first's own transcript, and a closing validation of every drained stream — in one sequential run, alongside a mechanical, non-self-tripping vocabulary guard proven to bite on a planted application-specific tool name; four runnable examples, compiled and run by the ordinary test command, cover building a harness, driving a run, consuming its event stream, and handling a permission suspension; and the compatibility statement (`decision.md`) freezes the v1 surface by capability, walks the completion checklist to its closing nodes, and registers the four named limitations — no production coordination tool for nested runs, a failover policy that always declines, no default context-reduction policy, and the abandoned-consumer contract inherited from Layer 1 — with the forwarder race explicitly excluded from that register as a fixed defect rather than a design limitation. The W-6 scope-fence disposition closes on the merits: `scheduler.go` is restored to the fence, and `import_boundary_test.go`'s release is made permanent, since that file is the layer's own designed extension point and freezing a designed extension point is a category error. The `TurnOptions.PreRequestHook` field's removal, forwarded across AG-20 and AG-22, is resolved here by decline: the field is kept, unamended in behavior, carries no deprecation marker, and is published frozen-and-superseded with a post-v1 removal path — this milestone freezes the v1 surface, and removing an exported member is the one thing the milestone that freezes a surface must not do. The three shipped specs that deferred their own test-convenience-wrapper rows to this milestone are back-annotated closed: delivered, relocated to `backend/agent/src/apptest`. **Wave 6 is complete. Layer 2 is complete.**
...
- [x] The package exists at the ADR 0005 § D2 location with both boundary guards biting — closed by AG-03.
- [x] Every event family of v2 § 4.3 — all eight — is constructible, validated, and guarded — closed by AG-04, AG-05, AG-06.
- [x] All four envelope invariants hold by test: indexed deltas, explicit nesting, non-blocking observers, typed errors — closed by AG-04.3, AG-19.1, AG-20.2, AG-11.2.
- [x] One turn runs end to end against the fake provider with events in contract order — closed by AG-07.
- [x] The pre-request hook can rebuild the outgoing request; the identity default changes nothing; unchanged inputs yield byte-identical prefix regions across turns — closed by AG-08.
- [x] Tools execute with read-parallel/write-serial policy, bounded fan-out, and call-ordered rejoin — closed by AG-09.
- [x] Permission is a suspension on the stream with all four outcomes; suspension blocks nothing else — closed by AG-10.
- [x] Refusal, pause, and unknown finish reasons produce three distinct behaviors — closed by AG-11.1.
- [x] The loop never retries and never decides policy; the guards and tests that prove it stay green — closed by AG-11.2, AG-10.1, AG-03.3.
...
- [x] The package is race-clean and leak-free under the combined-scenario matrix — closed by AG-21.
- [x] Telemetry stays inside the § D3 boundary with the denylist proven by absence — closed by AG-22.
- [x] The Layer 3 readiness contract is published with the consumer proof and the scripted-harness kit — closed by AG-23.
