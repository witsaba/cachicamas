...
- [ ] Interrupt and shutdown are distinguishable end to end; wind-down is bounded — closed by AG-14.
- [ ] Partial-output failures are never silently retried; retry attempts are visible events — closed by AG-15.1.
...
- [ ] The context strategy is consulted before every call; token counting is capability-discovered with a labelled fallback — closed by AG-17.
