...
> **Status:** Not started — **0 of 24** milestones shipped. **AG-00 is the first milestone, unblocked now:** every entry gate is satisfied — Layer 1 is complete (42 of 42, [doc 0002](./0002-cachicamas-ai-layer-1-task-graph.md)) and [AI-40 — the Layer 2 readiness contract](./0002-cachicamas-ai-layer-1-task-graph.md#ai-40--publish-the-layer-2-readiness-contract) merged as `7326a813` on 2026-08-10. Layer 2 code (`backend/agent/src/agent/`) does not exist yet; this document still precedes its first line, so the seams named in [the v2 architecture reference § 4](../0001-cachicamas-agent-stack-v2.md#4-layer-2--the-portable-agent-runtime) are built in rather than retrofitted.
