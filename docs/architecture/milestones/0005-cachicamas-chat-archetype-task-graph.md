...
> **Status:** CH-11 (the final milestone) shipped — **12 of 12** milestones complete; v1 statement published. **CH-00 is the first milestone.** After CH-03 the archetype owns its HTTP+SSE serving surface: `POST /api/agent/turns`, `GET …/events`, `DELETE …/:id` — the frozen wire that `frontend-chat-layer1` already drives. CH-04 (composition root, env wiring) shipped 2026-08-24: `backend/agent/src/cmd/chat/` now reads every required env var, installs the OpenTelemetry SDK, builds the openrouter provider + JWE IdentityResolver shim, mounts the chat surface, and binds Echo with graceful OTel shutdown on SIGTERM/SIGINT. CH-07 (chat store adapter, swap-in of the real database adapter) shipped 2026-08-24: `backend/agent/src/chat/store_postgres.go` implements the same two methods of the closed `ConversationStore` port, the chat-owned forward-only migration runner + 0001_init.sql land in `backend/agent/src/chat/{migrator,migrations}/`, the composition root reads `CACHICAMAS_CHAT_STORE_DSN` and dials Postgres through `pgx/v5/stdlib`, and the CH-07.2 guard at `backend/agent/src/chat/store_guard_test.go` walks the archetype's own file tree to prove the adapter swap changed no caller. CH-08 (browser resume) shipped 2026-08-24: `ConversationStore.List(participantID)` is the third additive method on the closed port (R-CCS-013, anticipatory text of R-CCS-010); two new GET endpoints — `GET /api/agent/conversations/:id` (CH-08.1 reload) and `GET /api/agent/conversations` (CH-08.2 list) — go behind the existing `identityMiddleware` and register via a new `chat.RegisterResumeRoutes`; the chat page's `useVisibleTask$` mount fires both GETs in parallel, seeds `useChatStream.reset(entries)`, and re-mounts the `ConversationList` rail against the wire shape (R-CRI-005, REQ-8); the closed `ChatStreamEvent` union is preserved (REQ-7 — `ExchangeDTO` and `ConversationSummary` are new types adjacent to it, never additions to it); no new top-level Go deps; no file under `backend/agent/src/agent/` modified (NFR-TLS-003). CH-09 (chat tool-source port + first tool) shipped 2026-08-25: `chat.ToolSource` is the chat-owned wrapper-direction port over `agent.Registry` (D-1); `chat.FromAgentRegistry(agent.Registry) ToolSource` adapts Layer 2's registry to chat's port; `chat.CurrentTimeTool` implements `agent.Tool` with an injectable `NowFunc` (D-2); `Config.ToolSource` is required (R-CTS-001, ErrNilToolSource typed); four new `WireEvent` variants (`ToolCallStart`, `ToolResult` plus the reserved-but-unused `ToolCallDelta` / `ToolCallEnd` for future progress-bearing tools) extend the wire shape (D-3, D-6 collapse); `Exchange` widens with `[]ToolCallRecord` + `[]ToolResultRecord`; forward-only sibling tables `chat_tool_calls` + `chat_tool_results` keyed by `(participant_id, exchange_position, position)` (NFR-CCS-006); `parseTranscript` switch covers all 9 event names (S-CTS-024); F-CHT-9.1 spec defect resolved at design — spec text aligned `state: "complete"` → `state: "done"` to match the existing `TranscriptLine` closed union at `mock/chat.ts:42` (no code ripple); the closed `ChatStreamEvent` union is preserved by the additive REQ-8..11 amendment (D-7); no new top-level Go deps; no file under `backend/agent/src/agent/` modified (NFR-TLS-003 — substrate guard `TestChat_SubstrateUntouched` runs inside `make test`).
...
- [x] Exactly one package reads the environment, installs the observability SDK, and is imported by nothing — closed by CH-04.1, CH-04.2 (closed by main.go + ambient_authority_test.go, evidence cite at bb9697bd)
- [x] One real turn against a live provider is recorded, and the suite stays green without a credential — closed by CH-04.3 (closed by main_test.go TestChatRoot_LiveSmoke + 4 gate-decision tests, recorded run operator-supplied at bb9697bd)
...
- [x] The chat archetype owns a tool-source port and exposes at least one tool the model can call — closed by CH-09.1 (chat.ToolSource wraps agent.Registry by interface; chat.CurrentTimeTool is the first tool; Config.ToolSource is required with typed ErrNilToolSource)
- [x] Tool-call events reach the browser as discrete transcript entries — closed by CH-09.2 (four WireEvent variants: ToolCallStart / ToolResult emitted at v1; ToolCallDelta / ToolCallEnd reserved-but-unused per D-6 wire collapse; SSE framing + parseTranscript switch in lockstep)
- [x] Tool-call records round-trip through the reload surface, no provider text leaks on failure — closed by CH-09.3 (Exchange widens with []ToolCallRecord + []ToolResultRecord; forward-only sibling tables chat_tool_calls + chat_tool_results; NFR-CCS-008 defensive copy on Load)
...
- [x] One deterministic acceptance drives the whole archetype uncached — closed by CH-11 (closed by chat_acceptance_test.go, evidence at chat_acceptance_evidence.txt at bb9697bd)
- [x] The v1 statement is published, every deferral seam-named, and what Layer 2's seam set got wrong is stated — closed by CH-11 (closed by docs/architecture/v1/cachicamas-chat-v1-completion.md at bb9697bd)
