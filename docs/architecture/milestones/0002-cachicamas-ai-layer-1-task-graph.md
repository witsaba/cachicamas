...
> **Status:** Wave 0 + Wave 1 + Wave 2 + Wave 3 + Wave 4 + Wave 5 (AI-33 + AI-34 + AI-35 + AI-36 + AI-37 + AI-41) complete · Wave 6 (AI-38, AI-39, AI-40) SDD-closed — **42 of 42** milestones SDD-closed. **All 42 milestones are landed and merged on `main` (reconciled 2026-08-10): AI-38 verified PASS (14/14 requirements, 47/47 scenarios), specs promoted, merged via PR #140 (`5bc2da4e`); AI-39 verified with 0 CRITICAL — machine verdict fail on evidence-completeness alone (33/39 scenarios pass, 6 partial credential-gated), implementation PASS WITH WARNINGS, specs promoted, merged via PR #142 (`b062be74`); AI-40 verified, archived at `openspec/changes/archive/2026-08-10-cachicamas-ai-layer2-handoff/`, merged via PR #144 (`7326a813`). See the AI-38 close amendment below for its caveat and risk acceptance; AI-39 has no close-amendment paragraph in this document — its close facts live in its archive records (an omission this reconciliation records rather than repairs retroactively).** The `backend/agent` module exists at `backend/agent/` with **96 production `.go` files and 171 test `.go` files** (fresh-measured at AI-40's close, the count including the three `testdata/`-nested compile-fixture `main.go` files that every close's applied practice counts as production — see the AI-40 close amendment's methodology note; supersedes AI-39's 95/168). Wave 4 closed the wire contract (provider, transport, request translation, streaming decoder, text / reasoning / tool-call / completion / error-mapping); OpenRouter concretized AI-24..AI-32 as the first conformant adapter. AI-33 closed Layer 1's cancellation contract over a real HTTP transport (four cancellation moments + a cross-cutting resource-discipline pass). AI-34 locks Layer 1's backpressure invariant: chosen capacity **`N=0`** per M1/M2/M3 measurement against realistic workloads (text + tool-call transcripts × 5ms / 50ms / pause-200ms consumer profiles, 20 runs each) and the doc 0002:432 tie-break; the chosen constant resolves the doc-vs-code divergence (spec § 6 line 387 said "starting capacity of 64 events" as a hypothesis since AI-02.1; implementation has been unbuffered since AI-28.1.1 with no commit ever introducing a buffer argument). AI-35 closes Layer 1's retry and idempotency policy: pre-stream auto-retry with bounded attempts, retry-after handling, byte-identical replay, and per-attempt drain; the in-adapter loop is factored into a shared helper invoked by the adapter's execute-once function, so future adapters reuse it without re-implementing the discipline. AI-41 lands out of dependency order — before AI-36 and AI-37 — because it discharges two Wave-2 carryovers (`W1`/`W2`) that neither blocked Wave 4 nor depended on Wave 5's other open milestones: the emission-boundary check's fourth rule is now proven directly, and the provider-failure payload's Go-syntax rendering is now redacted, matching its already-redacted textual rendering. This is why the landed range above is stated as "AI-00 through AI-37 plus AI-41" rather than a single contiguous range. AI-36 landed in dependency order behind AI-41, discharging the value-form follow-up AI-41 had recorded against itself. AI-37 then closed Wave 5 in full, adding Layer 1's observability boundary — the module's first and, per ADR 0005 § D3, second-and-last ADR-authorized dependency. **Wave 5 — Harden is now COMPLETE.** AI-38 runs the first real conformance bridge against OpenRouter (14 requirements, 47 scenarios, 0 waivers; transcript round-trip guarded; nine-entry capability record asserted). AI-39 adds the gated, internal smoke package with setup instructions, a sentinel-swept capture sink, and three stream-shape invariants, opt-in via two-stage environment-variable gate. **Remaining:** none — Layer 1 is complete; doc 0003's `AG-03` onward is unblocked.
...
> **Amended 2026-08-06 — Wave 4 close (Wave 0–4 reconciled).** Shipped counter ~~24 of 42~~ → **33 of 42**; landed AI range ~~AI-00 through AI-23~~ → **AI-00 through AI-32**. File count ~~49 prod / 58 test~~ → **85 prod / 133 test**, measured by `find backend/agent/src -name "*.go" -not -path "*/testdata/*"` (the +36 prod / +75 test delta is the Wave 4 deliverable in `backend/agent/src/ai/openaicompat/` plus the OpenRouter wrapper chain). Wire contract closed by merge commit `7f9822d` (PRs #98 Wave 0 / #101 Wave 1 / #104 Wave 2 / #107 Wave 3 / Wave 4 tracker `7f9822d`). First-provider SDD chain: PR #117 (wrapper), #118 (conformance bridge), #119 (live smoke), #120 (tracker merge `e44f59f`) — OpenRouter concretizes AI-24..AI-32 against the OpenAI-compatible Chat Completions streaming dialect. Per-record evidence: [`openspec/changes/archive/WAVE-4-ARCHIVE.md`](../../../openspec/changes/archive/WAVE-4-ARCHIVE.md) · [`WAVE-3-ARCHIVE.md`](../../../openspec/changes/archive/WAVE-3-ARCHIVE.md) · [`WAVE-2-ARCHIVE.md`](../../../openspec/changes/archive/WAVE-2-ARCHIVE.md) · [`WAVE-1-ARCHIVE.md`](../../../openspec/changes/archive/WAVE-1-ARCHIVE.md). Engram backfill follows this same pattern (one observation per wave archive, topic_key `sdd/cachicamas-ai-layer-1/wave-N-archive-report`). **Layer 2 stripping duty for OpenRouter's `reasoning_details` (recorded absence, AI-29) is delegated to AI-40.**

> **Amended 2026-08-07 — AI-33 close (Wave 5 — Harden, first milestone).** Shipped counter ~~33 of 42~~ → **34 of 42**; landed AI range ~~AI-00 through AI-32~~ → **AI-00 through AI-33**. Test file count ~~133~~ → **139** (the +6 is `a_i-33_{1,2,3,4,5a,5b}_test.go`); production file count unchanged at 85. AI-33 proves Layer 1's cancellation contract over a real HTTP transport — five subnodes: AI-33.1 (pre-headers cancellation, R-AIS-034), AI-33.4 (after-completion cancellation, R-AIS-037), AI-33.2 (between-frames cancellation, R-AIS-035), AI-33.3 (truly-abandoned consumer, R-AIS-036 pinned to conformance R-CNF-012 verbatim), AI-33.5 (drain-before-close + full-package leak check, R-AIS-033 + R-AIS-038). Production change is exactly **+1 line** in `backend/agent/src/ai/openaicompat/stream.go:362` (`io.Copy(io.Discard, resp.Body)` before the existing close in `run()`'s defer chain, mirroring `capture.go:117–122`); `go.mod` unchanged, conformance suite unchanged. Spec amendment: `openspec/specs/ai-stream-lifecycle/spec.md` gains R-AIS-033..038 as dated amendments under §§ 4 and 5 (behavior-only form, honors the spec's no-Go-identifier rule). Six implementation commits on `feat/ai-33-cancellation`: `07e4d0c` AI-33.1, `e6eb3a1` AI-33.4, `4ff662f` AI-33.2, `665fa3e` AI-33.3, `99fef5a` AI-33.5 drain, `83eef31` AI-33.5 wrapper. Plus three doc/chore commits: `62e323e` spec amendment, `bb0748b` archive, `fcb4384` untrack `.claude/evidence/`. Verify-report verdict: **PASS** (6/6 requirements, 22/22 scenarios, 0 CRITICAL). Per-record evidence: [`openspec/changes/archive/2026-08-07-cachicamas-ai-cancellation/`](../../../openspec/changes/archive/2026-08-07-cachicamas-ai-cancellation/) (issue #128, PR #129). Engram backfill: one observation per phase, topic_key `sdd/cachicamas-ai-cancellation/{explore,proposal,spec,design,tasks,verify-report,archive-report}` (Engram ids #2611, #2612, #2613, #2614, #2616, #2625, #2626). **Eight acknowledged deviations** documented in the archive-report — none blocking (notably: AI-33.2 size:exception approved by user; spec amendment restated behavior-only to honor the no-Go-identifier rule). **Remaining in Wave 5:** AI-34 (backpressure), AI-35 (retry/idempotency), AI-36 (secret redaction), AI-37 (observability), AI-41 (Wave-2 carryovers).

> **Amended 2026-08-07 — AI-34 close (Wave 5 — Harden, second milestone).** Shipped counter ~~34 of 42~~ → **35 of 42**; landed AI range ~~AI-00 through AI-33~~ → **AI-00 through AI-34**. Test file count ~~139~~ → **143** (the +4 is `measurement_test.go`, `drip_frames_test.go`, `a_i-34_2_test.go`, `a_i-34_3_test.go`); production file count unchanged at 85. AI-34 locks Layer 1's backpressure invariant — three subnodes: AI-34.1 (measured buffer decision; M1/M2/M3 workload recorded in `openspec/changes/cachicamas-ai-backpressure/decision.md`; tie-break lands at **`N=0`** per doc 0002:432), AI-34.2 (lossless ordering under pressure; `cap(ch) == streamCarrierBuffer` direct assertion + slow/bursty/pause-resume consumer profiles via new `dripFramesServer` helper at `backend/agent/src/ai/openaicompat/drip_frames_test.go`), AI-34.3 (no unsanctioned loss path across slow/bursty/pause-resume × text/tool-call × 50 leak-check repeats, serial-only via R-STK-008). Production change is **+15/-1** on `backend/agent/src/ai/openaicompat/stream.go` (new `const streamCarrierBuffer = 0` at line 123 alongside `streamReadBufferSize`; line 223 `make(chan ai.Event)` → `make(chan ai.Event, streamCarrierBuffer)`); runtime behaviour is functionally unchanged at `N=0` (`make(chan T, 0) == make(chan T)`) but the constant + the buffered `make` argument are now part of the contract. `go.mod` unchanged; conformance suite (`backend/agent/src/agenttest/conformance_cancellation.go:46-131`) untouched — R-CNF-011/012 still use `Buffer: 0` and their drop-physics proof is unaffected. Spec amendment: `openspec/specs/ai-stream-lifecycle/spec.md:387` ("starting capacity of 64 events" → "starting capacity of 0 events" per `decision.md`) was **deferred from this PR and applied at archive phase** (commit `7d76aa7`). R-AIS-031's measured-N wording is now in the canonical spec; R-AIS-031 S-1 ("spec wording matches the decision artefact") is satisfied. Six implementation commits on `feat/ai-34-backpressure`: `a110e84` decision.md skeleton, `719489e` M1/M2/M3 measurement, `3b7deb4` apply `streamCarrierBuffer = 0`, `e621a84` 5-line doc-fix on `a_i-33_3_test.go:28`, `9151234` `dripFramesServer` + lossless-ordering tests, `db768c7` exhaustive leak-check × 50. Plus one metadata commit `85cd936` (apply-progress). Verify-gate evidence: `cd backend/agent && make test` PASS (all 6 packages; 54s includes the leak-check × 50 × 6 cases); `make lint` 0 issues; `go test -tags=measurement -run TestMeasurement_Workload_M1M2M3 ./src/ai/openaicompat/...` PASS (27.57s). Per-record evidence: [`openspec/changes/archive/2026-08-07-cachicamas-ai-backpressure/`](../../../openspec/changes/archive/2026-08-07-cachicamas-ai-backpressure/) (PR #130 — https://github.com/witsaba/cachicamas/pull/130). Spec amendment **applied at archive phase**: `openspec/specs/ai-stream-lifecycle/spec.md:387` now reads "starting capacity of 0 events" per `decision.md` (resolves R-AIS-031's measured-N wording; was deferred from the PR per the AI-34 proposal). Engram backfill: 7 phase observations + 1 decisions observation + 1 tooling discoveries observation (`sdd/cachicamas-ai-backpressure/{explore,proposal,spec,design,tasks,apply-progress,decisions,tooling-discoveries}`); Engram ids #2629–#2635 + #2634 (decisions) + #2636 (tooling). **Three acknowledged deviations** documented in apply-progress: (1) **size deviation** — aggregate diff +1241/-2 lines vs ~600 forecast (~107% over forecast); 90% of overage is test code (`measurement_test.go`=438, `a_i-34_2_test.go`=338, `a_i-34_3_test.go`=208, `drip_frames_test.go`=80); production change is +15/-1, the overage is the load-bearing evidence for the N=0 decision; `size:exception` accepted by maintainer (Engram #2634) but the PR also exceeds the 1000-line review budget by ~241 lines. (2) **doc-vs-code divergence reconciled on the record** — spec § 6 line 387 said "starting capacity of 64 events" as a hypothesis (AI-02.1); implementation has been unbuffered since AI-28.1.1 with no commit ever introducing a buffer argument; AI-34.1 records the divergence explicitly and lands the first measured number against a real workload, applying the AI-33 living-graph precedent (spec amendment applied at archive phase, commit `7d76aa7`). (3) **formal verify/archive cycle blocked on tooling** — Gentle AI runtime ledger returns `state: complete` after the apply settle (`passed`) and blocks subsequent `sdd-attempt acquire` for the same change; `gentle-ai sdd-status` dispatcher requires a bounded review transaction (the reviewer sub-agent rejected the `GENTLE_AI_REVIEW_BINDING` prompts — binding format strict); user disabled receipt-driven development (`gentle-ai review mode disable`) to bypass; PR opened manually; tooling discoveries saved to Engram obs #2636. **Remaining in Wave 5:** AI-35 (retry/idempotency), AI-36 (secret redaction), AI-37 (observability), AI-41 (Wave-2 carryovers).

> **Amended 2026-08-07 — AI-35 close (Wave 5 — Harden, third milestone).** Shipped counter ~~35 of 42~~ → **36 of 42**; landed AI range ~~AI-00 through AI-34~~ → **AI-00 through AI-35**. Test file count ~~143~~ → **148** (the +5 is `retry_test.go`, `a_i-35_1_test.go`, `a_i-35_2_test.go`, `a_i-35_3_test.go`, `conformance_retry_test.go`); production file count unchanged at 85. AI-35 closes Layer 1's retry and idempotency policy — the pre-adopted policy decision plus three implementation subnodes: AI-35.0 (policy + seam decision, amendment 2026-08-07), AI-35.1 (pre-stream retry predicate with partial-output boundary marker, R-AIS-041 + R-AIS-043.S-4), AI-35.2 (backoff mechanics with retry-after override and context-aware waits, R-AIS-042), AI-35.3 (replayability and per-attempt drain, R-AIS-043); R-AIS-044 (cross-layer composed-bound ceiling) is a cross-cutting requirement landed alongside AI-35.2's bound documentation, not a dedicated subnode. Helper package factored into a shared seam at `backend/agent/src/ai/internal/retry/` (Go `internal/` directory per ADR 0005 § D1), exporting `retry.Loop` + `retry.Config` + `retry.AttemptReport` + `retry.DefaultMaxAttempts = 3`; wired into `stream.go:218–227` (the pre-stream seam before carrier handover). Conformance grows from `[8]` to `[9]` capabilities with `CapRetry` (CAP-O-04) added; R-CNF-019 added (7 scenarios); R-CNF-017 totality rebuild is mechanical (one new line). Aggregate code diff is **+1301/-62** across 15 files — production core is the helper package + execute-once seam + stream wiring; the bulk is strict-TDD test evidence (helper + stream wiring + 3 real-producer tests + conformance case + conformance suite extension); `go.mod` unchanged, forward guard `import_boundary_test.go:107–162` green. Spec amendments (applied at archive time, full inline promotion per the AI-33 precedent): `openspec/specs/ai-stream-lifecycle/spec.md` gains R-AIS-041..044 with all 14 scenarios inline under § 7 (failure delivery) plus a discovery cross-reference entry; `openspec/specs/ai-provider-conformance-suite/spec.md` gains R-CNF-019 with S-CNF-069..075 inline and R-CNF-017 modified in place to the nine-entry totality (S-CNF-047/048 restated, S-CNF-076 added). Ten commits on `feat/ai-35-retry-policy`: `5b51a20` WU-1 helper package, `94c8fed` WU-2 stream integration + predicate tests, `839e83b` WU-2 fixup context cancellation, `eed2071` WU-3 backoff mechanics, `cecab38` WU-4 replayability + boundary, `daddba1` WU-5 CapRetry + conformance case, `ceb85c8` WU-5 fixup case placement, `6bfc266` lint fix rename, `85d5250` archive + spec promotion + this amendment, `0800869` review-correction: transport-failure retryability derived via the typed taxonomy (the branch-wide receipt-driven review found transport-level pre-stream failures bypassed the retry loop — Retryable was never set by the transport constructor; fixed red→green with a raw-listener transport-failure test). Verify-gate evidence: `make test` PASS (899 PASS / 0 FAIL / 4 pre-existing SKIP, `-race`); `make lint` 0 issues; `make build` 0. Per-record evidence: [`openspec/changes/archive/2026-08-07-cachicamas-ai-retry-policy/`](../../../openspec/changes/archive/2026-08-07-cachicamas-ai-retry-policy/) (issue #131, PR #132 — https://github.com/witsaba/cachicamas/pull/132). Engram backfill: phase observations `sdd/cachicamas-ai-retry-policy/{explore,proposal,spec,design,tasks,apply-progress,verify-report,archive-report}` (Engram ids #2642–#2650). **Three acknowledged deviations** documented in archive-report: (1) **conformance case body placement** — moved from `agenttest/` to `openaicompat/` to test the package it exercises; resolves design D-R4 risk; both openaicompat and openrouter adapt inherit the same case without rewriting assertions (AI-38 preview). (2) **size:exception approved at ~1096, actual 1264** — conformance case body grew to 374 lines due to seven-scenario implementation (S-CNF-069..075) plus helper instrumentation; overrun is concentrated in test files (strict-TDD density per AI-33/AI-34 precedent); user approved exception cap ~1500; ledger reset by user (Engram #2649). (3) **lint fix commit after apply closed** — rename `RetryAfterReader` → `AfterReader` to fix package-level stutter (commit `6bfc266`); verify re-ran `make test` and `make lint` with final commit; no spec impact (R-AIS-042 behavior-level, not identifier-level per no-Go-identifier contract). **Remaining in Wave 5:** AI-36 (secret redaction), AI-37 (observability), AI-41 (Wave-2 carryovers).

> **Amended 2026-08-07 — AI-41 close (Wave 5 — Harden, Wave-2 carryovers).** Shipped counter ~~36 of 42~~ → **37 of 42**; landed AI range ~~AI-00 through AI-35~~ → **AI-00 through AI-35 plus AI-41** — stated non-contiguously because AI-41 lands before AI-36 and AI-37, the two milestones between it and the rest of Wave 5. No new test file was added — the two new tests extend the existing topical files `event_test.go` and `provider_failure_test.go`, and the constructor supporting the first lands in `export_test.go`; test file count is unchanged at **148**, production file count unchanged at **85**. AI-41 discharges both Wave-2 carryovers AI-21 parked and AI-24 assigned to this milestone — two independent subnodes: AI-41.1 (`R-AEE-021`) proves the emission boundary's payload-validation rule directly, by giving the test-only witness payload an additive, nil-by-default controllable rejection and asserting the surfaced failure matches the planted one by identity, sentinel and position, with the boundary's earlier rules shown structurally satisfied; AI-41.2 (`R-AIP-016`) closes the one leaking verb on the provider-failure payload — a redacting `GoString()` delegating to the already-redacted `Error()` rendering, nil-safe by delegation, with no `String()` added since the payload already implements `error` and fmt's dispatch order makes a `String()` on it unreachable dead code. Five commits on `feat/ai-41-wave2-carryovers`: `ca2ede7` (AI-41.1 — proves rule 4 surfaces the payload's own violation), `b82cfdf` (AI-41.2 — redacts `*Failure`'s Go-syntax rendering via `GoString`), `b271637` (narrows a pre-existing, out-of-scope terminal-exclusivity guard's exemption list from three names to the one real collision the new method introduced), `3adbb8c` (archives AI-41 and promotes the Wave-2 carryover specs), `c833a93` (resolves the Judgment Day review's findings on redaction scope and the rule-4 proof). Final aggregate Go-code diff against `origin/main@f2e460d`: **+257/−12 = 269 lines** across the same 4 files (`event_test.go`, `export_test.go`, `provider_failure.go`, `provider_failure_test.go`) — production code is `provider_failure.go` alone, **+27 lines, 0 deletions** — well within the session's 1000-line budget, and above the ~90–125-line apply forecast mainly because of test-evidence density (the D-4 canary loop, the rule-4 assertion set, the S-AIP-008 guard fix, and the Judgment Day correction round). Spec amendments (applied at archive time, full inline promotion per the AI-33/AI-35 precedent): `openspec/specs/ai-event-envelope/spec.md` gains `R-AEE-021` with `S-AEE-071`/`S-AEE-072` inline after `R-AEE-020`, plus an append-only discharge blockquote on the "Carried forward" section recording `W1` closed; `openspec/specs/ai-provider-errors/spec.md` gains `R-AIP-016` with `S-AIP-056`/`S-AIP-057` inline after `R-AIP-015`, before the NFR section; `openspec/specs/ai-stream-testkit/spec.md:39`'s carryover ledger line is amended append-only to read **discharged by AI-41**, naming both `W1` and `W2` and this change, satisfying the milestone's Acceptance line. **Deviation acknowledged and resolved before close**: `sdd-verify` flagged (W-1, WARNING, non-blocking) that apply's fix for a pre-existing, out-of-this-change's-scope guard — `TestErrorEvent_TerminalExclusivity_NeverBothCompletionAndError`'s `S-AIP-008` sub-test, which the new `GoString` method collided with by exported-method-name against the unrelated `Completion.GoString` — exempted three diagnostic-renderer names (`Error`, `String`, `GoString`) when only one (`GoString`) was the real collision; commit `b271637` narrows the exemption to `{GoString}` alone, restoring the guard's detection power for `String` (proof: with the broad exemption, a planted `func (f *Failure) String() string { return "finish reason: stop" }` passed silently; with the narrow exemption it fails, naming the shared non-diagnostic accessor). **Judgment Day review**: two blind judges reviewed the frozen target (HEAD `3adbb8c`) scoped to the four changed Go files and returned **0 SEVERE from either judge**; four findings confirmed by both judges — a pointer-receiver scope gap on `GoString`/`Error`/`Unwrap`/`Is` (classed pre-existing, not opened by this change), the witness payload discarding its `at Path` argument so rule 4's position was unproven, a tautological position assertion, and an unreachable `defer recover()` in the nil-receiver test — plus one single-judge `%q`-verb gap, were all fixed in commit `c833a93`; the witness now threads the received `at` argument into its planted violation, which makes the position assertion actually prove rule 4's `event` prefix (this forced the position field to `*Path` rather than `Path`, since a slice field would break `WitnessPayload`'s pre-existing zero-value comparability). Verify-gate evidence, independently re-run by the orchestrator at final HEAD `c833a93`: `make test` 0 `--- FAIL` lines across all 7 packages under `-race`; `make lint` → `0 issues.`; `make build` exit 0; `git status --porcelain` empty (clean tree); `go.mod`/`go.sum` byte-identical to base, untouched. Verify-report verdict: **PASS WITH WARNINGS** — 2/2 requirements, 4/4 scenarios compliant, **0 CRITICAL**; Judgment Day terminal state: **APPROVED**. W-1 is resolved as described above, not outstanding. W-2 (the design's grep-based blast-radius method cannot see a reflection-based guard such as the one S-AIP-008 turned out to be) is recorded as a **process gap for future milestones**, not a defect in this change — the recommended fix is to grep for `reflect.TypeOf(<Type>` / `NumMethod` in addition to literal verb occurrences whenever a change adds an exported method. The pointer-receiver scope finding is **not** fixed here — `GoString`/`Error`/`Unwrap`/`Is` remain pointer-receiver, so a copied `ai.Failure` *value* is still neither a `fmt.GoStringer` nor an `error` and still leaks every unexported field under `%#v` — that value-form sweep is recorded as a **follow-up owned by AI-36**, whose charter is exactly the exhaustive redaction-absence proof across every failure path. Per-record evidence: [`openspec/changes/archive/2026-08-07-cachicamas-ai-wave2-carryovers/`](../../../openspec/changes/archive/2026-08-07-cachicamas-ai-wave2-carryovers/). Engram backfill: phase observations `sdd/cachicamas-ai-wave2-carryovers/{explore,proposal,spec,design,tasks,apply-progress,verify-report,archive-report}` (Engram ids #2662, #2663, #2665, #2666, #2668, #2670, #2674, #2676). **AI-36 is now unblocked**: this milestone's charter declares `Blocks: AI-36`, and AI-36's adversarial redaction sweep can now run against the provider-failure payload's Go-syntax rendering, which did not exist before this close. **Remaining in Wave 5:** AI-36 (secret redaction), AI-37 (observability).

> **Amended 2026-08-08 — AI-36 close (Wave 5 — Harden, secret redaction).** Shipped counter ~~37 of 42~~ → **38 of 42**; landed AI range ~~AI-00 through AI-35 plus AI-41~~ → **AI-00 through AI-36 plus AI-41** — still stated non-contiguously, but the gap is now AI-37 alone. Production file count ~~85~~ → **86** (the +1 is `src/agenttest/sweep/sweep.go`); test file count ~~148~~ → **152** (the +4 is `src/agenttest/sweep/sweep_test.go`, `src/agenttest/sweep_convergence_test.go`, `src/ai/openaicompat/a_i-36_1_test.go`, `src/ai/openaicompat/a_i-36_2_test.go`). *Both counts are derived from this change's own recorded file list — one created production file, four created test files, no deletion — not from a fresh `find` measurement; a re-measure at the next milestone should confirm them.* AI-36 turns redaction from **a property of which surface a caller reached for** into **a property of the module**, and it **discharges the follow-up AI-41 recorded against itself at its own close** (the copied-value form of the provider-failure payload, doc 0002 line 22). Six work units across the charter's three leaves plus the inherited node: WU-1 (one shared, dependency-free sentinel sweep in `src/agenttest/sweep/`, with the conformance case and the live-smoke sweep converging on it, published names unchanged), WU-2 (adversarial sweep over every triggerable failure path plus a hostile server echoing the caller's authorization header and request body back), WU-3 (configuration, configured-client and header-capture redaction proven verb-exhaustively), WU-4 (the assertion helpers' own failure output proven sentinel-free with a control), WU-5 (the credential scan widened recursively across the whole adapter tree, its silent category exemption replaced by a declared, enumerated, falsifiable allowlist over six inventoried files), WU-6 (the copied-value scope stated and its escaping shape proven unreachable by a structural guard, with **no** rendering behavior added to the payload — `NFR-AIP-B`'s landed total nil-safety is untouched). **Both empirical branches resolved as BITES, not clean.** **D-4**: a hostile server echoing the authorization header and request body into a non-streaming response leaked the caller's own credential verbatim through the refusal's bounded excerpt, observed at unwrap depth 1 under all four verbs before any production change existed — so the contingent requirement `R-AEM-019` **triggered and is binding, not conditional**, and the removal step landed in `stream.go`. **D-5**: the bare, unwrapped configured client leaked the raw token under all four verbs in **both** pointer and value form, because a value reached through an unexported field cannot dispatch to its own redacting renderings; fixed with value-receiver rendering methods mirroring the wrapper's landed precedent. Ten commits on `feat/ai-36-redaction`, based at `origin/main@1103769`, final HEAD **`8e7caa31`**: `ba009d33` (WU-1/WU-2 shared sweep), `438ccb81` (WU-3 config/client/header), `6ba952f6` (WU-4 bounded summary), `3311aadb` (WU-5 recursive credential scan), `bdc5947e` (WU-6 copied-value guard), `4b7088e2` (planning artifacts), `8e7fdc97` (apply-progress), `9e2c149d` (falsifiable header/unwrap guards), `dfcc1663` (Judgment Day round 1), `8e7caa31` (Judgment Day round 2). Aggregate diff against `origin/main`: **36 files, +4997/−130**. `size:exception` was granted by the maintainer **up front**, who authorized automatic raising for this milestone specifically; the forecast was ~1310 and the overrun is driven by both empirical branches biting plus two review rounds — it is an accepted, non-blocking deviation, not an open decision. Spec amendments (applied at archive time, full inline promotion per the AI-33/AI-35/AI-41 precedent): `openspec/specs/ai-provider-conformance-suite/spec.md` gains `R-CNF-027` with `S-CNF-077`…`S-CNF-081`; `openspec/specs/ai-provider-client/spec.md` gains `R-APC-015` with `S-APC-077`…`S-APC-080`; `openspec/specs/ai-request-translation/spec.md` gains `R-ART-022` with `S-ART-090`…`S-ART-094` (`R-ART-004` amended **append-only**, its text and `S-ART-013`…`S-ART-017` unmodified and re-run green); `openspec/specs/ai-stream-testkit/spec.md` gains `R-STK-014` with `S-STK-047`…`S-STK-049`; `openspec/specs/ai-provider-errors/spec.md` gains `R-AIP-017` with `S-AIP-058`…`S-AIP-061`, plus an append-only note on its "Carried forward" `W2` entry recording the carryover closed (AI-41 for the verb surface, AI-36 for the scope residual); `openspec/specs/ai-provider-error-mapping/spec.md` gains `R-AEM-019` with `S-AEM-071`/`S-AEM-072`, promoted **as landed**. **Identifier hazard, deliberately left open:** `R-CNF-020`…`R-CNF-026` are consumed by two archived conformance amendments whose requirements were never promoted into the canonical conformance spec, while `R-CNF-023`/`R-CNF-024` are already cited as binding dependencies from `ai-provider-text-stream` and `ai-provider-completion`. AI-36 took `R-CNF-027` — the first genuinely free identifier — and **neither renumbered nor backfilled the gap**; an identifier note now records it in place. Closing it is a separate, tracked repository defect. Verify-gate evidence, independently re-run by the orchestrator at final HEAD `8e7caa31`: `make test` exit 0 — **0 `--- FAIL`, 2472 PASS, 13 SKIP across 8 packages under `-race`**; `make lint` → `0 issues.`; `make build` exit 0; `git status --porcelain` empty; `go.mod`/`go.sum` byte-identical to base. Verify-report verdict: **PASS WITH WARNINGS** — 6/6 requirements, 23/23 scenarios compliant, **0 CRITICAL**; six separate redaction guards were empirically defeated-and-restored to prove no absence assertion is vacuous, including blinding the shared sweep itself. Of verify's three warnings, **W-1** (a tautological header-admission guard that also missed a second header-reading site) and **W-2** (a dead unwrap-chain guard) are **RESOLVED** in commit `9e2c149d` and are not outstanding. **Judgment Day: two rounds, terminal state APPROVED.** Round 1 (target `9e2c149d`) returned **two different CRITICALs from two judges** — the refusal rendered two response-derived fields but redacted only the excerpt, so a credential echoed into a content-type parameter surfaced verbatim at unwrap depth 1 (reproduced empirically by the orchestrator); and whole-needle replacement could not match a credential bisected by the capture layer's fixed truncation offset, leaving an attacker-chosen prefix — plus three warnings (case-sensitive matching evadable by a case-folding provider, with the guard sharing the blind spot; a placeholder documented as 11 bytes that is 10, with an unenforced growth invariant; a header-read pin claiming to over-match while under-matching). All were fixed in `dfcc1663`. Round 2 (target `dfcc1663`) returned **no SEVERE from either judge**; three findings were fixed in `8e7caa31` — the re-cap truncated without marking itself so a cut excerpt read as complete; the truncation-edge heuristic applied to the content-type header could erase the very media type the refusal must name; and the header pin's comment still overclaimed while missing struct-field-qualified reads and multi-hop aliases. **Two open items the maintainer owns, both unresolved at close:** (1) the Gentle AI runtime attempt ledger is **blocked with `reason: maintainer_decision`** because the final diff exceeds the 1600-line cap declared at acquire time, even though the attempt itself recorded `outcome: passed` — the same class of tooling friction as AI-34 and AI-35; (2) **W-3, awaiting explicit sign-off** — the prompt-body echo through the non-streaming excerpt is left unfixed. It is spec-anticipated (`S-CNF-081` item 2, `R-AEM-019` items 2 and 4, `S-AEM-072`) because suppressing it would defeat the landed requirement that the excerpt stay diagnostically visible, and the **credential's** absence is provably guarded — but it narrows this charter's Goal prose, so it is recorded as an accepted, maintainer-visible residual rather than silently passed. **Informational, deliberately not fixed and recorded for AI-37 or later:** interior non-tail credential prefixes left by non-overlapping replacement; the deliberate-plant allowlist being per-file rather than per-plant (the judges disagreed — one flagged it, the other verified both falsifiability directions hold); a conformance sweep lacking a corpus-non-empty guard; the by-value structural guard not covering composite or package-level variable shapes. Per-record evidence: [`openspec/changes/archive/2026-08-08-cachicamas-ai-redaction/`](../../../openspec/changes/archive/2026-08-08-cachicamas-ai-redaction/). Engram backfill: phase observations `sdd/cachicamas-ai-redaction/{explore,proposal,spec,design,tasks,apply-progress,verify-report,archive-report}` (Engram ids #2679, #2681, #2682, #2684, #2685, #2687, #2688, #2692), plus a Judgment Day round-1 findings observation (#2690). **AI-37 is now unblocked**: AI-36's charter declares `Blocks: AI-37`, and AI-37's observability boundary may not emit a span until the redaction sweep it depends on exists — it now does. **Remaining in Wave 5:** AI-37 (observability).

> **Amended 2026-08-08 — AI-37 close (Wave 5 — Harden, last milestone; Wave 5 complete).** Shipped counter ~~38 of 42~~ → **39 of 42**; landed AI range ~~AI-00 through AI-36 plus AI-41~~ → **AI-00 through AI-37 plus AI-41**, now a single contiguous block through AI-37 (AI-41 remains named separately per its own out-of-order landing). Production file count ~~86~~ → **93** (+7); test file count ~~152~~ → **162** (+10). **File-count provenance, corrected.** Both counts are orchestrator-measured by `find backend/agent/src -name "*.go" -not -path "*/testdata/*"` at HEAD `66e12147` and at pre-AI-37 `origin/main@ff28240a`: the pre-AI-37 measurement returned **91 production / 154 test**, not the **86/152** AI-36 self-reported "from its own recorded file list ... not from a fresh find measurement" (AI-36's own archive report explicitly asked for this re-measurement, § "File-count provenance"). AI-37 itself adds exactly **+2 production** (`src/ai/openaicompat/trace.go`, `src/agenttest/tracetest/tracetest.go` — the latter is test-support infrastructure, not a vendor adapter, but is not named `*_test.go` so it counts as production under this document's own counting convention) and **+8 test** files, landing at the reconciled **93/162**. The intermediate, AI-36-reported 86/152 figure is therefore superseded by the fresh measurement, not by AI-37's own delta.
>
> **What AI-37 delivered.** Layer 1's observability boundary: minimal OpenTelemetry-API spans and attributes drawn from ADR 0005 § D3's twelve-key allowlist, the guard that keeps the SDK and every other OTel module out, and an absolute content denylist proven by absence. Four charter subnodes: AI-37.1 (dependency + import-only guard + two recorded bites), AI-37.2 (span seam + twelve-attribute mapping), AI-37.3 (denylist proven by absence with four non-vacuity guards), AI-37.4 (nil-safe no-op equivalence).
>
> **§ D3's global getter was deliberately declined, in favour of an injected `TracerProvider`.** § D3's import table marks the ecosystem's root package (`go.opentelemetry.io/otel`, the global getter) ✅ for Layer 1, but its measured package closure contains `go.opentelemetry.io/auto/sdk` — a path literally named for the SDK, inside the one milestone whose Acceptance clause is "the OTel API and nothing else from that ecosystem." Layer 1 instead takes a tracer provider **only by injection** — a new optional `TracerProvider` field on `openaicompat.Config`, defaulted with the tracing API's own no-op provider when absent. This is a **strict subset** of what § D3 authorises (importing fewer of the table's paths, never more), so it needed no ADR amendment; the declined row is recorded in the guard's own source. Final dependency shape: **2 direct requires + 1 indirect**, **10 non-stdlib packages** in the closure, all at OpenTelemetry **v1.44.0** — the version the repo-root `go.work` workspace build list already resolved (inherited from a sibling module's own pin), chosen explicitly over `@latest` (v1.45.0) to avoid forcing the whole three-module workspace up a version inside a boundary milestone.
>
> **A hand-rolled recording tracer lives at `src/agenttest/tracetest/`** (the real `tracetest` package lives inside the forbidden OTel SDK, which the forward guard's forbidden-prefix table already excludes and which AI-37 neither weakened nor extended). It embeds the tracing API's own `noop.TracerProvider`/`noop.Tracer`/`noop.Span` types and overrides every method to record, with a `reflect`-based interface-method-count pin (Span=11, Tracer=2, TracerProvider=2) that fails loudly the moment a future SDK version silently grows the interface.
>
> **The forward guard's zero-requires pin was replaced by an exact-require-set assertion plus a set-equality package-closure pin — deliberately version-fragile.** `TestLayer1_ModuleHasNoDependencies_ZeroRequires` is renamed and re-expressed: (1) `go.mod`'s `require` set must equal an exact in-test table of module paths and versions (via `go mod edit -json`, workspace-immune); (2) the module's complete non-standard-library package closure must equal an exact enumerated 10-entry set, by **set equality, not prefix subset** — because the allowlist matcher is prefix-based, a bare ecosystem-root entry would silently admit that ecosystem's metric, baggage and propagation packages the moment a version bump pulled them in; this closure pin is recorded as a requirement of its own (`R-AGM-008`) precisely because it is the load-bearing half. Version fragility is a stated, intentional property: a bump rotates the vendored semconv sub-path and breaks equality on purpose, forcing a deliberate, reviewed update rather than silent drift.
>
> **Two CRITICALs were found and fixed after apply, both the same defect shape on different terminal paths.** The shape: a lost carrier send (the consumer cancelled and stopped draining) left `outcome.failure` nil, so the deferred span finalizer took the **success** branch — `codes.Ok` plus every usage attribute — for a stream whose consumer received nothing. Instance 1, the `[DONE]`-path completion send, was corroborated independently by **both** Judgment Day judges in round 1 and fixed by deriving a typed cancellation failure before the shared return, mirroring the events loop's own established recovery. Instance 2, the in-band-error-frame block-close send, was found by **one** judge in round 2 and independently confirmed by the orchestrator by tracing every one of `run`'s 11 named terminal paths against source — 10 of 11 were already correct via the same `emitFailure`-routing or direct-cancellation-derivation guarantee; only the in-band-error branch had built its failure value **after** attempting a send instead of before. Both fixes hoist failure construction ahead of the send that could lose it — the same reordering `emitFailure` itself already used elsewhere in the file. A third, earlier defect — `retry.count` recorded correctly on every retry-loop exit in production code but asserted by only one of five named rows, so two independent whole-module overlay mutations (hard-wiring the value to zero; deleting the pre-handover recording call) both left the suite green — was found by `sdd-verify` via `go test -overlay` mutation testing before either Judgment Day round began, and fixed test-only with a 5-row retry-outcome table.
>
> **The OpenRouter wrapper gained a `TracerProvider` door** (`R-APC-016`, `S-APC-085`) discovered missing by Judgment Day round 1: without it, the only shipped concrete provider adapter was permanently untraceable even though the underlying adapter's own doc comment called the field "the only door a tracer reaches Layer 1 through." The wrapper now threads the value verbatim to `openaicompat.New`, proven end-to-end by a dedicated discrimination test.
>
> **`finalizeSpan` was widened, not narrowed, to record the exact HTTP status code on every post-handover (mid-stream) failure** (`R-AOB-006`, `S-AOB-041`) — round 1's own spec amendment had already generalised the requirement's text to cover this case; round 2 found the code had not caught up, traced the call graph exhaustively (one `finalizeSpan` call site, one `run` launch site, both guarded so a real `*http.Response` is unconditionally in hand), and widened the code to match the already-general spec text rather than narrowing the spec back — because omitting a status code genuinely in hand is the exact falsehood the requirement exists to prevent.
>
> **Three follow-ups deferred and recorded by name and owner, none blocking archive**: `S-AOB-003` — add the package-closure/`auto/sdk` declination reason to the forward guard's own source comment, alongside the injection reason already there (owner: a later round editing `import_boundary_test.go`). `S-AOB-038` — widen the nil-check-absence guard's directory scan from its own implementing package to all of Layer 1, so the automated guard proves what is currently only independently hand-verified (owner: a later round editing `ai37_noop_equivalence_test.go`). `S-AOB-029`/`R-AOB-007` — give the content-denylist scan a location-carrying result (span + attribute key), not only a vector name, so a leak's failure message can name all three (owner: a later round editing `ai37_denylist_test.go` and the shared sweep).
>
> **Accepted tradeoff, recorded rather than silently shipped**: the terminal-race recovery sends (both CRITICAL fixes) are kept, not dropped, even though they cost a Stamper sequence gap on the abandoned-consumer path (the lost event's sequence number is burned, never delivered) and a bounded ~5s, worst case ~10s, delay before `close(out)`. Kept because `ai.CheckStream` deliberately does not check 1…N sequence contiguity — that is AI-22.3's own charter, confirmed by direct source read — so no shipped assertion fails on the gap; and because dropping the recovery sends would regress the pre-existing `S-AEM-051`/`S-AEM-052` requirement that a caller using a deadline (not an explicit cancel) who keeps draining per this package's own drain-before-close convention must still receive a typed terminal failure on every mid-stream failure shape.
>
> Twenty-six commits on `feat/ai-37-observability`, based at `origin/main@ff28240a`, final HEAD **`66e12147`**. Aggregate diff against `origin/main`: **35 files changed, 6551 insertions(+), 163 deletions(-)** — this figure already includes both committed verify reports (the historical `verify-report.md` and the superseding `verify-report-final.md`) and is the complete, final number. `size:exception` was granted by the maintainer **up front** with automatic raising authorized for this milestone specifically; the forecast was ~1,660 code lines and the overrun is driven by the retry-count remediation round plus two full Judgment Day correction rounds — an accepted, non-blocking deviation. Spec amendments (applied at archive time, full four-part promotion transform per the WAVE-1-ARCHIVE precedent — header rewrite, cross-reference re-resolution, an added canonical-home Status section, body otherwise unchanged): `openspec/specs/ai-observability-boundary/spec.md` created in full, `R-AOB-001`…`R-AOB-009` plus `NFR-AOB-001`/`002`, `S-AOB-001`…`S-AOB-041`; `openspec/specs/agent-module-scaffold/spec.md` gains `R-AGM-001`/`003`/`005` MODIFIED (every prior scenario restated, none lost) and `R-AGM-008` ADDED, `S-AGM-068`…`S-AGM-073`; `openspec/specs/ai-provider-client/spec.md` gains `R-APC-001`/`003` MODIFIED and `R-APC-016` ADDED, `S-APC-081`…`S-APC-085`. Verify-gate evidence, independently re-run by the orchestrator at final HEAD: `make test` exit 0 — 9/9 packages `ok`, 0 `--- FAIL` lines, under `-race`; `make lint` → `0 issues.`; `make build` exit 0; `git status --porcelain` empty (clean tree). **Verify verdict: the round-1 `verify-report.md` FAIL (one real CRITICAL, `retry.count` unasserted) is superseded by `verify-report-final.md` — 0 CRITICAL, 0 code defects, verdict FAIL on the completeness gate alone** (the validator admits only 18/18 requirements + 80/80 scenarios as `pass_with_warnings`; at archive time all eight PARTIAL scenarios named in `verify-report-final.md` had been reconciled to what their shipped instruments actually prove, closing the gate). **Judgment Day terminal state: APPROVED** — two rounds, both CRITICALs independently defeat-tested by the orchestrator via `go test -overlay` (reverting each fix reproduces the original failure; the worktree was never mutated during any probe). 21/21 tasks complete. No commit carries `Co-Authored-By` or any other AI attribution. Per-record evidence: [`openspec/changes/archive/2026-08-08-cachicamas-ai-observability/`](../../../openspec/changes/archive/2026-08-08-cachicamas-ai-observability/). **Wave 6 — Hand off (AI-38, AI-39, AI-40) is now unblocked**: AI-37's charter declares `Blocks: AI-38`, and AI-38's full deterministic conformance run depends on AI-29 … AI-37 all being landed — they now are. **Remaining:** Wave 6 — Hand off, 3 milestones.

> **Amended 2026-08-09 — AI-38 close (Wave 6 — Hand off, first milestone).** SDD-closed counter ~~39 of 42~~ → **40 of 42**. **Landed-on-`main` range unchanged at AI-00 through AI-37 plus AI-41** — AI-38 is SDD-complete (verified PASS, specs promoted, change archived) but ships on branch `feat/ai-38-adapter-conformance`, 16 commits over `origin/main@033baa67`, final HEAD `135b5105`, behind **PR #140 (issue #139), which is OPEN and NOT YET MERGED** at the time of this amendment. The merge is owed by the maintainer, not by this archive step — following the exact precedent `WAVE-1-ARCHIVE.md` recorded for PR #101: when PR #140 merges, the honest edit is to append the merge commit to this paragraph and to the promoted specs' `Introduced by` lines; nothing else about the specs or this amendment changes.
>
> **What AI-38 delivered.** The AI-23 deterministic conformance suite's unscoped entry point, `agenttest.RunConformance`, ran for the first time against a real, shipped adapter (OpenRouter over the real `openaicompat` client) rather than only AI-21's in-process fake. The RED baseline (`TestOpenRouterAdapter_FullConformance`, replacing three `t.Skip`'d per-capability drivers) failed **five** case families — cancellation, terminal, `finish_reason`/refusal, `usage`/absent-vs-zero, and redaction — two more than the three skipped drivers suggested, confirming the design's own binding-evidence gate before any resolution direction was chosen. Each was resolved on a named side, with zero cases removed and zero waivers: two suite-side spec amendments (cancellation now admits either a bare close or exactly one cancellation-category terminal, resolving a real conflict with the promoted `ai-provider-error-mapping` `R-AEM-014` typed-terminal obligation; finish-reason reachability and mid-stream failure-category classification each narrowed to a **dialect-aware** absence/collapse — naming the value/category and the dialect, paired with an anti-escape scenario proving the narrowing cannot become a general skip) and one bridge-side rendering fix (in-band error frames for mid-stream failures, present-fields-only usage rendering). A byte-transparent recording helper (`recordTranscript`, raw `io.ReadAll`, zero normalization) replaced hand-typed SSE fixtures with `testdata/*.sse` goldens behind a drift guard that fails a hand edit; three of the four committed transcripts round-trip byte-identically (the fourth, `reasoning_extension.sse`, hand-authors a vendor wire-extension field with no `ai.Event` preimage to render from and is explicitly excluded, named as such, not silently omitted). The generated nine-entry capability record (`CAP-R-01…05` satisfied, `CAP-O-01/02/03` absent, `CAP-O-04` retry satisfied) is asserted entry-by-entry against a committed expectation; a generated `CAP-O-01 = satisfied` is wired to hard-block and escalate as an AI-29 reopen trigger rather than silently updating the expectation. A two-tier boundary-replay sweep (event-level exhaustive at every byte offset for three eligible ≤1024-byte transcripts, 2514 real HTTP round trips in 1.03s; record-level sampled at `{1, len/2, len-1}` per design's own literal choice) proves the verdict survives adversarial stream fragmentation, anchored against a hard-coded event list so a uniformly broken harness cannot pass vacuously. Retry-declaration parity is now enforced mechanically at all three declaration sites across the module (two by direct parity self-test, the third — blocked from importing the shared source by a genuine Go import cycle — by a raw-bytes literal scan with its own bite-proof).
>
> **Verify trajectory: three rounds, no CRITICAL surviving.** Round 1 found 3 CRITICAL by defeat-testing spec obligations that gates alone could not see (retry parity not mechanically enforced; a stale `go.mod`-zero-requires spec sentence falsified by AI-37's own already-landed OTel dependency; the scoped-run-is-never-evidence rule shipped as prose comments with zero checks) — all fixed in a WU10 remediation round. Round 2 found 0 CRITICAL, a completeness-gate FAIL only (three scenarios PARTIAL pending an honest narrowing of an over-general "every transcript" claim). Round 3, after the WU11 narrowing (audited for honesty — bounded by a structural property, naming the excluded fixture, adding a positive obligation rather than only deleting one) and closing the third retry-parity site plus the scoped-run citation guard: **PASS — 14/14 requirements, 47/47 scenarios, 0 CRITICAL, 0 blockers**, `gentle-ai sdd-verify-validate --requirements 14 --scenarios 47` granting the verdict. **10 of 10** guard-defeat probes across all three rounds bit as designed; zero guards were accepted on inspection alone.
>
> **Receipt-driven review: APPROVED.** Lineage `review-d5bd9a2a0cd4e4e0`, reliability lens, full 33-path inspection, 0 severe findings, 2 WARNING + 2 SUGGESTION recorded as non-blocking follow-ups (`R3-1`…`R3-4`, carried in `ai-adapter-conformance-run/spec.md`'s own "Out of scope" table). Pre-commit, pre-push and pre-pr gates all returned `allow`.
>
> Sixteen commits on `feat/ai-38-adapter-conformance`, based at `origin/main@033baa67`, final HEAD **`135b5105`**. Aggregate diff against `origin/main`: **33 files changed, 4312 insertions(+), 705 deletions(-)**. `size:exception` was pre-approved by the maintainer **up front**, scoped to AI-38, with automatic raising authorized; the runtime attempt ledger was reset twice under that same authority (budget 2000→3500; a remediation-round reopen) — an accepted, non-blocking tooling interaction, the same class AI-34/35/36 each recorded. Four maintainer decisions locked the product forks before design (Engram `#2763`): suite-side cancellation resolution; `CAP-O-04` recorded satisfied; the recording helper proven against a local endpoint only, zero vendor network spend; dialect-aware finish-reason absence with no `ai-provider-completion` `R-ACP-002` reopen. **Four acknowledged deviations**, all recorded in `tasks.md` and none blocking: (1) design D4's literal cancellation-admission text was refined during Phase 2/WU3 — from "bare close OR exactly one terminal, optionally preceded ONLY by block-closing end events" to also tolerating an ordinary non-terminal prefix raced by `select`'s no-case-priority — narrowing the ADMISSION shape only, every rejection guarantee (two terminals, wrong category, post-terminal event, invented Completion) unchanged; (2) the recorded goldens live at `fixtures/testdata/*.sse`, not `conformance/testdata/*.sse` as design's file-changes table listed, because Go's `//go:embed` forbids a `..` path element; (3) `with_usage.sse`'s recorder-round-trip exclusion (deferred pending D6's usage rendering) was closed in the WU10 remediation round once that rendering landed; (4) `tool_call.sse` (1073 bytes) is structurally excluded from the 1024-byte boundary-sweep bound, mirroring the AI-27.3 `checkSweepFixtureBound` precedent exactly, with its own coverage preserved by a dedicated test outside the sweep. A fifth reconciliation, not a deviation: design D8's record-level sweep tier is sampled globally at three relative offsets per its own literal "Choice" text, not per-curated-fixture exhaustively as one task-list sentence could be read to imply — resolved by following the higher-authority design text, recorded in `tasks.md` Phase 7's own evidence.
>
> Spec amendments (applied at this archive step): `openspec/specs/ai-adapter-conformance-run/spec.md` **created in full** — `R-ACR-001`…`R-ACR-006` plus `NFR-ACR-A`, `S-ACR-001`…`S-ACR-021` (21 scenarios) — full four-part promotion transform (header rewrite to `Introduced by`/`Status: live`, cross-reference re-resolution, an added canonical-home `## Status` section, body otherwise unchanged) per the `WAVE-1-ARCHIVE.md` precedent. `openspec/specs/ai-provider-conformance-suite/spec.md` gains `R-CNF-028` **ADDED** (`S-CNF-085`/`086`) and `R-CNF-010`/`011`/`012`/`016` **MODIFIED in place** — every prior scenario restated, none lost, six new scenarios added (`S-CNF-082`…`S-CNF-087`); requirement count **twenty → twenty-one**, scenario count **eighty → eighty-six**, both counted and verified before/after this promotion. `openspec/specs/ai-openrouter-first-provider/spec.md` gains `R-OR-05`/`06` **MODIFIED in place** — every prior scenario preserved, one new scenario added to `R-OR-05` (the AI-29-reopen-block scenario). A pre-existing defect in that same file, **not introduced and not repaired by this change**: the file's own header still reads `Status: DRAFT` and cites the active change-folder path rather than having gone through the four-part promotion transform at all — a divergence this change's own proposal (`explore.md` correction #4) traced to an orphan post-archive revision of `add-openrouter-first-provider` carrying a since-superseded `R-OR-07`; reconciling it is explicitly AI-39's, per that same proposal's scope fence. **WARN-B closed at this promotion**: `R-OR-05`'s "default-model swap" scenario is corrected to name the actual enforcement mechanism (`TestOpenRouterAdapter_DefaultModelChangeFailsUntilSpecAmendment` + the `openrouterDefaultModel` pin, not the model-insensitive capability-record test the prior text named) — the last spec-text inaccuracy verify-report.md carried into promotion.
>
> **Production/test file-count reconciliation not performed at this close.** Unlike AI-33 … AI-37's own close amendments, this archive step had no shell tool available in its worktree session and could not run a fresh `find` measurement against both `origin/main@033baa67` and final HEAD `135b5105` to state an honest delta — the design's own Threat Matrix records AI-38 as "test-only plus one test-support package" (`openaicompat/conformancetest/`), so a small positive delta is expected but not independently measured here. Per this document's own established pattern of flagging an un-reconciled estimate rather than silently repeating a stale one (AI-36's own self-flagged 86/152 figure, corrected by AI-37's close), the **93 production / 162 test** figures above are left as AI-37's own last fresh measurement and are **not** claimed current; reconciliation is owed at the next milestone's close (AI-39 or later) with shell access.
>
> Verify-gate evidence, orchestrator-cited from `verify-report.md`'s own recorded round-3 gate execution (this archive session had no shell tool to independently re-run gates): `make test` exit 0 — **2841 `--- PASS`, 0 `--- FAIL`, 26 `--- SKIP`** (all benign — 25 nested declared-absent-optional-capability/retry-absent subtests plus AI-39's own gated live-smoke top-level skip), under `-race`; `make lint` → `0 issues.`; `make build` exit 0; `go.mod`/`go.sum` byte-identical to `origin/main` (this change adds no NEW module dependency; AI-37's own OTel requires and indirect `xxhash` pre-date this branch's base and are unaffected). Per-record evidence: [`openspec/changes/archive/2026-08-09-cachicamas-ai-adapter-conformance/`](../../../openspec/changes/archive/2026-08-09-cachicamas-ai-adapter-conformance/) (issue #139, PR #140 — https://github.com/witsaba/cachicamas/pull/140, **open**). Engram backfill: phase observations `sdd/cachicamas-ai-adapter-conformance/{explore,proposal,spec,design,tasks,verify-report,archive-report}` (Engram ids #2761, #2762, #2764, #2765, #2768, #2773, plus this report's own id on save). **AI-39 (opt-in live smoke) and AI-40 (Layer 2 readiness handoff) are now unblocked** on the SDD-plan axis — AI-38's charter declares `Blocks: AI-39, AI-40`, and both now have a real conformance artifact to compare against or certify — but AI-39's real-network work should account for AI-38's own scope fence naming `R-OR-07`/`R-OR-08` and the orphan `add-openrouter-first-provider` change-folder reconciliation as explicitly AI-39's, not AI-38's. **Remaining:** Wave 6 — Hand off — AI-39 and AI-40 not yet started, plus AI-38's own merge to `main`, owed by the maintainer.
>
> **Merge recorded 2026-08-10 — the owed back-annotation.** PR #140 merged as **`5bc2da4e`**. This is the honest edit the opening paragraph of this amendment promised at promotion time; the promoted spec's own `Introduced by` line (`openspec/specs/ai-adapter-conformance-run/spec.md`) carries the matching edit, made in the same 2026-08-10 reconciliation.

> **Amended 2026-08-10 — AI-40 close (Wave 6 — Hand off, last milestone; Layer 1 complete).** SDD-closed counter ~~41 of 42~~ → **42 of 42**; Wave 6 (AI-38, AI-39, AI-40) is now fully SDD-closed, and this is Layer 1's last milestone. Production file count 95 → **96** (`src/handoff/doc.go`, +1); test file count 168 → **171** (`src/handoff/handoff_test.go`, `src/ai/example_test.go`, `.../openrouter/conformance/doc_matrix_guard_test.go`, +3) — fresh `find backend/agent/src -name "*.go"` measurements at this close. Note on methodology: this session's fresh count, taken with a literal `-not -path "*/testdata/*"` exclusion as AI-33/34's own prose describes it, disagreed with AI-39's reported 95/168 baseline by exactly three files; a `git ls-tree` measurement at base `b062be74` resolved it — the three `testdata/`-nested compile-fixture `main.go` files (`.../openaicompat/testdata/internal_import_probe/main.go`, `.../ai/testdata/constructed/main.go`, `.../ai/testdata/handrolled/main.go`) are counted as production by every close's actually-applied practice, matching AI-39's own 95/168 exactly once included. AI-39's figure was correct; the stated exclusion in AI-33/34's own prose has never actually excluded these three files in practice, and this note records that for the next reconciliation.
>
> **What AI-40 delivered.** AI-40.1: `src/handoff/`, a new sibling package whose external test package (`TestHandoff_ConsumerProof`; three subtests — drains a scripted stream, surfaces a scripted terminal error, exits boundedly on cancellation) proves a third-party consumer builds a request, drives the AI-21 fake, drains a stream to its terminal event, inspects a scripted failure through the neutral surface only, and honors cancellation within a bounded deadline — zero vendor imports, proven by the existing module-wide boundary guard, unmodified. AI-40.2: four runnable examples in `src/ai/example_test.go` (`ExampleNewRequest`, `ExampleModelProvider_streaming`, `ExampleModelProvider_toolCallReconstruction`, `ExampleFailure_inspection`), each compiled **and run** with a deterministic `// Output:` block under the ordinary test command; the AI-38.2 nine-row capability matrix published in `src/ai/doc.go`, transcribed entry-for-entry from the committed expectation and protected against silent drift by a new read-only test (`TestPublishedCapabilityMatrix_MatchesTheCommittedExpectation`); both inherited publication duties stated beside the matrix — item 6's wire clause published as not exercisable in v1, with AI-17's stream half named unaffected and not reopened in the same sentence, and Layer 2's duty to strip OpenRouter's `reasoning_details` on the wire. AI-40.3: `decision.md` — the frozen v1 surface enumerated by capability (ten categories; three marked experimental: the live-smoke package, token counting, and the `R-CNF-020`…`026` identifier gap), the eighteen-item [completion-checklist](#layer-1-completion-checklist) walk against this session's fresh on-disk read, and the never-cancelled abandoned-consumer posture documented as contract (abandoned-**then**-cancelled tested by AI-23.5 and AI-33.3; never-cancelled marked untestable to termination, no coverage claim).
>
> Doc-0002 reconciliation (this amendment): items 11, 12, 14, 15, 16 and 17 of [the completion checklist](#layer-1-completion-checklist) ticked individually, each citing its own closing evidence — item 11 (AI-33 close amendment, above); item 12 (AI-34 close amendment, above); item 14 (AI-23, its own Wave 3 close amendment on this checklist, plus AI-38.1, AI-38 close amendment above, PR #140 `5bc2da4e`); item 15 (AI-38 close amendment, above); item 16 (AI-36 close amendment, above, plus the suite's own redaction case named on [the traceability spine](#traceability-spine)); item 17 (AI-39.1, PR #142, merged `b062be74`). Item 18 ticked citing AI-40.1/AI-40.3. **Item 6 stays `[ ]` by design**, unchanged — no artifact in this change ticks it or claims it closed. Status line 3 (top of document): 41 of 42 → **42 of 42**; Remaining → **none**.
>
> Four implementation commits on `feat/ai-40-layer2-handoff`, based at `origin/main@b062be74`: `26291554` (AI-40.1 consumer proof), `747e13a9` (AI-40.2 examples), `bbde0221` (AI-40.2 matrix + inherited duties), `7465c856` (SDD planning artifacts + `decision.md`) — plus this doc-0002 reconciliation commit. `size:exception` was pre-accepted by the maintainer up front, scoped to AI-40 only, with automatic raising authorized (`state.yaml: delivery.size_exception`); no chain-strategy decision was needed.
>
> Verify-gate evidence, orchestrator-run at this close: `cd backend/agent && make test` (`go test -race -v ./...`) exit 0 — **1017 `--- PASS`, 0 `--- FAIL`, 1 `--- SKIP`** (the pre-existing, benign `TestOpenRouterAdapter_LiveSmoke` gate-closed top-level skip — AI-39's designed normal state; nested subtest skips such as `cap_retry_absent_reported_not_silent` are indented and not counted by the start-anchored pattern) across all 11 packages, `src/handoff` included; `make lint` → `0 issues.`; `make build` exit 0; `backend/agent/go.mod` and `go.sum` byte-identical to base `b062be74` (empty diff both ways); `git diff b062be74 --stat` shows no rename or move of `src/agenttest` or `src/ai` — the signature guard (`TestModelProviderInterface_SignatureGuard`) and the import-boundary guard (`TestLayer1_ImportsOnlyStdlibAndItsOwnPackages_DenyByDefault`) both ran inside that same full suite and passed, unmodified.
>
> Per-record evidence: not yet archived — this amendment lands at `sdd-apply`; `sdd-verify` and `sdd-archive` remain, and the archive link will be added at that close, following the AI-33…AI-39 precedent. Engram topic keys: `sdd/cachicamas-ai-layer2-handoff/{explore,proposal,spec,design,tasks,decision,apply-progress}` (`explore` Engram `#2799`; `proposal` `#2800`; `spec` `#2801`; `design` `#2802`; `tasks` `#2803`; `decision` and `apply-progress` saved at this apply close). **Layer 1 is complete.** Doc 0003's `AG-03` onward is unblocked.
>
> **Archive and merge recorded 2026-08-10 — the owed close annotations.** `sdd-verify` and `sdd-archive` completed: per-record evidence at [`openspec/changes/archive/2026-08-10-cachicamas-ai-layer2-handoff/`](../../../openspec/changes/archive/2026-08-10-cachicamas-ai-layer2-handoff/) (archive commit `c08a0c75`; PR number recorded by `c409494d`); **PR #144 merged as `7326a813`**. Spec amendments this change landed — the enumeration the paragraph above omitted, against this document's own AI-37/AI-38 precedent: `openspec/specs/ai-layer2-handoff/spec.md` **created in full** (`R-L2H-001`…`R-L2H-009` plus `NFR-L2H-A`/`B`, 42 scenarios — Layer 2's entry gate, including `R-L2H-006`'s normative publication of the `reasoning_details` stripping duty delegated at Wave 4 close); `openspec/specs/ai-provider-conformance-suite/spec.md` acceptance item 10 corrected "eight entries" → **nine** (a stale count AI-35 introduced and AI-38 recorded without fixing).
...
- **The module stays dependency-free until a milestone has an ADR to add one.** `go.mod` carried zero requires from AI-00 until ~~AI-24 selects a transport (its own ADR gate) and~~ ~~AI-37 adds the OTel API (pre-authorized by ADR 0005 § D3, and by nothing else)~~ **AI-37 added the OTel API** (pre-authorized by ADR 0005 § D3, and by nothing else) — see the amendment below.
...
> **Amended 2026-08-08 (AI-37)** — AI-37 added the OpenTelemetry **API** modules, pre-authorized by ADR 0005 § D3 and by nothing else: `go.opentelemetry.io/otel` and `go.opentelemetry.io/otel/trace` (2 direct requires) plus `github.com/cespare/xxhash/v2` (1 indirect, transitively required by `otel/attribute`), all pinned to v1.44.0. The AI-00.3 forward guard's allowlist gained five entries (`otel/trace`, `otel/attribute`, `otel/codes`, `otel/semconv`, `xxhash/v2`) — **not** root `go.opentelemetry.io/otel`, which remains forbidden by omission because Layer 1 takes a tracer provider only by injection, never through that path; `go.opentelemetry.io/otel/sdk`, `.../exporters` and `.../contrib/bridges/otelslog` remain on the forbidden-prefix list, unchanged. Per ADR 0005 § D3's closing blockquote, AI-37 is the second and last milestone permitted to add a dependency; the module's dependency-free bullet above is now historical. Full detail: the AI-37 close amendment above.
...
- **Closing checklist:**
  ...
  3. Whichever is chosen, the abandoned-consumer and cancellation paths get leak assertions — those are the two paths this whole discipline exists for.
     ...
     > **Amended 2026-08-10 — recorded narrowing (living-graph clause; the note AI-22's close owed).** The landed kit asserts the **cancellation** and **abandoned-then-cancelled** paths only; abandoned-**never**-cancelled is deliberately unasserted, because it cannot be tested to termination. The narrowing is canonical at `openspec/specs/ai-stream-testkit/spec.md` `R-STK-010`/`S-STK-030` and reasoned in `stream_kit_leak.go`'s own header; this item's broader wording predates it.
- **Depends on:** AI-22.1.
...
> **Amended 2026-08-10 — items 2 and 3 resolved differently than written, recorded late (living-graph clause; the amendment AI-31's close owed and never made).** *Item 2:* the landed rule is **overwrite, never fold** — the last populated usage chunk wins wholesale (`stream_state.go`, design D10), because this dialect delivers cumulative snapshots, not increments; `openspec/specs/ai-provider-completion/spec.md` `S-ACP-018`/`S-ACP-019` pin exactly the no-double-count outcome this item wanted (three frames of 10/20/30 → 30, never 60), so the *property* holds while the stated *mechanism* ("merges") does not exist. *Item 3:* the exclusivity relation was **not** verified against a real cache-hit transcript by this node — `R-ACP-006` records the adapter's non-attestation on this dialect and routed the discharge forward to AI-38.2. Both resolutions were already canonical in the promoted spec; only this test list still asserted the opposite. Found by the 2026-08-10 whole-layer re-verification.
...
> **Archived 2026-08-09 (AI-39)** by `cachicamas-ai-live-smoke` SDD change. **Evidence trail**: Engram artifacts — explore #2785, proposal #2786, spec #2788, design #2789, apply-progress #2791, verify-report #2792, verify-report-final #2793. **Implementation**: 9 commits on `feat/ai-39-live-smoke` (febf28ff, 00cdacf0, fd3bcd8d, 164b9e75, 3496e546, b4c6809c, a345e8a4, 3a15a961, 2a9763bc), all merged to HEAD. **Verification**: round-1 report (commit a345e8a4, 1 FAIL on completeness) superseded by final report (commit 2a9763bc, 0 CRITICAL, 2 WARNING, 5 SUGGESTION informational; 33/39 scenarios pass, 6 partial credential-gated). **Review**: APPROVED, lineage `review-49adf5db282ef46c`, receipt present, pre-commit gate allow, 4 lenses, 0 severe findings. **Risk acceptance**: credentialled live dispatch was never executed in this environment (no credential present); the milestone's charter makes the live run optional and the gate-closed skip is the designed normal state — this acceptance is recorded explicitly for future maintainers. **Specs promoted**: new `ai-live-smoke` capability (8 requirements R-LSM-001…008, 30 scenarios S-LSM-001…030) created at `openspec/specs/ai-live-smoke/spec.md` via four-part promotion transform; `ai-openrouter-first-provider` modified in place — R-OR-07 and R-OR-08 deltaed (environment-variable-only opt-in, capture-sink sweep binding, positive control), orphan `add-openrouter-first-provider` folder deleted, canonical header promoted to live. **Dependencies**: zero NEW require lines; go.mod/go.sum byte-identical to origin/main (3 pre-existing AI-37 requires, zero NEW from AI-39). **File counts** (production + test): orchestrator `git ls-tree`-measured at this close — `origin/main@5bc2da4e` (AI-38 landed): **94 production / 167 test** `.go` files under `backend/agent`; AI-39 HEAD: **95 production / 168 test** (+1 production is the `testdata/internal_import_probe/main.go` compile-negative fixture, +1 test is `reachability_guard_test.go`; the moved smoke files net zero). This discharges the reconciliation AI-38's close deferred.
...
- [x] The `backend/agent` module exists at the ADR-defined location, and `database_administrator/src/tools/` is untouched.
- [x] **Both** import directions are mechanically guarded, and each guard has been recorded biting.
- [x] Neutral request, message, content and tool contracts are documented and tested.
...
- [x] Cancellation cannot leak goroutines.
- [x] Backpressure is bounded and lossless, with exactly one sanctioned loss path.
- [x] The fake provider supports deterministic Layer 2 tests.
- [x] The conformance suite can be reused for every future adapter.
- [x] The first concrete adapter passes deterministic conformance.
- [x] Secrets and sensitive bodies are absent from diagnostics by default.
- [x] The live test is optional, bounded, and unreachable from any entry point.
- [x] The Layer 2 handoff example compiles without vendor dependencies, and the v1 surface is declared frozen.
