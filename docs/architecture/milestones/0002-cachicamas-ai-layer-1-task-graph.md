...
> **Amended 2026-08-07 — AI-35 close (Wave 5 — Harden, third milestone).** Shipped counter ~~35 of 42~~ → **36 of 42**; landed AI range ~~AI-00 through AI-34~~ → **AI-00 through AI-35**. Test file count ~~143~~ → **148** (the +5 is `retry_test.go`, `a_i-35_1_test.go`, `a_i-35_2_test.go`, `a_i-35_3_test.go`, `conformance_retry_test.go`); production file count unchanged at 85. AI-35 closes Layer 1's retry and idempotency policy — four subnodes: AI-35.1 (pre-stream retry predicate with partial-output boundary marker, R-AIS-041 + R-AIS-043.S-4), AI-35.2 (backoff mechanics with retry-after override and context-aware waits, R-AIS-042), AI-35.3 (replayability and per-attempt drain, R-AIS-043), AI-35.4 (cross-layer composed-bound ceiling, R-AIS-044). Helper package factored into a shared seam at `backend/agent/src/ai/internal/retry/` (Go `internal/` directory per ADR 0005 § D1), exporting `retry.Loop` + `retry.Config` + `retry.AttemptReport` + `retry.DefaultMaxAttempts = 3`; wired into `stream.go:218–227` (the pre-stream seam before carrier handover). Conformance grows from `[8]` to `[9]` capabilities with `CapRetry` (CAP-O-04) added; R-CNF-019 added (7 scenarios); R-CNF-017 totality rebuild is mechanical (one new line). Production change is **+1264/-61** across 15 files (helper + stream wiring + 3 real-producer tests + conformance case + conformance suite extension); `go.mod` unchanged, forward guard `import_boundary_test.go:107–162` green. Spec amendments: `openspec/specs/ai-stream-lifecycle/spec.md` gains R-AIS-041..044 as a dated amendment blockquote under § 7 (failure delivery) at archive time; `openspec/specs/ai-provider-conformance-suite/spec.md` gains R-CNF-019 + modified R-CNF-017 as a dated amendment blockquote at end of file (archive time). Eight implementation commits on `feat/ai-35-retry-policy`: `5b51a20` WU-1 helper package, `94c8fed` WU-2 stream integration + predicate tests, `839e83b` WU-2 fixup context cancellation, `eed2071` WU-3 backoff mechanics, `cecab38` WU-4 replayability + boundary, `daddba1` WU-5 CapRetry + conformance case, `ceb85c8` WU-5 fixup case placement, `6bfc266` lint fix rename. Verify-gate evidence: `make test` PASS (899 PASS / 0 FAIL / 4 pre-existing SKIP, `-race`); `make lint` 0 issues; `make build` 0. Per-record evidence: [`openspec/changes/archive/2026-08-07-cachicamas-ai-retry-policy/`](../../../openspec/changes/archive/2026-08-07-cachicamas-ai-retry-policy/) (issue #131, PR link added at PR open). Engram backfill: phase observations `sdd/cachicamas-ai-retry-policy/{explore,proposal,spec,design,tasks,apply-progress,verify-report,archive-report}` (Engram ids #2642–#2650). **Three acknowledged deviations** documented in archive-report: (1) **conformance case body placement** — moved from `agenttest/` to `openaicompat/` to test the package it exercises; resolves design D-R4 risk; both openaicompat and openrouter adapt inherit the same case without rewriting assertions (AI-38 preview). (2) **size:exception approved at ~1096, actual 1264** — conformance case body grew to 374 lines due to seven-scenario implementation (S-CNF-069..075) plus helper instrumentation; overrun is concentrated in test files (strict-TDD density per AI-33/AI-34 precedent); user approved exception cap ~1500; ledger reset by user (Engram #2649). (3) **lint fix commit after apply closed** — rename `RetryAfterReader` → `AfterReader` to fix package-level stutter (commit `6bfc266`); verify re-ran `make test` and `make lint` with final commit; no spec impact (R-AIS-042 behavior-level, not identifier-level per no-Go-identifier contract). **Remaining in Wave 5:** AI-36 (secret redaction), AI-37 (observability), AI-41 (Wave-2 carryovers).
