...
> [!WARNING]
> **ARCHIVED 2026-08-11 — superseded by the v2 restructure.** This is the v1 (pre-Gherkin) revision of doc 0003, kept verbatim for the record. The living document is [`../0003-cachicamas-agent-layer-2-task-graph.md`](../0003-cachicamas-agent-layer-2-task-graph.md); its inconsistency register records every deliberate difference from this file. No AG milestone had merged when the restructure landed, so the append-only rule was never in tension: every id below survives unchanged in v2. This file is frozen — amendments land only in the v2 document. Relative links were rewritten once for this archive location; nothing else was touched.
