...
- **S-CPB-094** — Given the merged tree, when `make lint` and `gofmt -l` run, then the first exits 0 with no reported issue and the second prints nothing. `make all` is not run — its formatting step rewrites committed files.
...
- `make all` MUST NOT be run: its formatting step rewrites committed files and turns a verification step into a mutation.
