...
- **S-DEL-023** — **(bite)** RED-first, **mandated**. Given a scratch tree in which the test tool builds its child harness on an independent background context instead of the tool's own `ctx`, when `S-DEL-014` runs, then it **FAILS** on assertion 1 — the child's run error does not match the interrupt sentinel, and the child runs on past the parent's interrupt — proving the inheritance is the tool's context derivation rather than an ambient effect. RED-recorded BEFORE `S-DEL-014` is GREEN, with `-count=1`, then reverted.
...
- **S-DEL-020** — **(bite)** RED-first, **mandated**. Given a scratch tree in which gate 5 is removed so a child `cost_turn` is admitted, when `S-DEL-016` runs, then it **FAILS** with the parent's cumulative inflated by exactly the child's spend and the strict inequality broken — proving the refusal is what protects `R-CST-004` and that the forwarder's unfiltered fold (`harness.go:633-635`) is a live hazard rather than a hypothetical one. RED-recorded BEFORE `S-DEL-016` is GREEN, with `-count=1`, then reverted.
