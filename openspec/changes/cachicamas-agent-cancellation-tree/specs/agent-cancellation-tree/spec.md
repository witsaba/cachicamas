...
- **S-CAN-001** — **Charter AG-14.1, scenario 1, first Then.** Given a harness-driven run whose provider stream is held mid-stream and which has a cancellation-observing tool call in flight, when the interrupt signal fires, then the provider stream is cancelled per the Layer 1 fake's cancellation-fidelity contract (`R-CNF-011`/`R-CNF-012`: the channel closes bare, synthesizing no terminal event), the in-flight tool observes cancellation and returns rather than running to completion, orphan synthesis has closed every open call in the transcript with entries whose origin discriminator reads `synthesized` (`R-HIS-007`), the run-end event carries the interrupted run outcome with a nil failure, the run's returned error satisfies `errors.Is` against the interrupt sentinel, and `CheckStream` accepts the whole emitted stream **unmodified**. *(Also certifies `R-CAN-002`.)*
...
| Harness-owned tasks | `runDispatcher` (`scheduler.go:267`), the per-turn event forwarder (`harness.go:274-279`), every per-call goroutine the scheduler starts, and `Run`'s own control flow | MUST all exit within the bound. Every writer of the rejoin slice and of the emissions channel MUST remain a harness-owned goroutine that provably exits, so abandoning the join is forbidden: it would race the still-running call's later slot write and could send on a closed emissions channel. |
...
| Retry or failover on a cancelled turn | **AG-15.** A cancellation is never retried; `R-RUN-011`'s no-retry rule extends verbatim to the cancellation path |
...
| Cross-run transcript state beyond the terminal shutdown flag | **AG-21** (`agent-run-driver/spec.md:72`) |
