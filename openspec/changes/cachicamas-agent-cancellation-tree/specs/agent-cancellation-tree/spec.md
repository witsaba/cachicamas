...
- **S-CAN-001** — **Charter AG-14.1, scenario 1, first Then.** Verified as two independent arms, each proving one of the two cancellation-observation mechanisms.
  ...
  - **Arm B does not assert synthesized origins, and the reason is structural.** A call that reached the scheduler is rejoined and committed synchronously by `finishContinuationTurn` (`loop.go:493-513`) whether it succeeded or failed, so its transcript entry origin is `appended` — the call was resolved, not orphaned. Orphan synthesis (`R-HIS-007`) covers calls that never reached a result at all; demanding `synthesized` origins here would demand an observation this path cannot produce, and would be satisfiable only by breaking the rejoin. The property Arm B actually owns is **no open call survives the wind-down**, which is what `CloseTurn` would reject and what the scenario asserts.
