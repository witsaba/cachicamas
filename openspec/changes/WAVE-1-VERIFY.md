...
> **RESOLVED (2026-08-01) — W3 is closed.** The collision was ended by re-prefixing **AI-05**, not AI-10: `cachicamas-ai-message-roles` now uses `R-AMSG-` / `S-AMSG-`, and `cachicamas-ai-model-request` keeps `R-AMR-` / `S-AMR-` unchanged. AI-05 is the smaller block — 51 distinct IDs against AI-10's 82 — so re-prefixing it moves the fewest citations and leaves every landed Go comment correct as written. **No ID was renumbered**: the numeric part is identical on both sides of the rename, so `R-AMR-011` (totality) became `R-AMSG-011`, and a reader following an old citation lands on the same requirement.
>
> Every citation outside the two delta specs was resolved individually rather than by pattern substitution, because AI-05 and AI-10 overlapped on **every** number AI-05 used — `R-AMR-001`…`011` and `S-AMR-001`…`038` existed in both. Six citations moved, all of them `R-AMR-009` → `R-AMSG-009` in `cachicamas-ai-content-parts` (`decision.md`, `design.md`, `explore.md` ×2, `proposal.md`, `specs/ai-content-parts/spec.md`), where the surrounding sentence is AI-06 superseding AI-05's content-seam placeholder. Everything else — `cachicamas-ai-cache-breakpoints`, `cachicamas-ai-request-extension-points`, AI-10's own artifacts, and all 60 citations in `backend/agent/src/ai` and `backend/agent/src/agenttest` — was verified to mean AI-10 and left untouched. No Go file changed.
>
> The paragraph above is preserved as it was written, so its `R-AMR-011` citations describe the pre-rename state.
