...
- [x] 2.8 GREEN — extend `Run` to iterate: loop calling `Turn` with continuation; per-turn channel + **mandatory** forwarder goroutine relaying events to the consumer sink while the turn is in flight (so `permission_decision_required` is observable before the turn returns); dispatch on finish reason (`ToolCalls`/`PauseTurn` iterate; other members terminal-candidate); `CloseTurn` at each boundary.
- [x] 2.9 GREEN confirm — `S-RUN-010` full kind order, `Run` returns turn-two's message/finish/nil error.
- [x] 2.10 RED — `TestHarness_EachTerminalCandidateFinishReason_EndsAfterOneTurn` (`S-RUN-011`). Note: this scenario alone passed even against the single-turn stub (vacuous in isolation) — real discrimination comes jointly from 2.7/2.12; recorded honestly in apply-progress.md.
- [x] 2.11 GREEN confirm — dispatch is total over `ai.FinishReason`'s vocabulary; each terminal-candidate member ends the run after exactly one turn with an empty queue.
- [x] 2.12 RED — `TestHarness_SteerNearTerminal_AtomicQueueCheckYieldsAdditionalTurn` (`S-RUN-012`, Gate-held final turn).
- [x] 2.13 GREEN — implement the atomic terminal-decision: under the queue mutex, non-empty → take + iterate; empty → mark queue closed in the same critical section, then terminate. No check-then-close race.
- [x] 2.14 GREEN confirm — `S-RUN-012`: `Steer` returned nil, additional turn bracket appears, run terminates only after it. Stable across 10 runs under `-race`.
- [x] 2.15 RED — `TestHarness_EventStream_OneRunBracketContiguousLane_CheckStreamAccepts` (`S-RUN-020`).
- [x] 2.16 GREEN confirm — one run-open/run-close pair, N turn brackets nested, sequence 1..N with no gap/repeat/restart; `CheckStream` accepts unmodified (no new production code needed — composition proof, confirmed literally green on first run per design's own prediction).
- [x] 2.17 RED — `TestHarness_RunIdentity_ConsistentAcrossEventsAndProvenanceDistinct` (`S-RUN-030`, `S-RUN-031`). Composition proof (green on first run); genuine causality proven via scratch-revert (temporarily re-minted a fresh run ID per loop iteration) — RED evidence in apply-progress.md.
- [x] 2.18 GREEN confirm/fix — every event in one run carries the same run identity, asserted by this test, not `CheckStream`; harness-driven identity carries `run-hrn-` prefix, distinct from the loop's `run-lsk-` prefix.
- [x] 2.19 GREEN confirm — `S-RUN-030`/`S-RUN-031`.
- [x] 2.20 RED — `TestHarness_History_AlternatingTranscriptEveryPairMatched` (`S-RUN-040`).
- [x] 2.21 GREEN confirm — read-back holds prompt, turn-one assistant+call, matching result, turn-two assistant, in order; no duplicate/missing; `S-RUN-041` cross-checked at 6.4/6.8 (deferred to Phase 6, next batch).
- [x] 2.22 RED — `TestHarness_LoopAccess_PublicOneTurnSurfaceOnly_SourceScanGuard` (`S-RUN-050`). Composition proof (green on first run, harness.go was already clean); genuine causality proven via scratch-insert (temporary `_ = emitStamped` reference) — RED evidence in apply-progress.md.
- [x] 2.23 GREEN — write the source-scan guard (`scheduler_test.go` regex precedent): `harness.go` contains no reference to any enumerated loop internal (`turnAccumulator`, loop identity minters, `emitStamped`, `closeSink`, the request builder, the finish-reason mapper) and no `.Schedule(` call site.
- [x] 2.24 GREEN confirm — `S-RUN-050`; every capability test declares `package agent_test` (verified by both the regex scan and a directory-scan sub-check). `S-LSK-017`'s per-tool schedule-invocation-count assertion deferred to Phase 6 (S-LSK-008/008a already pin it at the `Turn` level; not re-duplicated here).
- [x] 2.25 RED — **charter AG-13.1 sc.2**, written together per the mirror-of-`S-LSK-003a/b` pattern: `TestHarness_RunStream_ReconstructsHistoryAtRunScope` (`S-RUN-060`) AND `TestHarness_RunStream_ReconstructionBiteDropsTurnTwoEvent` (`S-RUN-061`, bite). Written together with the reconstruction helper (test-only, `package agent_test` — no new production surface, matching design.md's file layout); both passed on first run. Non-vacuity proven separately via scratch (see 2.27).
- [x] 2.26 GREEN — implement run-scope reconstruction (test-side helper `reconstructRunScope`, `harness_test.go`): partition the replayed events by turn bracket, reconstruct each turn's messages + tool outcomes, deep-equal against `history.Entries()` (excluding the run-level prompt, which is never itself emitted as an event — scoping recorded explicitly in the test's own doc comment).
- [x] 2.27 GREEN confirm — `S-RUN-060` deep-equal holds; `S-RUN-061` PASSES because, with exactly one turn-two event dropped from a copy of the slice, the comparator REPORTS divergence. **Non-vacuity independently proven**: a scratch hardcoded (vacuous) text reconstruction kept `S-RUN-060` green but was CAUGHT by `S-RUN-061`, which failed with exactly the predicted message ("did NOT report divergence... vacuous AG-05 W1 failure mode") — verbatim RED evidence in apply-progress.md.
- [x] 2.28 RED — `TestHarness_TurnError_RunEndsTypedNoAppendNoCloseNoRetry` (`S-RUN-100`).
- [x] 2.29 GREEN — implement the failure path: on non-nil `Turn` error, emit `run_end(RunOutcomeFailed, failure)` via `ai.MidStreamFailure` + `NewFailure`, close the sink, return the error; no append, no `CloseTurn`, no retry/fallback.
- [x] 2.30 GREEN confirm — `S-RUN-100`: typed closing brackets then `run_end(Failed)` then sink close; transcript holds only what was committed before failure; provider recorded exactly one request.

## Phase 3: AG-13.2 — Steering queue (`R-RUN-008`)

- [x] 3.1 RED — **charter AG-13.2 sc.1**: `TestHarness_MidTurnSteer_EntersAtBoundaryBeforeNextProviderCall` (`S-RUN-070`) in new `harness_steering_test.go`. Landed in the SAME commit as widening both filters with `/harness_steering_test.go`. **Deviation recorded**: uses `agenttest.Provider`'s own `.Requests()` accessor directly (it already captures every consumed request in call order) instead of a separate `newCtxRecordingProvider`-shaped wrapper — equivalent evidence, less code; the wrapper precedent was for context-capture, which this scenario doesn't need.
- [x] 3.2 GREEN — `steeringQueue` (mutex + FIFO slice + closed flag) and the drain-at-boundary step were already implemented in Phase 2 (S-RUN-002's typed-rejection test required them). This task and 3.3 are therefore composition proofs, not independent RED→GREEN — recorded honestly in apply-progress.md.
- [x] 3.3 GREEN confirm — `S-RUN-070`: in-flight turn's events unchanged from the un-steered baseline; steered message between turn-1 and turn-2 messages; turn-2's recorded request transcript contains it.
- [x] 3.4 RED — **charter AG-13.2 sc.2, first Then**: `TestHarness_SteerBurst_ArrivalOrderZeroDrops` (`S-RUN-071`) — N `Steer` calls from a second goroutine in a test-determined order. **Redesigned mid-implementation**: turn one is tool-calling (ends `ToolCalls`) rather than text-ending, so the burst is drained by the ordinary per-boundary `drain()` — genuinely distinct code path from `S-RUN-072`'s atomic `takeOrClose()`; the first draft (text-ending turn) accidentally exercised the SAME `takeOrClose()` path as 072, discovered when a `drain()`-only scratch bug left the test green.
- [x] 3.5 GREEN confirm — all N appear in arrival order, none missing, none duplicated. Genuine causality proven via scratch-revert (reversed `drain()`'s returned order) — RED evidence in apply-progress.md.
- [x] 3.6 RED — **charter AG-13.2 sc.2, second Then**: `TestHarness_FinalTurnSteer_YieldsNewTurn` (`S-RUN-072`) — Gate-held final turn, steered before release.
- [x] 3.7 GREEN confirm — via 2.13's atomic terminal decision: `Steer` returns nil, an additional turn bracket appears, run terminates only after it.
- [x] 3.8 RED — `TestHarness_SteerAfterTermination_QueueClosedTypedRejection` (`S-RUN-073`). Calls `Steer` twice after termination to prove the closed state persists, not merely transiently empty.
- [x] 3.9 GREEN confirm — typed rejection of `R-RUN-001`/2.5, transcript unchanged; queue is closed, not merely empty.

## Phase 4: AG-13.3 — Pause resumption (`R-RUN-009`)

- [x] 4.1 RED — **charter AG-13.3, both Thens**: `TestHarness_PauseFinish_ResumesVerbatimToRealTerminal` (`S-RUN-080`, `S-RUN-081`, split into two dedicated test functions) in new `harness_pause_test.go`. Landed in the SAME commit as widening both filters with `/harness_pause_test.go`. Script 1: reuses `scriptReasoningTextResponse` (loop_test.go, package-visible) with `PauseTurn` + a non-empty binary round-trip token; script 2: `scriptTextResponse` with `Stop`.
- [x] 4.2 GREEN confirm — via Phase 1's continuation-mode partial append (1.8) and Phase 2's `PauseTurn`-iterates dispatch (2.8): transcript entry for the paused turn deep-equal to the returned message including round-trip token bytes; turn-two's recorded request contains it byte-verbatim; run ends `Completed`. Composition proof (green on first run); genuine causality proven via scratch-revert (dropped `PauseTurn` from the iterate case) — RED evidence in apply-progress.md.
- [x] 4.3 GREEN confirm — `turn_end` carries `TurnOutcomePaused`, forwarded unrewritten; outcome differs from both finished and aborted; no assertion reads an `ai.FinishReason` to establish it. Jointly closes `S-ATT-013`.

## Phase 5: Permission-suspension acceptance clause + `R-APP-002` parked-wait bite (`R-RUN-010`)

- [x] 5.1 RED — **fourth acceptance clause**: `TestHarness_PermissionDefer_RunSurvivesSuspensionAcrossWake` (`S-RUN-090`, jointly `S-APP-015`) in new `harness_suspension_test.go`. Land in the SAME commit as widening both filters with `/harness_suspension_test.go`. Policy defers resolution #1, allows #2; test reads the consumer sink event by event.
- [x] 5.2 GREEN confirm — via Phase 0's injected `*Scheduler` and Phase 2's forwarder goroutine: on reading `decision_required`, a non-blocking read of the run's completion channel shows not-returned and zero tool invocations; `WakeParked(callID)` returns nil; run completes; suspension events sit inside turn one's bracket; `CheckStream` accepts.
- [x] 5.3 GREEN confirm — record evidence; no third resolution path, no timeout (context cancellation propagated unmodified is the only other exit).
- [x] 5.4 RED bite — `TestHarness_PermissionDefer_ParkedWaitObservedBite` (`S-RUN-091`, jointly `S-APP-016`): policy's Resolve#2 asserts a `wake-issued` flag set immediately before `WakeParked` runs. Scratch-replace the parked-wait select with an immediate re-resolution; rerun under `go test -race -count=15 ./src/agent/`; confirm FAILS because the flag is unset at re-resolution (proves the assertion observes the parked **wait**, not the registration); record the `-count=15` output in `apply-progress.md`; revert the scratch.
- [x] 5.5 GREEN confirm — after revert, `git diff` for `scheduler.go`/`harness.go` is byte-empty; `S-RUN-091`/`S-APP-016` pass at `-count=15`.
- [x] 5.6 **Settle the `agent-permission-protocol/spec.md:172` staleness question by running it, not asserting it.** Scratch-delete the AG-10 acknowledgement wait in `Schedule` (`spec.md:172`'s named gap); run the full `agent/` package suite; record the OBSERVED result — whether `TestPermission_WakeParked_AckGatesCompletion_NoRunBeforeSinkDelivery` (AG-10 remediation) already fails it (making `:172`'s "leaves the package green" claim stale) or whether the package genuinely stays green. Record the result in `apply-progress.md` either way; revert the scratch.

## Phase 6: Substrate, spec/code twins, docs, final gates

- [x] 6.1 Confirm both substrate filters (`S-RUN-111`, `S-LSK-016`): identical entry sets, exactly the five `/harness*.go` suffixes, no wildcard/prefix/directory pattern, `loop.go`/`tool.go`/`scheduler.go` already present and untouched in the filter lists.
- [x] 6.2 **`ToolSource` spec/code twin re-home, SAME PR.** Confirm the promoted `agent-tool-scheduler` delta re-homes `agent-tool-scheduler/spec.md:138` to AG-20 (already drafted in `specs/agent-tool-scheduler/spec.md`); re-home the twin code comment at `backend/agent/src/agent/tool.go:239-240` ("`ToolSource` port (G6) is AG-13's widening" → AG-20) in the same commit. Verify `grep -n "AG-13" backend/agent/src/agent/tool.go` returns nothing about a `ToolSource` widening.
...
- [x] 6.5 **Coverage gate.** `TestTurn_CoverageGate` — confirm `loop.go` line coverage ≥ 80% under `make test`, including every new continuation branch (`NFR-RUN-004`); extend harness tests if any branch is unexercised.
- [x] 6.6 Confirm every AG-09/AG-10 scheduler test passes file-unchanged (`S-TLS-015`, final check after Phase 0's early pass at 0.6).
- [x] 6.7 Confirm import guard and ambient-authority guard pass with zero changes over the production and test closures (`harness.go` imports stdlib + `ai` only; no clock, no I/O — the forwarder syncs by channel close, never a sleep); confirm the stamper is touched only between turns (`NFR-RUN-003`), race-free under `-race`.
...
- [x] 6.9 Confirm the five spec deltas' cited lines land verbatim in the promoted specs (already drafted under `specs/`): `agent-loop-skeleton` `R-LSK-001/002/004/006`; `agent-permission-protocol` `R-APP-002`; `agent-history` `R-HIS-010` + `NFR-HIS-003`; `agent-turn-termination` `R-ATT-004`; `agent-tool-scheduler` `R-TLS-012`.
- [x] 6.10 **Docs.** Tick `docs/architecture/milestones/0003-cachicamas-agent-layer-2-task-graph.md:2170` — "A multi-turn run completes with steering, pause resumption, and a complete event story — closed by AG-13"; bump the Layer 2 wave/milestone status header counters to **13/24** (status-header prose only — no count in any requirement or spec text).
