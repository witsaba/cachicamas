...
### R-ATE-009 — A fragment is bounded by the existing text ceiling *(provisional)*

A delta fragment SHOULD be rejected with AI-04's out-of-range sentinel when it exceeds `MaxTextLen` bytes, reusing that existing constant. No new bound is invented at Layer 1. This requirement is `[provisional]`: `design.md` MAY drop or replace it, and MUST record the reason for doing so; if it is dropped, `S-ATE-022` is struck rather than left unimplemented.
...
1. `R-ATE-009` is `[provisional]` — `design.md` owns keeping, changing or dropping the `MaxTextLen` fragment cap, with the reason recorded.
