...
- [x] 9.1 Verify NFR-A/S-051 — `backend/agent/go.mod` still zero `require`; both AI-00 import guards pass; `agenttest` imports only stdlib + `ai`. Verified: `go.mod` unchanged (`module .../agent` / `go 1.26.3`, no `require` block); `go test -race -run 'TestLayer1_ImportsOnlyStdlibAndItsOwnPackages_DenyByDefault|TestLayer1_ModuleHasNoDependencies_ZeroRequires' -v ./src/ai/...` → both `PASS`.
- [x] 9.2 Verify NFR-B/S-052–053 — no diff under `backend/agent/src/ai/`; AI-20 signature guard passes; `src/ai` tests pass unaffected. Verified: `git diff --stat main -- backend/agent/src/ai/` → empty; `TestModelProviderInterface_SignatureGuard` → `PASS`; `go test -race ./src/ai/...` → `ok`.
- [x] 9.3 Modify NFR-E/S-058 — `backend/agent/src/agenttest/doc.go`: state both roles (Layer-1 proof + importable testing library), retain the ADR 0005 Guard C sibling-layout note, state the contract-faithful-not-convenient rule. Done: rewrote the package doc comment with a numbered proof-role/library-role list, a "Contract-faithful, not convenient" section (naming the non-negotiable physics and pointing at design.md for the Gate/exhaustion decisions), and the retained (lightly-updated) sibling-layout section. Verified rendering with `go doc ./src/agenttest` and `go build ./... && go vet ./...`.
- [x] 9.4 Verify NFR-C/S-054–055 — run the full suite twice under `-race`, results identical; grep new `fake_*_test.go` files for `time.Sleep` used as scheduling (only bounded deadlines/documented settle steps allowed). Verified: two forced (`-count=1`) full `agenttest` runs under `-race` → identical PASS/FAIL lines for all 41 top-level results (only the final timing summary differs, 1.491s vs 1.482s). `grep -n "time.Sleep" fake_*.go` → exactly one hit, the documented `settleAfterFakeCancel` (50ms), used only to avoid a test-vs-producer `select` race (Phases 1 and 5) — never to order scripted events. All other timer use is `time.After(boundedFakeTimeout)` inside a `select`, a bounded failure deadline.
- [x] 9.5 Final gate/NFR-F/S-059 — `make test` green, `make lint` clean from `backend/agent/`; confirm every RED/GREEN/REFACTOR field above is filled before closing the milestone. `make test` → 361 `--- PASS`, 0 `--- FAIL`, exit 0, both packages `ok`. `make lint` (first run) → 3 `revive: package-comments` issues: `fake_gate.go`, `fake_provider.go` and `fake_script.go` each attached their milestone-header comment directly to `package agenttest` with no blank line, which golangci-lint treats as an invalid package doc comment (only `doc.go`'s comment is meant to be the package doc — `src/ai`'s own files avoid this by always separating the header from `package ai` with a blank line, a convention this change's three new production files had not followed). Fixed by adding the missing blank line in each of the three files (whitespace-only, no logic change) → re-run: `0 issues.` Final state: `go.mod`/`src/ai` diff against `main` still empty, both AI-00 guards and the AI-20 signature guard still `PASS`. Every RED/GREEN/REFACTOR field in Phases 1–8 above is filled.
