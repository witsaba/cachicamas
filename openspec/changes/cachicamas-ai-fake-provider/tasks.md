...
- [ ] 3.1 RED — S-018–019 (R-AFP-008): every AI-19 failure category scriptable with and without prior output; partial-output discriminator matches the actual stream state. RED output:
- [ ] 3.2 GREEN — confirm `Emit(ai.ErrorEvent(f))` as terminal step (per design's uniform-vocabulary decision) already satisfies; extend only if a gap surfaces. GREEN output:
- [ ] 3.3 REFACTOR — note:
- [ ] 3.4 RED — S-020–021 (R-AFP-009): nothing follows a terminal error (closed-stream next receive); an event scripted after a terminal fails loudly and names the misplaced event. RED output:
- [ ] 3.5 GREEN — implement/confirm terminal-exclusivity enforcement (rely on `ai.Stamper`/`CheckEmit` where possible; add fake-side check only if uncovered). GREEN output:
- [ ] 3.6 REFACTOR — note:
