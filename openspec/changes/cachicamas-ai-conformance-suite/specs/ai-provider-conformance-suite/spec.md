...
> **Capability**: `ai-provider-conformance-suite` — **new**. Promoted to `openspec/specs/ai-provider-conformance-suite/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-conformance-suite/proposal` (obs #2406), `sdd/cachicamas-ai-conformance-suite/explore` (obs #2405)
...
Four items are **real design work**, deliberately not decided here:

1. The concrete signature of `R-CNF-001`'s factory and of `R-CNF-002`'s capability-expectation value, subject to the fixed constraint that both travel together.
2. The case table's concrete keying mechanism that makes `R-CNF-003`'s marking mechanical and `R-CNF-017`'s record total by construction.
3. The concrete drift-guard mechanism of `R-CNF-016` for the seven finish-reason values.
4. How `R-CNF-013`'s sentinel travels from the suite's configuration into each subject's material, given that AI-21's constructor takes only scripts.

Three boundaries are **already decided** and are not design's to reopen: package placement inside `agenttest` (`NFR-CNF-C`), no `src/ai` modification (`NFR-CNF-B`), and the factory-declared capability expectation (`R-CNF-002`).
