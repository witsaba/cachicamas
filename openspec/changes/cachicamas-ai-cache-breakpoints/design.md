...
Each is an assumption about work landing concurrently, not an undecided design point. The full register with files and symbols is `explore.md` § 8; these are the three that would actually change this document.

- [ ] **`Request.Tools()` is the tool accessor** (`explore.md` § 8 row 1). Blocking for `R-ACB-007`'s tools leg. If AI-10.3 landed another spelling, § 3.2's walk takes it; nothing else changes.
- [ ] **`Request.Equal` is exported and compares regions by readback** (`explore.md` § 8 rows 4–5). If it is not exported, `R-ACB-004`'s scenarios compare region-wise in a local helper. If it *is* exported but ignores markers, that is a defect in AI-10.6 to escalate, not to work around: `S-ACB-013` will fail and should.
- [ ] **The final structural rule number** (`explore.md` § 8 row 3). The cap takes the last structural slot whatever its number; only § 3.3's table changes.
