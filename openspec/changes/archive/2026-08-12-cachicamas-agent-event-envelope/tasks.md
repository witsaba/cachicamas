...
- [x] 5.1 Confirm `import_boundary_test.go` and `ambient_authority_test.go` pass with zero changes to their own logic. (NFR-AEV-003) Confirmed: `git diff --stat origin/main` shows no entry for either file.
- [x] 5.2 Confirm `backend/agent/go.mod`, `go.sum`, `Makefile`, `.golangci.yml` are byte-unchanged (`git diff --stat` shows no entry). Confirmed.
- [x] 5.3 Run `cd backend/agent && make test` (`go test -race -v ./...`) and `make lint`; confirm both green/clean, recorded pre- and post-change. **`make test`**: all 12 packages `ok`, zero FAIL/DATA RACE, both pre-change (baseline) and post-change. **`make lint`**: `./src/agent/...` scoped alone is 0 issues (after fixing 3 self-introduced `package-comments` findings — a missing blank line before `package agent`, mirroring Layer 1's own uniform convention, applied to all 7 new production files for consistency). Full-module `make lint` exits 1 solely due to ONE PRE-EXISTING, unrelated finding (`src/ai/openaicompat/openrouter/conformance/doc_matrix_guard_test.go:17`, `var-naming` underscore in package name) — verified zero diff vs `origin/main` and reproduces in complete isolation; not introduced by AG-04, out of its charter to fix (frozen Layer 1 vendor-adapter surface). Recorded honestly rather than silently claimed clean.
- [x] 5.4 Update the status header of `docs/architecture/milestones/0003-cachicamas-agent-layer-2-task-graph.md` (currently "4 of 24 milestones shipped", AG-03 named latest) to "5 of 24", naming AG-04 (event envelope + ordering invariants) as newly landed, following the same pattern as AG-03's own status-header update (`dbd7a33e`). Done.
- [x] 5.5 Assemble every recorded RED (S-AEV-001…102, tasks 1.6–1.10, 2.4–2.9, 3.4–3.7, 4.3–4.7) into the PR evidence section; write the PR description stating why the change exceeds the default 400-line budget, citing the pre-authorized `size:exception` against the 1000-line budget. (NFR-AEV-004) See apply-progress.md's TDD Cycle Evidence table. **Note**: the actual size (2687 changed lines in `backend/agent/src/agent`, 3608 including openspec planning docs) exceeds even the 1400–2200 forecast's own upper bound and the session's 1000-line pre-authorized ceiling — flagged explicitly, not silently rounded down, mirroring AG-03's own precedent.
- [x] 5.6 Confirm `git status` clean and no scratch violation file remains in the merged diff. Confirmed after each bite via targeted `git diff --stat`; final confirmation below.
