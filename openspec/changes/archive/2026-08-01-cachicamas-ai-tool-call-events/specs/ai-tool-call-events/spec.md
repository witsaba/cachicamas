...
> **ARCHIVED DELTA — historical record.** The live contract is [`openspec/specs/ai-tool-call-events/spec.md`](../../../../../specs/ai-tool-call-events/spec.md), promoted from this file on 2026-08-01. This copy is preserved as the artifact AI-18 was verified against. Relative links below were re-resolved for the archive path; nothing else was changed.

> **Change**: `cachicamas-ai-tool-call-events` · **Milestone**: AI-18 — Add tool-call delta events (Wave 2 "Stream")
> **Nodes**: AI-18.1 `[leaf]` call lifecycle · AI-18.2 `[leaf]` delta optionality · AI-18.3 `[leaf]` interleaving and ordinal
> **Capability**: `ai-tool-call-events` — **new**. Promoted to `openspec/specs/ai-tool-call-events/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-tool-call-events/proposal` (obs #2346) · [`explore.md`](../../explore.md)
> **Requirement IDs**: `R-ATC-0NN` · **Scenario IDs**: `S-ATC-0NN` — prefix re-verified unused across `openspec/specs/` and `openspec/changes/` at spec time
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml` `rules.specs`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-STR-10` event, `V-STR-11` event kind, `V-STR-12` payload, `V-STR-14` block, `V-STR-15` block index, `V-STR-16` delta, `V-STR-17` ordering invariant, `V-STR-21` call ordinal, `V-FAIL-13` redaction
> **Binding predecessors**: [tool messages](../../../../../specs/ai-tool-messages/spec.md) — `R-ATM-002`/`R-ATM-003` byte exactness, `R-ATM-005` one rule set, `R-ATM-006` derived ordinal · [validation error taxonomy](../../../../../specs/ai-validation-errors/spec.md) — every rule violation reports through AI-04's failure value and its landed sentinels
> **Binding sibling**: [AI-16 text events](../../../2026-08-01-cachicamas-ai-text-events/specs/ai-text-events/spec.md) — `R-ATE-003`/`R-ATE-004` fix the shared block-index space as **1-based with 0 as a rejected sentinel**. This spec adopts that convention verbatim; it does not restate a competing base.
> **Depends on**: AI-09 (shipped), AI-15 (response lifecycle), transitively AI-14 (envelope, kind registry, block index). **Parallel with**: AI-16, AI-17
> **Sources**: doc 0002 §§ AI-18.1 … AI-18.3 · proposal Locked Decisions **D1**, **D2**, **D3**
...
`openspec/specs/` was re-read at spec time. It contains **no event capability** — AI-14's and AI-15's specs have not landed. There is therefore **no MODIFIED requirement in this change**: this is a full new capability spec. `R-ATM-006` and `V-STR-21` are **restated, never re-defined**; neither file is amended, because this change stores no ordinal.
...
1. AI-14/AI-15 have not landed on disk. Every symbol-level assumption here (registry shape, payload interface spelling, sentinel names, position rendering, block-index type) is subject to the proposal's reconciliation gate before `sdd-apply`.
2. `design.md` owns whether a size ceiling applies to an argument fragment or to the call-end's complete argument bytes. This spec deliberately states none, because AI-09 ships no argument-length constant; inventing one at Layer 1 would be a new bound.
3. `design.md` owns whether call-start, call-delta and call-end share one payload struct family or three independent structs, provided `R-ATC-001` and `R-ATC-012` both hold.

> **Deviation note**: this artifact exceeds the sdd-spec 650-word budget. The landed capability-spec precedent in this repository (`openspec/specs/ai-tool-messages/spec.md`) and the sibling AI-16 spec carry per-requirement scenario blocks, non-functional requirements and acceptance criteria at this density; house convention wins.
