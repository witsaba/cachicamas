...
>
> 1. **The out-of-scope lint issue is closed.** AI-17's own NFR close-out (`4ceb77c`) applied the same blank-line fix to `reasoning_event.go`. `make lint` is `0 issues` at wave head, and `make lint`'s exit code is now 0 for the whole module.
> 2. **The design deviation recorded above is the wave's single most consequential finding**, and the Wave 2 verify report catalogues it as **D13** — "a design error, caught at apply. This is the gate working." It is the only case in Wave 2 where a reconciliation gate actually bit, and it bit because this milestone's own `design.md` had written its unverified assumption down as an open question rather than assuming it away.
> 3. **One side effect outlived the correction.** AI-14 had deferred `CheckEmit` rule-4's failure-path coverage to "AI-15+", assuming a later milestone would ship a payload whose `validate` could fail *after* construction. The eager-validating model this run correctly adopted designed that receiver away, and nobody connected the two decisions. Escalated to **W1** in the verify report and owned by Wave 3 — either give `WitnessPayload` a controllable failure mode, or record rule 4 as deliberately unreachable defence.
