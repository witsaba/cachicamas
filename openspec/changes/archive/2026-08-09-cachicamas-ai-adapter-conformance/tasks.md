...
- [x] 8.1 `make test` (repo root, `-race`): zero `t.Skip` in conformance drivers, AI-21 fake still green. Maps NFR-ACR-A/S-ACR-019, R-ACR-001/S-ACR-002.
...
- [x] 8.3 If Phase 0 evidence diverges from the predicted 5-family set, escalate per design before continuing — do not silently widen resolutions.
  ...
  **8.2 evidence**: `make lint` — final run exit 0, "0 issues." One real finding was caught and fixed on the first run: `fixtures/with_usage.go`'s WU2-authored comment had a line starting `go:embed (task 1.3)...` (prose, not a directive) that `staticcheck`'s SA9009 flagged as an ineffectual/malformed compiler-directive attempt (a real `//go:embed` needs no space after `//`); reworded so no comment line starts with the token `go:embed`. `make build` (`go build -trimpath ./...`) exit 0.
