...
> **ARCHIVED DELTA — historical record.** The live contract is [`openspec/specs/ai-reasoning-events/spec.md`](../../../../../specs/ai-reasoning-events/spec.md), promoted from this file on 2026-08-01. This copy is preserved as the artifact AI-17 was verified against. Relative links below were re-resolved for the archive path; `R-ARE-013`'s stale `[provisional]` marker was resolved (see the requirement); nothing else was changed.

> **Change**: `cachicamas-ai-reasoning-events` · **Milestone**: AI-17 — Add reasoning delta events (Wave 2 "Stream")
> **Nodes**: AI-17.1 `[leaf]` block lifecycle · AI-17.2 `[leaf]` token delivery · AI-17.3 `[leaf]` redacted and signature-only streams
> **Capability**: `ai-reasoning-events` — **new**. Promoted to `openspec/specs/ai-reasoning-events/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-reasoning-events/proposal`
> **Requirement IDs**: `R-ARE-0NN` · **Scenario IDs**: `S-ARE-0NN` — prefix re-verified unused across `openspec/specs/` and `openspec/changes/` at spec time (`R-ARC` is AI-07's, `R-ATE` is AI-16's)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml` `rules.specs`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-REQ-09` reasoning content, `V-REQ-10` reasoning state, `V-REQ-11` round-trip token, `V-STR-10` event, `V-STR-11` event kind, `V-STR-12` payload, `V-STR-14` block, `V-STR-15` block index, `V-STR-16` delta, `V-STR-17` ordering invariant
> **Binding predecessors**: [reasoning content](../../../../../specs/ai-reasoning-content/spec.md) `R-ARC-002`, `R-ARC-004`…`R-ARC-009` (consumed unchanged) · [validation error taxonomy](../../../../../specs/ai-validation-errors/spec.md) — every rule violation here reports through AI-04's failure value and its landed sentinels
> **Depends on**: AI-07 (landed), AI-15, transitively AI-14. **Parallel with**: AI-16, AI-18
> **Sources**: doc 0002 §§ AI-17.1 … AI-17.3 (lines 968–998) · doc 0001 § 3.1, § 3.2, § 4.3 invariant 1
...
`openspec/specs/` was re-read at spec time. It carries **no event capability** — AI-14's and AI-15's specs have not landed — so this is a **full new capability spec with no MODIFIED requirement**. `ai-reasoning-content` is consumed unchanged and is not modified here. The proposal's conditional stands for the reconciliation gate: if AI-14's kind vocabulary lands as a closed set before `sdd-apply`, appending three kinds to it is a spec-level change to *that* capability and MUST be added here as a delta at that time, not assumed.

## Cross-check performed at spec time (D3)

AI-16's landed spec `R-ATE-004` declares one block-index space per stream, shared across all content families, and states it as **binding on AI-17**. Its `R-ATE-003` fixes that space as **1-based, with 0 a rejected sentinel**, and records that the 0-based phrasing illustrating the collision in AI-17's proposal decision D3 must be reconciled here. **This spec adopts `R-ATE-003` and `R-ATE-004` verbatim**: the shared space is 1-based, the first block of a stream carries index 1, and 0 is rejected at construction. No 0-based reasoning-block index exists anywhere in this capability. AI-18's tool-call *ordinal* remains a distinct concept from the block index.
...
> **Resolved 2026-08-01 at Wave 2 archive.** The last sentence carried a `[provisional]` marker at spec time — ~~"`design.md` MAY replace it with a carry-and-report reading … if it is dropped, `S-ARE-036` is struck"~~. `design.md`'s first architecture decision resolved it to **KEPT**, and the rejection ships and is enforced at `reasoning_event.go:189`. The marker is removed rather than frozen into the archive, because a landed and tested rule must not read as an open option. The design rationale is retained as context below.
...
1. ~~`R-ARE-013`'s "a redacted block carries no text" sentence is `[provisional]` — `design.md` owns keeping or replacing it, with the reason recorded, and strikes `S-ARE-036` if it is dropped.~~ **Closed at design time**: resolved to **KEPT**, shipped and enforced at `reasoning_event.go:189` via `ErrMisplaced` at `"fragment"`. `S-ARE-036` stands and passes.
2. The proposal's open question 1 (may a consumer *display* redacted reasoning at all?) is a Layer 2 rendering policy. This spec takes no position beyond `R-ARE-012` making the signal available before the first delta.
3. AI-14/AI-15/AI-16 have not landed in this worktree. Every symbol-level assumption here (registry shape, payload interface spelling, sentinel names, position rendering, AI-16's block-index field spelling) is subject to the proposal's mandatory reconciliation gate before `sdd-apply`.

> **Deviation note**: this artifact exceeds the sdd-spec 650-word budget. The landed capability-spec precedent in this repository (`openspec/specs/ai-reasoning-content/spec.md`, `openspec/specs/ai-completion-metadata/spec.md`) and the sibling AI-16 spec carry per-requirement scenario blocks, non-functional requirements and acceptance criteria at this density; house convention wins.
