...
> **ARCHIVED DELTA — historical record.** The live contract is [`openspec/specs/ai-text-events/spec.md`](../../../../../specs/ai-text-events/spec.md), promoted from this file on 2026-08-01. This copy is preserved as the artifact AI-16 was verified against. Relative links below were re-resolved for the archive path; `R-ATE-009`'s stale `[provisional]` marker was resolved (see the requirement); nothing else was changed.

> **Change**: `cachicamas-ai-text-events` · **Milestone**: AI-16 — Add text delta events (Wave 2 "Stream")
> **Nodes**: AI-16.1 `[leaf]` lifecycle · AI-16.2 `[leaf]` byte fidelity · AI-16.3 `[leaf]` zero-delta blocks
> **Capability**: `ai-text-events` — **new**. Promoted to `openspec/specs/ai-text-events/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-text-events/proposal`
> **Requirement IDs**: `R-ATE-0NN` · **Scenario IDs**: `S-ATE-0NN` — prefix re-verified unused across `openspec/specs/` and `openspec/changes/` at spec time
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml` `rules.specs`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-STR-10` event, `V-STR-11` event kind, `V-STR-12` payload, `V-STR-13` sequence, `V-STR-14` block, `V-STR-15` block index, `V-STR-16` delta, `V-STR-17` ordering invariant
> **Binding predecessor**: [validation error taxonomy](../../../../../specs/ai-validation-errors/spec.md) — every rule violation here reports through AI-04's failure value and its landed sentinels
> **Depends on**: AI-15 (response start / completion kinds), transitively AI-14 (envelope, kind registry, per-stream sequence). **Parallel with**: AI-17, AI-18
> **Sources**: doc 0002 §§ AI-16.1 … AI-16.3 (lines 937–966) · doc 0001 § 4.3 invariant 1
...
`openspec/specs/` was re-read at spec time. It contains **no event capability** — AI-14's `ai-event-envelope` and AI-15's `ai-stream-events` specs have not landed. There is therefore **no MODIFIED requirement in this change**: this is a full new capability spec. The proposal's conditional stands for the *reconciliation gate*: if AI-14's kind vocabulary lands as a closed set before `sdd-apply`, adding three kinds to it is a spec-level change to that capability and MUST be added here as a delta at that time, not assumed.
...
> **Resolved 2026-08-01 at Wave 2 archive.** This requirement carried a `[provisional]` marker at spec time — ~~"`design.md` MAY drop or replace it … if it is dropped, `S-ATE-022` is struck"~~. `design.md`'s first architecture decision resolved it to **KEPT**, and the cap ships and is enforced at `text_events.go:145`. The marker is removed rather than frozen into the archive, because a landed and tested rule must not read as an open option. The design rationale is retained as context below.

**Why this bound and not another.** `MaxTextLen`'s own rationale — make an unbounded value decidable from the request alone (`V-REQ-24`) — applies verbatim to a fragment, which would otherwise be the only unbounded caller-supplied byte carrier in Layer 1. The cap is unobservable to a correct producer, because a fragment of a legal text is never longer than the text and `NewText` already bounds that at `MaxTextLen`. A new, smaller per-fragment constant was rejected: any smaller cap could reject a fragment that a legal complete text can contain. **Recorded asymmetry**: only the single fragment is bounded; the block's reconstructed total is not, because bounding it requires accumulation, which `R-ATE-011` forbids at Layer 1.
...
1. ~~`R-ATE-009` is `[provisional]` — `design.md` owns keeping, changing or dropping the `MaxTextLen` fragment cap, with the reason recorded.~~ **Closed at design time**: resolved to **KEPT**, shipped and enforced at `text_events.go:145`. `S-ATE-022` and `S-ATE-023` both stand and both pass.
2. AI-14/AI-15 have not landed. Every symbol-level assumption here (registry shape, payload interface spelling, sentinel names, position rendering) is subject to the proposal's mandatory reconciliation gate before `sdd-apply`.
3. AI-17's 0-based illustrative phrasing in its D3 must be reconciled to `R-ATE-003` in its own spec, not in this one.

> **Deviation note**: this artifact exceeds the sdd-spec 650-word budget. The landed capability-spec precedent in this repository (`openspec/specs/ai-completion-metadata/spec.md`, `openspec/specs/ai-cache-breakpoints/spec.md`) carries per-requirement scenario blocks, non-functional requirements and acceptance criteria at this density; house convention wins.
