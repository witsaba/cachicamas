...
- [x] 7.1 Update `docs/architecture/milestones/0003-cachicamas-agent-layer-2-task-graph.md`: tick AG-20's row(s), append narrative sentence, bump counter to 20/24, mark Wave 5 complete. **Correction round 2** (`sdd-verify` MINOR-4): the counter this task bumped to "20/24" is itself corrected to **21/24** (a pre-existing off-by-one this task's own arithmetic carried forward rather than caught), and a stale "5 of 7 wave-2" is corrected to "Wave 2 complete" — see `apply-progress.md`'s MINOR-4 row. AG-20's own two ticks and narrative sentence were independently confirmed accurate by `sdd-verify` and are unchanged.
- [ ] 7.2 **[ARCHIVE OBLIGATION — sdd-archive]** Promote `agent-hook-taxonomy` + merge 10 deltas. When promoting `agent-pre-request-hook`, replace the header's "6 charter → 7 spec + 2 bites = 9 total" (`spec.md:7`, `:11-15`, `:147`) with the allocated ID range + named bites — the `S-LSK-020` treatment. (Item 3) **Now 11 deltas** (`agent-cancellation-tree` added during `sdd-verify`, CRITICAL-2) — `sdd-archive` MUST also promote it and extend that spec's own Allocated IDs line with `S-CAN-016`/`S-CAN-017`, per that delta's own header note.
...
- [x] 7.4 **[VERIFY OBLIGATION — sdd-verify]** Judge on evidence the 6 ADDED-not-MODIFIED requirements (`R-CMP-015`, `R-LSK-008`, `R-RUN-013`, `R-ATT-010`, `R-DEL-011`, `R-AGS-016`, `R-AIV-014`) — confirm each targeted original requirement is genuinely not falsified (only needing back-annotation) by opening the cited text, rather than accepting the deferral claim. (Item 5) **DISCHARGED by `sdd-verify`**: each targeted original opened; verdict DEFENSIBLE — none falsified. `agent-cancellation-tree`'s own `R-CAN-006`/`R-CAN-008` amendment (CRITICAL-2) is a separate, later finding, not one of these six, and is its own MODIFIED delta rather than a back-annotation. See `verify-report.md` §5 and this file's own `apply-progress.md` mapping table (CRITICAL-2 row).
