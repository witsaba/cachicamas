...
> **ARCHIVED DELTA — historical record.** The live contract is [`openspec/specs/ai-provider-errors/spec.md`](../../../../../specs/ai-provider-errors/spec.md), promoted from this file on 2026-08-01. This copy is preserved as the artifact AI-19 was verified against. Relative links below were re-resolved for the archive path; nothing else was changed.

> **Change**: `cachicamas-ai-provider-errors` · **Milestone**: AI-19 — Wave 2 "Stream" **keystone**, deliberately before AI-20
> **Nodes**: AI-19.1 `[leaf]` terminal error event · AI-19.2 `[leaf]` category vocabulary · AI-19.3 `[leaf]` retry hints + safe metadata · AI-19.4 `[leaf]` partial-output discriminator · AI-19.5 `[leaf]` one vocabulary, two paths
> **Capability**: `ai-provider-errors` — **new**. Promoted to `openspec/specs/ai-provider-errors/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-provider-errors/proposal`
> **Requirement IDs**: `R-AIP-0NN` · **Scenario IDs**: `S-AIP-0NN` — prefix re-verified unused across `openspec/specs/` and `openspec/changes/` at spec time
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml` `rules.specs`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-FAIL-05` provider/transport failure, `V-FAIL-06` failure category, `V-FAIL-07` retryability, `V-FAIL-08` partial output, `V-FAIL-09` partial-output discriminator, `V-FAIL-10` terminal error event, `V-FAIL-11` pre-stream delivery, `V-FAIL-12` mid-stream delivery, `V-FAIL-13` safe metadata. Cited by identifier, never redefined
> **Binding neighbours**: `V-STR-18` terminal-event container (AI-14) · `V-FAIL-15` Layer 1 retry clause (AI-35) · `V-OUT-11` retry/failover policy (Layer 2) · [`ai-validation-errors`](../../../../../specs/ai-validation-errors/spec.md) `R-AIE-006` content-free rendering, `R-AIE-010` caller-contract-only reservation
> **Closes**: **C4** by construction · the Layer 1 half of **G8**
> **Sources**: doc 0002 §§ AI-19.1 … AI-19.5 (lines 1035–1097) · doc 0001 § 3.1 **C4**, § 4.3 invariant 4, § 7 **G8**
...
`openspec/specs/` was re-read at spec time. It carries **no provider-failure capability**, and AI-14's and AI-15's event specs have not landed in this worktree. This is therefore a **full new capability spec with no MODIFIED requirement**. `ai-validation-errors` `R-AIE-010` already *reserves* this surface for AI-19 rather than describing it, so no requirement of it changes. `ai-contract-vocabulary` needs no append: `V-FAIL-05` … `V-FAIL-13` already exist.

Every symbol-level assumption about AI-14's envelope (sealed payload interface, kind derivation, validation shape) is subject to the proposal's reconciliation gate before `sdd-apply`.
...
1. **AI-19.6 split trigger — checked at spec time, not triggered.** The charter's minimum category list is nine members, and doc 0002's trigger reads "category-specific metadata (rate-limit reset, quota identity) grows the list past seven items". This spec adds **no category-specific metadata field**: `R-AIP-008`'s retry-after is uniform across categories, not rate-limit-specific. The trigger is therefore about metadata-driven growth, not the base member count, and AI-19.6 is **not** appended. `sdd-tasks` MUST re-check this against the landed category count.
2. **The raw-label bound is `design.md`'s.** `R-AIP-006` requires bounded and sanitized; whether an over-long label truncates or rejects, and the exact bound, is a design decision with the reason recorded. `S-AIP-019` accepts either, so the scenario does not need rewriting.
3. **AI-14's sealed-payload assumption.** The C4-by-construction argument in `R-AIP-001` assumes AI-14 ships a sealed payload interface. If AI-14 landed it exported and unsealed, the in-package location remains correct but the "not expressible" phrasing weakens to "not constructible by accident"; re-verify at design sign-off.
4. **Sentinel granularity.** `R-AIP-014` requires a sentinel per category. Whether an additional umbrella sentinel for "any provider failure" ships is `design.md`'s call; if it does, it MUST NOT become an alternative to the per-category sentinels.

> **Deviation note**: this artifact exceeds the sdd-spec 650-word budget. The landed capability-spec precedent in this repository (`openspec/specs/ai-completion-metadata/spec.md`, and AI-16's `specs/ai-text-events/spec.md` in this same wave) carries per-requirement scenario blocks, non-functional requirements and acceptance criteria at this density; house convention wins.
