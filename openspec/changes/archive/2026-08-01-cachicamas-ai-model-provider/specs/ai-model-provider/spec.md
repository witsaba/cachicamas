...
> **ARCHIVED DELTA — historical record.** The live contract is [`openspec/specs/ai-model-provider/spec.md`](../../../../../specs/ai-model-provider/spec.md), promoted from this file on 2026-08-01. This copy is preserved as the artifact AI-20 was verified against. Relative links below were re-resolved for the archive path; nothing else was changed.

> **Change**: `cachicamas-ai-model-provider` · **Milestone**: AI-20 — Define the provider interface (Wave 2 "Stream", the wave's **join point**)
> **Nodes**: AI-20.1 `[leaf]` the interface · AI-20.2 `[leaf]` pre-stream contract · AI-20.3 `[leaf]` mid-stream contract · AI-20.4 `[guard]` signature guard · AI-20.5 `[leaf]` optional capabilities
> **Capability**: `ai-model-provider` — **new**. Promoted to `openspec/specs/ai-model-provider/spec.md` at archive.
> **Predecessor**: [`proposal.md`](../../proposal.md) · Engram `sdd/cachicamas-ai-model-provider/proposal` (obs #2347)
> **Requirement IDs**: `R-AMP-0NN` · **Scenario IDs**: `S-AMP-0NN` — prefix re-verified unused across `openspec/specs/` and `openspec/changes/` at spec time
...
> [`ai-stream-lifecycle`](../../../../../specs/ai-stream-lifecycle/spec.md) — AI-02.1 § 3 carrier, § 4 ownership, § 5 cancellation, § 6 buffering, § 7 failure delivery, **§ 9 the eight statements**, § 10 "AI-20 — the provider interface";
> [`ai-minimum-capabilities`](../../../../../specs/ai-minimum-capabilities/spec.md) — AI-03.1 § 9 discovery, § 12 "AI-20.5";
> [`ai-contract-vocabulary`](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-PRV-01` … `V-PRV-08`, `V-PRV-16`, `V-PRV-17`, `V-STR-04` … `V-STR-09`, `V-FAIL-10` … `V-FAIL-12`, `V-REQ-22`;
> [`ai-model-request`](../../../../../specs/ai-model-request/spec.md) and [`ai-validation-errors`](../../../../../specs/ai-validation-errors/spec.md) — the request and the failure value on the pre-stream path
> **Depends on**: AI-03, AI-10, AI-12, AI-14, AI-19 · **Blocks**: AI-21 and everything after it
> **Sources**: doc 0002 §§ AI-20.1 … AI-20.5 (lines 1099–1161) · doc 0001 § 2.2, § 4.3 invariant 4, § 9 · ADR 0005 § D2 / Guard C
...
`openspec/specs/` was re-read at spec time. It carries **no provider capability**: `ai-model-provider` is new, so this is a full capability spec with **no MODIFIED and no REMOVED requirement**. `ai-stream-lifecycle`, `ai-minimum-capabilities` and `ai-contract-vocabulary` are read and cited by identifier and are **not modified by this change**.

**Reconciliation gate.** AI-14 (event envelope) and AI-19 (failure taxonomy) have **not landed**: no `ai-event-envelope` and no provider-failure spec exists yet. Every symbol-level assumption here — the carrier's element type, the terminal error event's shape, the cancellation category's name, the failure value's spelling — is named **by charter item, never by guessed identifier**, and MUST be re-verified against the landed specs before `sdd-apply`. Apply is ordered **last in wave 2** by design for exactly this reason. Guessing a type name is a defect, not a shortcut.
...
1. The externally-visible "was this request ever constructed" question of `R-AMP-006` — spec requires the rejection; **design owns the accessor's shape** (precedent: `MessageID.IsZero`, `Segment.IsZero`).
2. AI-20.3's single-purpose test producer (`R-AMP-013`) — design owns its shape, without pre-empting AI-21.
3. The guard's target-file resolution mechanics via `runtime.Caller`, and its bite-proof procedure (`R-AMP-014` … `R-AMP-016`) — design owns both; the bite proof follows AI-00.3's existing manual-mutation-and-revert precedent.
4. **Reconciliation gate**: AI-14 and AI-19 have not landed. Every symbol-level assumption — carrier element type, terminal event shape, cancellation category, failure value spelling — MUST be re-verified against the landed specs before `sdd-apply`, which is ordered last in wave 2 for this reason.

> **Deviation note**: this artifact exceeds the sdd-spec 650-word budget. The landed capability-spec precedent in this repository (`openspec/specs/ai-completion-metadata/spec.md`, `openspec/specs/ai-minimum-capabilities/spec.md`, and the sibling `cachicamas-ai-text-events` spec) carries per-requirement scenario blocks, non-functional requirements and acceptance criteria at this density; house convention wins.
