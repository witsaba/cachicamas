...
> **Canonical spec**: `openspec/specs/ai-request-extension-points/spec.md` — created by `sdd-archive` from this file
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Date**: 2026-08-01
> **Requirement IDs**: `R-REX-0NN` · **Scenario IDs**: `S-REX-0NN`
> **Binding input**: [the register](../../../../../specs/ai-contract-vocabulary/spec.md) — `V-REQ-19` … `V-REQ-29`, `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13` · [AI-10's spec](../../../2026-08-01-cachicamas-ai-model-request/specs/ai-model-request/spec.md) `R-AMR-001` … `R-AMR-017` and its [`design.md`](../../../2026-08-01-cachicamas-ai-model-request/design.md) §§ 8, 11, 12 · [`explore.md`](../../explore.md) · [`proposal.md`](../../proposal.md)
...
This spec **adds** a capability. It restates no `R-AMR-*` requirement; where it constrains a surface AI-10 owns, it cites AI-10's requirement and states the additional obligation.
...
- **A region** — AI-10's term for one addressable part of a request. This spec's region list is AI-10's six — model identity, system instruction, messages, tool set, tool choice, generation options — **plus** the cache-boundary markers `V-REQ-23` attaches, and **plus** the escape-hatch region this change adds.
...
These rules MUST run in the request's documented rule order and MUST report through AI-04's existing rule classes. This change MUST NOT append a rule class.
