...
> **Revision note (re-verify corrective, 2026-08-05, rev 1).** S-ATL-061 originally required "no file under `src/agenttest/` was modified by this change". That clause is unsatisfiable as written: the cachicamas-ai-conformance-tool-amendment merged into the AI-28 chain at `6732b65` (R-CNF-019 boundary case for fragmentable argument channels) requires that `toolCallZeroDeltaCase` and `mixedTextAndToolCallCase` in `src/agenttest/conformance_tool_call.go` migrate from `requireDrainedKinds` to `requireRelativeKindOrder`. Without those case edits, slice 2's `CapToolCalls` conformance turn-on cannot pass against the wire as designed. The amendment's disposition is **accepted, not fixed** — the spec acknowledges the rationale rather than treating it as a defect. The remaining inspections (bridge-only-in-`_test.go` files; no conformance case forked or skipped) stay unchanged. Same instrument as the AI-28 rev-4 S-ATS-089 disposition (Engram obs-579c70ab5030532b).

- **S-ATL-061** *[inspection]* — Given the shipped sources, when a reviewer reads them, then the bridge exists only in `_test.go` files of `openaicompat`, the only file under `src/agenttest/` modified by this change is `conformance_tool_call.go` whose `toolCallZeroDeltaCase` and `mixedTextAndToolCallCase` were migrated from `requireDrainedKinds` to `requireRelativeKindOrder` per the R-CNF-019 boundary case (commit `6732b65`, conformance tool amendment), and no conformance case was forked or skipped.
...
> **Revision note (re-verify corrective, 2026-08-05, rev 1).** S-ATL-070 originally required "no file in either package was modified". That clause is unsatisfiable as written: design D1–D5 explicitly direct modifications to `openaicompat/stream_state.go` (shared block allocator + per-call accumulation map), `openaicompat/chunk.go` (`wireToolCallElement` byte-preserving decode), `openaicompat/stream.go` (`isOutputEvent` widened, D8 close-discipline site inherited), `openaicompat/errors.go` (malformedToolCallAssembly typed cause), and `openaicompat/doc.go` (citations at the source); plus the conformance tool amendment at `6732b65` requires `agenttest/conformance_tool_call.go` case edits. The clause is narrowed to: intentional modifications match design D1–D5 plus the conformance amendment; no other files modified; both suites pass with `-race -count=1`. Same corrective form as the AI-28 rev-4 S-ATS-089 disposition.

- **S-ATL-070** *[test]* — Given `src/ai` and `src/agenttest` before and after this change, when their suites are run with `-race -count=1`, then the only modifications in `src/ai/openaicompat` are exactly the design D1–D5 scope (`chunk.go` +47, `stream_state.go` +424, `stream.go` +63, `errors.go` +13, `doc.go` +23), the only modification in `src/agenttest` is `conformance_tool_call.go` (cases 2/4 migrated to `requireRelativeKindOrder` per commit `6732b65`), no other files in either package were modified, and both suites pass unchanged under `-race -count=1`.
