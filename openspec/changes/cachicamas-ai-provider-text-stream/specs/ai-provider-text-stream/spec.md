...
> **Revision note (re-verify corrective, 2026-08-04, rev 4).** S-ATS-089's second conjunct originally required "the credential-scan guard covers the excerpt path". That conjunct was unsatisfiable as written: the credential-scan guard's designed scope is **external-package** (`package openaicompat_test`) files only, and every test file in this change is internal-package **by load-bearing necessity** — the error-mapping milestone plants above-threshold credential sentinels in internal test files precisely because the guard cannot see them there, so widening the guard to internal files would break the build on those deliberate plants. The conjunct is narrowed to the guard's designed scope, with the disclosed residual (a package-internal sweep is possible future hardening, owned by a future guard change, not this milestone) recorded where the excerpt is built. Same corrective form as S-ATS-046 rev 3.

- **S-ATS-089** *[inspection]* — Given the shipped source and its tests, when a reviewer reads them, then the excerpt bound is a named constant, and the excerpt-path source records the credential posture: the credential-scan guard's designed scope is external-package test files, this change's tests are internal-package by load-bearing necessity (the error-mapping milestone's planted-sentinel interaction), and the package-internal sweep is a recorded possible future hardening rather than a silent gap.
