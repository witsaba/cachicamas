...
- **S-ATS-046** *[test]* — Given a transcript ending with `data: [DONE]` and no trailing blank line before EOF, when the stream is drained, then the outcome is identical to the terminated form — the sentinel is not reported as truncation.
...
- **S-ATS-089** *[inspection]* — Given the shipped source and its tests, when a reviewer reads them, then the excerpt bound is a named constant, and the credential-scan guard covers the excerpt path.
