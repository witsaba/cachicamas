...
> **Corrective (slice-6 coordinator ruling, 2026-08-04).** Slice 5 shipped a disclosed deviation from this rule — skipping only on a *present-and-mismatched* `object`, tolerating absence — because none of the then-176 chunk fixtures carried the discriminator and the literal rule would have skipped them all. The fixtures were the defect, not the rule: C1 makes `object` required, so fixtures omitting it did not model legal wire. Slice 6 normalized every chunk fixture to carry `"object":"chat.completion.chunk"` (its own commit, suite green, before any rule change) and then implemented this paragraph as written — absent → skip, mismatched → skip, present-and-correct with a missing C1 required field → malformed (S-ATS-081). The slice-5 deviation is superseded. Full rationale: Engram `sdd/cachicamas-ai-provider-text-stream/slice6-rulings`.
