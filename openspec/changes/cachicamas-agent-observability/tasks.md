...
- [ ] 5.1 In `harness.go`, add the exported `TracerProvider trace.TracerProvider` field (D-A), documented as nil-defaults-to-noop, never read via a global getter.
- [ ] 5.2 In `Run` (`harness.go:408`), open the run span **after** the pre-identity refusals (`:444-461` emit nothing → no span, per D-D's placement rule), register the deferred finalizer at open, set `gen_ai.operation.name`, `cachicamas.run.id`, `cachicamas.run.parent_id` (iff present via `Event.Parent()`), and at close set `cachicamas.run.outcome` + `error.type` (iff failed) from `RunEnd.Outcome()/Failure()`, plus the error/ok status per `R-AGO-002`.
- [ ] 5.3 In `Compact` (`compaction.go:396`+), open a run span the same way, no span on the pre-flight refusals (`:415-422`).
- [ ] 5.4 Run `-count=1` on `observability_parity_test.go` (4.3) and `observability_lifecycle_test.go`'s run-only rows (4.4) — confirm `Started() > 0` now passes and the run-family rows go GREEN; tool/turn/compaction rows remain RED (expected — they instrument in Phases 6–8).
...
- [ ] 6.1 In `loop.go`, open the turn span at `emitStamped(sink, stamper, turnStart)` (`:314`), nothing on the pre-emission refusals (`:274-313`, incl. `validateContinuation` at `:274`); additionally open the run span on the nil-continuation path (`:300-307`) if not already covered by 5.2.
- [ ] 6.2 At turn close, set `cachicamas.run.id`, `cachicamas.turn.id`, `cachicamas.turn.outcome`, `error.type` (iff aborted) from `TurnEnd.Outcome()/Failure()`, register the finalizer at open so every one of loop.go's later exit sites (the mid-stream block, retry loop, etc.) converges on it.
- [ ] 6.3 Confirm Layer 1's own `chat` request span (AI-37, `openaicompat/trace.go`) nests under the turn span by construction — no code change needed, just verify the ctx handed to `provider.Stream` still carries the turn span (D-B).
- [ ] 6.4 Run `-count=1` on `observability_nesting_test.go` (4.1) for the run→turn parent-chain assertion — confirm it now passes that layer while tool/compaction rows remain RED.
