# Spec delta — The agent event envelope and ordering invariants (`agent-event-envelope`)
...
> **Format**: Given/When/Then + RFC 2119 per `openspec/config.yaml` `rules.specs`. Every scenario is independently verifiable.
...
> **New capability** — no prior `openspec/specs/agent-event-envelope/spec.md`, so this is a full spec, promoted at archive.
