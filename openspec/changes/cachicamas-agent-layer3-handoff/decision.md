...
- **A production coordination tool for nested runs.** The structural properties a delegation tool would stand on — nested identity, cost, and permission scope — are proven; no such tool ships in v1, and none is configured by default. Its post-v1 path is named in § 4.
- **A failover policy that actually switches providers.** The seam exists and is consulted exactly once at the retry bound's exhaustion; the only shipped implementation declines every time. A real policy re-opens token budgets and cached-prefix assumptions and needs its own design; its post-v1 path is named in § 4.
- **A default context-reduction policy.** The seam is consulted once per logical turn; left unset, no reduction is ever attempted and the run behaves exactly as it did before the seam existed. What triggers a reduction, and the instruction driving it, are both Layer 3's to supply; its post-v1 path is named in § 4.
...
|---|---|---|---|---|
| 1 | The package exists at the ADR 0005 § D2 location with both boundary guards biting. | `[x]` | AG-03 | doc 0003 checklist line 2161 (flipped this change — a pre-existing documentation gap, closed long before this milestone; see § 3 note below) |
| 2 | Every event family of v2 § 4.3 — all eight — is constructible, validated, and guarded. | `[x]` | AG-04, AG-05, AG-06 | doc 0003 line 2162 (flipped this change) |
| 3 | All four envelope invariants hold by test. | `[x]` | AG-04.3, AG-19.1, AG-20.2, AG-11.2 | doc 0003 line 2163 (already checked) |
| 4 | One turn runs end to end against the fake provider with events in contract order. | `[x]` | AG-07 | doc 0003 line 2164 (flipped this change) |
| 5 | The pre-request hook can rebuild the outgoing request; the identity default changes nothing. | `[x]` | AG-08 | doc 0003 line 2165 (flipped this change) |
| 6 | Tools execute with read-parallel/write-serial policy, bounded fan-out, and call-ordered rejoin. | `[x]` | AG-09 | doc 0003 line 2166 (flipped this change) |
| 7 | Permission is a suspension on the stream with all four outcomes; suspension blocks nothing else. | `[x]` | AG-10 | doc 0003 line 2167 (flipped this change) |
| 8 | Refusal, pause, and unknown finish reasons produce three distinct behaviors. | `[x]` | AG-11.1 | doc 0003 line 2168 (already checked) |
| 9 | The loop never retries and never decides policy; the guards and tests that prove it stay green. | `[x]` | AG-11.2, AG-10.1, AG-03.3 | doc 0003 line 2169 (flipped this change) |
