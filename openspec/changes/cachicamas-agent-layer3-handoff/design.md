...
**Defeat in both directions** (planted at apply, watched failing, reverted, recorded): (A) revert the send to a bare `sink <- ev`, keeping the deferred join in place → the forwarder can no longer abandon its send, so the join's wait on the forwarder's completion signal blocks forever against a forwarder permanently parked in that send: the RED **deadlocks** (test timeout naming both blocked goroutines) — `close(sink)` is structurally unreachable while the join still holds, so a crash cannot occur on this defeat; (B) keep the abort selects but delete the deferred join → `close(forwarderAbort)` and `close(sink)` run back-to-back, a forwarder at its select then has both cases ready and Go picks uniformly at random → per-iteration crash probability ≈ ½, miss probability over 100 iterations ≤ 2⁻¹⁰⁰. Defeat A therefore proves the termination half (its absence hangs); defeat B proves the happens-before half (its absence crashes).
...
**The stdlib blind spot, stated — and corrected (sdd-verify round 1, W-11).** Check 5 never lists dependencies, so `{{if not .Standard}}` filtering cannot blind it the way it can checks 1/2 — it is a zero-hop scan of the trees' own sources, with no closure to filter in the first place. That does NOT mean every standard-library-mediated reach is closed: check 5 denies only a **direct** import of one of its five named families; a tree importing some OTHER standard-library package that itself reaches one of those five internally (an intermediary such as `text/template`, which reaches `os`) is caught by NEITHER check 5 (direct-imports-only; the intermediary's own name is not on the denied list) NOR check 2 (its closure listing filters the standard library out before the allowlist ever sees it, so a standard-library intermediary — and whatever it reaches — is invisible to that check too). This is a genuine residual gap in the mechanism, not one closed elsewhere: `R-L3H-002` requires denial "by name", and the scan denies the five names it lists, no more. A non-standard-library intermediary is still bounded, since it would itself need a place in check 2's own allowlist (checks 3/4 additionally bound `src/agent` production itself, and Layer 1's own guards bound `src/ai` and `src/agenttest`); a standard-library one is not. The residual floor besides this gap is the test framework's own unavoidable `testing`→`os` reach, the same floor check 3's comment already records — restated, not hidden.
