...
| 10 | cache-boundary breakpoint cap (`cacheBoundaryCapRule`) | `ErrOutOfRange` at `cacheBoundaries` | draft + `messages` param | AI-11.2 |
| 11 | generation-option bounds (`boundsRule`) | `ErrOutOfRange`, `ErrEmpty` | draft | AI-10.1 |
| **12** | **each extension has a namespace, then a non-empty value, in ordinal order** | **`ErrEmpty` at `extensions[i].namespace` / `extensions[i].value`** | draft | **AI-12.3** |
...
**Correction 2 — resolved 2026-08-01 against the landed AI-11 surface: AI-12's row is 12, not 11.** This worktree is now rebased onto Wave 1 head `66e960d`, which carries AI-11 complete. AI-11's cap landed as `draft.cacheBoundaryCapRule(messages)` (`cache_boundary.go:163–170`), positioned at row **10** of the `FirstFailure` list, immediately before `draft.boundsRule()` at row **11** — `cacheBoundaryCapRule`'s own GoDoc names itself "rule 10 of NewRequest's documented order". AI-12.3's extension rule therefore appends as row **12**, definitively, not "11 or 12". This is still a pure append: no landed first-failure outcome changes, and `R-REX-005`'s scenarios assert class-and-position equivalence between the two doors, never an absolute ordinal, so no spec edit was needed either way. `tasks.md` § 0.3 records the same resolution against source.
...
## 13. Re-verification register — **resolved 2026-08-01, all eight rows**

This was an eight-row checklist of assumptions about surfaces that had not landed. **AI-10 landed first** (rows 1–6, resolved by the Phase 0 gate against Wave 1 head `1c4171e`), and **AI-11 landed second** (rows 7–8, resolved by the apply agent's task 0.3 against Wave 1 head `66e960d`, which this worktree is now rebased onto). Every row is a **resolved fact**, each with the signature that was read; none required a branch this file did not already carry.
...
### 13.2 Rows 7–8 — **resolved 2026-08-01**, against the landed AI-11 surface

**Verified present at `66e960d`** (the Wave 1 head this worktree is rebased onto, carrying AI-11 complete): both `Segment` (`system_instruction.go:15`) and `Tool` (`tool.go:24`) and `Message` (`message.go:116`) carry an unexported `cacheBoundary bool` field plus `MarkCacheBoundary()`/`IsCacheBoundary()` methods; `NewRequest`'s `FirstFailure` list (`request.go:180–234`) carries `draft.cacheBoundaryCapRule(messages)` at row 10. Both rows took **branch A**, exactly as the two forward references in the AI-10-era source hinted: `Segment.String()`'s GoDoc's *"AI-11 may extend this to name a cache marker, because a marker is structural rather than payload"* (`system_instruction.go:158`) and `ErrOutOfRange`'s GoDoc already naming `V-REQ-24` (`validation.go:82`). `tasks.md` § 0.3 records the same resolution independently, with the exact line numbers.

| # | Check | Status | **Branch A** — and its exact edit | **Branch B** — and its exact edit |
| --- | --- | --- | --- | --- |
| 7 | Markers ride on `Segment` / `Tool` / `Message` rather than on the request | **Resolved — branch A taken** | **Markers on the values.** They are reachable **transitively** through `WithSystemInstruction`, `WithTools` and `WithMessages`. **AI-12 adds no option and no production line.** Edit: § 8's totality table gets its markers row with `derive` = those three options and `changed` = a marker read on the derived value; `tasks.md` 1.7 records "transitive" and `S-REX-010` is satisfied as written | *(not taken — kept for the audit trail)* A request-level marker region. Edit: AI-12.1 appends **one `RequestOption`** wrapping whatever constructor AI-11 names, **one row** to § 8's table, and **one item** *(appended)* to AI-12.1's test list. `R-REX-002`'s marker clause already admits both spellings, so the delta spec needs no edit either way |
| 8 | The breakpoint cap `V-REQ-24` is a `Rule` in the request's rule list | **Resolved — branch A taken** | **The cap is in the list**, at row 10 of `FirstFailure` (§ 3, corrected). The rebuild **re-runs it for free** — zero AI-12 production code. Edit: AI-12.2 appends one derive-path assertion *(appended)* — a derivation that pushes the request past the cap fails with `ErrOutOfRange` at the position construction reports. AI-11's row landed before AI-12's, so AI-12's own extension rule is row **12** | *(not taken — kept for the audit trail)* The cap is enforced elsewhere (inside a marker constructor, or at a boundary the request never re-runs). Then the rebuild does **not** re-run it. Edit: AI-12 **records the gap** in `tasks.md` for the Wave 1 verify phase and asserts nothing. It must **not** add the rule itself — `V-REQ-24` belongs to AI-11, and adding it here would put a second owner on one register term |
...
- [ ] **None blocking.** Every row in § 13 is resolved — AI-10's six by the Phase 0 gate, AI-11's two by the apply agent's task 0.3 — and none required a branch this file did not already carry.
...
- [ ] **Considered and deferred to Wave 2, not this change: a region-level exhaustiveness guard over `Request`/`requestDraft`, in AI-06.4's idiom.** Three milestones running (AI-10.5's `agenttest` round trip, AI-11's `Message.Equal`/`toolsEqual`, and now AI-12.3's `Request.Equal` plus the same two `agenttest` helpers, § 7.2) each had a hand-rolled region walk that did not see a newly added region "for free" — a real, repeating defect class, caught each time by a re-verification gate rather than by a test. A `go/parser` AST guard over `requestDraft`'s field declarations, comparable to `content_part_registry_test.go`'s guard over `PartKind`'s constant space, would catch it mechanically. Judged **out of scope for AI-12**: § 8's enumeration table already gives today's regions their totality proof and is on record as "the cheap half of AI-06.4's guard idea without adding a `[guard]` node the charter does not carry" (§ 8, `explore.md` § 9); a struct's fields are not a closed constant-space vocabulary the way `PartKind` is, so a faithful guard needs a value-field/presence-flag pairing heuristic that is itself a design decision, not a mechanical port of the landed pattern, and this milestone is already `High` risk under `size-exception` delivery. `tasks.md`'s closing checklist carries the same note as a Wave 2 proposal.
