# Spec — Repo-level validation harness

> **Introduced by**: `openspec/changes/archive/2026-08-11-repo-validation-harness/` · `size:exception` approved, single-PR
> **Status**: **live**
> **Capability**: `repo-validation-harness` (**new** — full spec, no MODIFIED/REMOVED/RENAMED)
> **Requirement IDs**: `R-RVH-NNN` · **Scenario IDs**: `R-RVH-NNN / S-N` (per-requirement local)
...
> **Conformance**: every scenario is fixture-based (no live vulnerability DB, no compose stack required)

---

## Purpose

No command today proves "tests + linters + type-checks + vuln scans green" across `frontend/` and the three `backend/*` Go modules. This capability states the contract for `scripts/validate.py`: a single entry point that emits a stable, dedupable, machine-parseable report (`validation-report.json` + human-readable `validation-report.md`) consumed by an AI agent fixing every finding. The report must always carry the full picture — never fail-fast — and its shape, run semantics, opt-in surface, and CI-ready guarantees are fixed here.

## Definitions

- **Check** — one row of the check matrix: a scope + a check name + a command + a parser. One subprocess invocation per check.
- **Finding** — one observed issue: `id` (sha256[:16] of normalized content), `severity` (verbatim from tool), `category`, `tool`, `scope`, `rule`, `file`, `line`, `column`, `message`, `evidence`, `reproduce`, `fix_hint`. Exactly 13 fields, in this order.
- **`tool-missing`** — a synthetic category: the harness could not invoke the underlying tool (no Makefile target, no binary on PATH, parser failed). Distinct from a check that ran and returned findings.
- **`go.work-mismatch`** — a synthetic category: `go.work`'s Go version differs from any module's `go.mod` Go directive.
- **Allowlist** — `(scope, target)` pairs documented as deliberate absences. The default config allowlists `(backend/agent, install)` per `backend/agent/Makefile` lines 55–58.
- **Opt-in checks** — `make test/integration` (boots Postgres) and `pnpm test:e2e` (needs compose stack). Default runs MUST NOT require docker.
...
`go test ./...` and `go vet ./...` already compile every package including `cmd/`; `frontend` `build.types` covers the frontend. `build` adds binary-writing side effects without new signal.
...
The harness is CI-ready even though CI wiring is out of scope for this change.
