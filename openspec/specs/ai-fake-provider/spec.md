...
> **Milestone**: AI-21 — Build a scripted fake provider (Wave 3 "Prove", the wave's **first** milestone) · **Nodes**: AI-21.1 `[leaf]` walking skeleton · AI-21.2 `[leaf]` tool call · AI-21.3 `[leaf]` terminal error · AI-21.4 `[leaf]` delays and the blocked stream · AI-21.5 `[leaf]` cancellation fidelity · AI-21.6 `[leaf]` request capture · AI-21.7 `[leaf]` scripted reasoning · AI-21.8 `[leaf]` sequential-call scripting
> **Introduced by**: `openspec/changes/archive/2026-08-03-cachicamas-ai-fake-provider/`, merged to `main` in PR #107 (commit `0d5fd91`)
...
> **Closes**: doc 0002's **AI-21** milestone in full — promotes `R-AMP-013`'s local single-purpose producer into an importable, contract-faithful testing library that AI-22 and AI-23 (and Layer 2, doc 0003 wave C) build on
> **Requirement IDs**: `R-AFP-0NN` · **Scenario IDs**: `S-AFP-0NN`
...
> [`ai-model-provider`](../ai-model-provider/spec.md) (AI-20) — `R-AMP-003` external implementability, `R-AMP-005` … `R-AMP-008` the pre-stream contract, `R-AMP-009` … `R-AMP-012` the mid-stream contract, `R-AMP-013` the local single-purpose producer that this milestone succeeds;
> [`ai-stream-lifecycle`](../ai-stream-lifecycle/spec.md) — § 3 carrier, § 4 ownership, § 5 cancellation, § 6 buffering, § 7 failure delivery, § 9 the eight statements;
> [`ai-event-envelope`](../ai-event-envelope/spec.md) (AI-14) — the envelope, its kind vocabulary, `CheckEmit`, and per-stream sequencing;
> [`ai-text-events`](../ai-text-events/spec.md) (AI-16), [`ai-reasoning-events`](../ai-reasoning-events/spec.md) (AI-17), [`ai-tool-call-events`](../ai-tool-call-events/spec.md) (AI-18), [`ai-provider-errors`](../ai-provider-errors/spec.md) (AI-19) — the four script vocabularies;
> [`ai-model-request`](../ai-model-request/spec.md) (AI-10.5 readability, AI-10.6 immutability) — what request capture asserts on
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The scripted fake provider's twenty requirements and the contract-faithful-not-convenient rule therefore live here, in their own text, not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-03-cachicamas-ai-fake-provider/`](../../changes/archive/2026-08-03-cachicamas-ai-fake-provider/) is the historical record of how AI-21 was explored, proposed, designed, applied and verified — including the mid-cycle correction to `R-AFP-014` (see below).

**`R-AFP-004` and `R-AFP-014` carry the most weight against erosion.** `R-AFP-004` is the non-negotiable "physics, not approximation" rule the rest of the spec exists to serve; a later contributor simplifying the fake's concurrency model for convenience breaks the reason this capability exists. `R-AFP-014` was corrected once already, before implementation (see the design resolution note below) — a future amendment should re-read that history before touching capture semantics again.
...
## What design resolved, and what remains for later milestones

> **Resolved at archive 2026-08-03.** This section originally left two items to design, deliberately undecided at spec time. Both are settled and shipped; recorded here so a future reader finds the resolution rather than a stale open question.

`R-AFP-020`'s loud failure is a **typed pre-stream error**, `ErrScriptsExhausted`, not a captured `testing.TB` handle — `design.md`'s rationale: Layer 2's agent loop calls `Stream()` from its own goroutines, and `TB.Fatal` from a non-test goroutine is undefined behavior. `R-AFP-010`'s synchronization primitive is `Gate`, an exported two-channel type (`Reached()`/`Release()`) that removes wall-clock sleeps from both the held-open and the deterministic-saturation recipes.

Vendor wire-format mocking remains AI-27's fixtures and AI-38's transcripts. The stream recording and assertion helpers are AI-22 (`ai-stream-testkit`, promoted alongside this file) and the conformance suite is AI-23 (`ai-provider-conformance-suite`, also promoted alongside this file) — siblings in this wave, separate capabilities, both consumers of this one rather than parts of it.
