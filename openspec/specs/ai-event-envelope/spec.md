...
> **Milestone**: AI-14 — Define the event envelope with per-stream sequencing (Wave 2 "Stream") · **Nodes**: AI-14.1 `[leaf]` envelope skeleton, AI-14.2 `[leaf]` per-stream sequence, AI-14.3 `[guard]` no process-global sequence state, AI-14.4 `[leaf]` ordering invariants
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-event-envelope/`, merged to `main` in PR #104 (commit `37898c7`)
...
> **Closes**: **C3** — the process-global sequence counter, made structurally unwritable: AI-14.2 defines the sequence as per-stream state and AI-14.3 guards the whole non-test package against a package-level counter reappearing. It also lands the mechanism doc 0002 records for **C4** (AI-14.1's exhaustiveness assertion — a registered kind whose payload cannot be constructed fails on the day it is reintroduced) and the Layer 1 half of completion-checklist item 7, order and per-stream sequence (with AI-14.4; AI-22.3 packages it)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-AEE-0NN` · **Scenario IDs**: `S-AEE-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) § 4 — `V-STR-10` event, `V-STR-11` event kind, `V-STR-12` payload, `V-STR-13` sequence, `V-STR-14` block, `V-STR-15` block index, `V-STR-16` delta, `V-STR-17` ordering invariant, `V-STR-18` terminal event. All nine are **owned** by AI-14; this spec realizes them and MUST NOT re-define, re-paraphrase or narrow them
> **Binding predecessor**: [stream lifecycle](../ai-stream-lifecycle/spec.md) — § 4 single-producer ownership, § 10 the inherited/owned split
> **Binding predecessor**: [the validation error taxonomy](../ai-validation-errors/spec.md) — every rejection here reports through AI-04's landed failure value and its **closed, append-only** sentinel set
> **Blocks**: AI-15 … AI-20. **Depends on**: AI-02, AI-04
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-14--define-the-event-envelope-with-per-stream-sequencing) §§ AI-14.1 … AI-14.4 (lines 855–909) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) § 3.1 defects **C3** and **C4**, § 4.3, § 9 · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The envelope, the per-stream sequence, the descriptor table and the ordering invariants therefore live here, in their own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-event-envelope/`](../../changes/archive/2026-08-01-cachicamas-ai-event-envelope/) is the historical record of how AI-14 was explored, proposed, designed, applied and verified.

`R-AEE-015`'s extensibility claim is the load-bearing property of this file, and it held across the whole of Wave 2: five milestones (AI-15 … AI-19) registered twelve production kinds and not one commit after AI-14's own close-out touched the checker, the descriptor types or the sequence. A later milestone that needs a new kind appends a constant, a table entry, a payload, a constructor, an accessor and a descriptor — and amends nothing here. A rule added later arrives by amendment **to this file**, together with the descriptor component it constrains.
...
> **Amended 2026-08-01 (archive).** `R-AEE-006` describes the vocabulary **as AI-14 landed it**, and it is preserved verbatim because it is the requirement AI-14 was verified against. It is not a standing prohibition: AI-15 … AI-19 registered twelve production kinds in the same wave, exactly by the append route `S-AEE-018` requires, and without editing any requirement of this capability. A reader checking the live vocabulary should enumerate `EventKinds()`, not this requirement.
