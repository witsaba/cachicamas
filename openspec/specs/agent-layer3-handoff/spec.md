...
> **Introduced by**: `openspec/changes/archive/2026-08-21-cachicamas-agent-layer3-handoff/` (**AG-23**, Wave 6 — Harden and hand off; the **last** milestone of Layer 2, the layer's exit)
> **Status**: **live** — this file carries the contract for Layer 3's entry gate. The v1 surface freezes here.
> **Capability**: `agent-layer3-handoff` (**new** — full spec; no MODIFIED/REMOVED/RENAMED inside this file)
> **Requirement IDs**: `R-L3H-0NN` · **Scenario IDs**: `S-L3H-0NN` (both prefixes verified free across `openspec/specs/` and `openspec/changes/` at mint time)
...
> **Binding decisions**: proposal `D-1`…`D-8` as **corrected by** design `AD-1`…`AD-8`, both in the archived change folder. Where the two disagree, the design binds: it corrected the proposal twice (the determinism claim, `AD-3`; the stale-row count, `AD-6`).
> **Amended in the same change** (each owns an amendment this capability depends on, and none of them is restated here): [`agent-package-scaffold`](../agent-package-scaffold/spec.md) (`R-AGP-003` — the guard extension) · [`agent-run-driver`](../agent-run-driver/spec.md) (`R-RUN-014` — the forwarder's bounded exit) · [`agent-hook-taxonomy`](../agent-hook-taxonomy/spec.md) (`R-HKS-010` — the W-6 freeze disposition) · [`agent-v1-scope`](../agent-v1-scope/spec.md) (`R-AGS-016` — the declined removal).
