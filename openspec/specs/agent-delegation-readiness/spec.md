...
> **IDs**: `R-DEL-0NN` / `S-DEL-0NN` / `NFR-DEL-0NN`, **append-only**. Allocated `R-DEL-001`…`R-DEL-011`, `S-DEL-001`…`S-DEL-026` and `NFR-DEL-001`…`NFR-DEL-005`, of which `S-DEL-020`…`S-DEL-023` are the four **bites**. The header states the allocated **range** and never a total, because a total is defended by no test and goes silently false on the next append (`S-LSK-020`). AG-20 appends `R-DEL-011` / `S-DEL-026`. Prefix `DEL` verified free under `openspec/specs/` before minting, re-verified in this phase: zero occurrences.
...
> **Amended 2026-08-20 (AG-20)**: `R-DEL-011` ADDED, alongside `R-DEL-009` and `R-DEL-010`, both of which are byte-unchanged. AG-20 proves an observing hook cannot reach the delegation seam and back-annotates that the scope fence and the closed-sequence table both pass unedited. See delta spec at `openspec/changes/cachicamas-agent-hook-taxonomy/specs/agent-delegation-readiness/spec.md`.
...
### R-DEL-011 — AG-20's observing hooks CANNOT reach the publishing seam, and the fence passes unedited (AG-20 addition)
