...
- **Inside the attempt loop (rejected).** A seam consulted per attempt is a seam that — once AG-18 gives its verdict compaction teeth — could rewrite the transcript **between two attempts of one logical turn**. That does not merely risk breaking the byte-identity clause above; it defeats the exact argument the implementing comment relies on (`harness.go:550-561`): the attempt loop reuses "the transcript slice built once above ... **BY REFERENCE** across attempts — this, not an assumption, is what makes 'identical transcript' provable". A per-attempt seam makes the *provable* into the *merely intended*.
