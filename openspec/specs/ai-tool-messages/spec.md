...
> **Milestone**: AI-09 — Define tool calls and tool results · **Nodes**: AI-09.1 `[leaf]`, AI-09.2 `[leaf]`, AI-09.3 `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-tool-messages/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Closes**: the Layer 1 half of **G5** — the tool-call ordinal surviving normalization (AI-09.2). The stream and wire halves are AI-18.3 and AI-30.5 (Wave 2+)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-ATM-0NN` · **Scenario IDs**: `S-ATM-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-REQ-16`, `V-REQ-17`, `V-REQ-18`, `V-STR-21`, `V-REQ-04` … `V-REQ-07`, `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13`, `V-OUT-02`, `V-OUT-04`; every Layer 1 noun below is one of its rows, cited by identifier
> **Binding predecessor**: [content parts, readable and sealed](../ai-content-parts/spec.md) and its [`decision.md`](../../changes/archive/2026-08-01-cachicamas-ai-content-parts/decision.md) — inherited whole; both kinds below are added under AI-06's procedure rather than re-deciding it
...
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-09--define-tool-calls-and-tool-results) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The tool-call and
tool-result kinds, the byte-exactness of argument bytes, and the derived call ordinal therefore live
here, in their own text, and not only as a pointer into the archive. The archived change folder at
[`openspec/changes/archive/2026-08-01-cachicamas-ai-tool-messages/`](../../changes/archive/2026-08-01-cachicamas-ai-tool-messages/)
is the historical record of how AI-09 was explored, proposed, designed, applied and verified.

`R-ATM-006`'s derivation rule — the ordinal is a call's **position**, never a stored field — is what
AI-18.3 and AI-30.5 must preserve when the same calls arrive as stream deltas. A later milestone that
needs to store an ordinal amends **this file** rather than adding a parallel field.
