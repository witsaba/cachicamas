...
> **Amended 2026-08-10 — the payload's delivery path now participates in R-AIP-002's rejection rule.** A terminal error event construction attempt whose payload reports the pre-stream delivery path fails with the misplaced-value sentinel at a position naming the payload: a pre-stream failure was returned directly and never crossed a carrier, so wrapping it as a stream terminal would produce an event whose own payload contradicts the carrier it arrived on. This closes the one axis where R-AIP-010's "contradictory combination is unconstructible" posture was documentation rather than construction — the partial-output axis already had it; the delivery axis now does too. Found by the 2026-08-10 whole-layer re-verification; every shipped producer already passed only mid-stream payloads, so no landed behavior changed.
...
> **Carried forward** *(historical — every claim in this entry describes the pre-AI-41 state; see the discharge entry below, and note `*Failure` HAS carried a `GoString()` since AI-41)*. `R-AIP-009`'s guarantee covers `%v`, `%s` and `%+v`, which were reproduced clean. `*Failure` is the only Wave 2 payload without a `GoString()`, so `%#v` falls back to reflection and reproduces two provider-supplied fields — the sanitized, bounded `rawLabel` and `requestID`. The wrapped cause's text does **not** leak (it renders as a pointer address), so `R-AIP-009`'s literal claim holds; but the inconsistency with the wave's own four-verb rendering convention is real. Recorded as `W2` in the Wave 2 verify report and owned by Wave 3. The fix is one `GoString()` method plus one four-verb canary test.
