...
>
> **Amended 2026-08-21 (AG-23 archive)** by [`cachicamas-agent-layer3-handoff`](../../changes/archive/2026-08-21-cachicamas-agent-layer3-handoff/) (**AG-23** — Layer 3 readiness handoff, Wave 6, the **last** milestone of Layer 2): `R-HKS-010` MODIFIED and two scenarios appended (`S-HKS-027`, `S-HKS-028`; the allocated `S-HKS-` range above grows accordingly, as a range and never a total). The W-6 disposition AG-22 handed forward is closed on the merits — `scheduler.go` is **RESTORED** to the scope fence and `import_boundary_test.go`'s release is made **PERMANENT** with its extension-point reasoning recorded twice — and consequence 2's forward assignment of `TurnOptions.PreRequestHook`'s removal to AG-23 is **RESOLVED by DECLINING the removal**: the field is kept, unamended in behaviour, carries no deprecation marker, and freezes into v1 as frozen-and-superseded with a **post-v1** removal path. `R-HKS-002`'s closing sentence and the "Explicit non-requirements" table's removal row are corrected to match, so no requirement in this file still names AG-23 as the holder of that removal (`agent-layer3-handoff`'s `R-L3H-009`). No other requirement, scenario or non-functional requirement is modified, and no existing identifier is renumbered.
...
**AG-08's shipped field is kept, unamended in behavior, and superseded in prose only.** It MUST NOT be marked with Go's `Deprecated:` convention, which would make every existing internal reference a lint finding for no behavioral gain. Its removal was assigned to AG-23 and was **not** decided here; **AG-23 has since DECLINED it** and frozen the field into v1, so the removal path is **post-v1** and is held by no Layer 2 milestone (`R-HKS-010` consequence 2).
...
- **S-HKS-053** — **(bite)** RED-first, **mandated** — **its stated observable corrected by `sdd-verify` (MAJOR-6) to the one actually reachable and actually observed, after the property itself was independently reproduced and confirmed load-bearing.** Given a scratch tree in which the post-hook re-resolution is skipped and the hook's returned cut is used directly, when `S-HKS-008` runs, then it **FAILS**: `compaction_finished` count is **zero** (want exactly one), the compaction bracket takes its typed failure arm instead, because `hist.markSpan` rejects the non-mark, non-pairing-closed cut the skipped re-resolution would otherwise have retracted — proving the re-resolution is load-bearing rather than incidentally satisfied by a fixture whose adjusted cut already landed cleanly. **This is deliberately NOT** "the committed prefix splits a call/result pair" (the original text's claim): pairing safety here is layered — `hist.markSpan` is not the only guard; `buildLoopRequest` independently refuses a pairing-broken prefix even when the `markSpan` guard is ALSO neutralised in the same scratch tree — so a split prefix is not reachable by this sabotage, or by this sabotage plus removing the second guard, at all. The property this bite proves is that removing the re-resolution step turns a successful compaction into a failed one; the mechanism by which it fails is typed refusal, never a committed split. RED-recorded BEFORE `S-HKS-008` is GREEN, with `-count=1`, then reverted.
...
- **S-HKS-012** — **The cost is the sum over the turn's attempts, and the multi-attempt fixture proves the achievable half of that claim — corrected by `sdd-verify` (MAJOR-5) after its original two clauses were found unachievable.** Given a logical turn scripted to make **three** attempts — two retried after a retryable failure, the third succeeding — when the post-turn report arrives, then its cost equals the sum of the per-turn cost events observed inside that logical turn's brackets on the recorded stream, and the report's attempt count equals three. **What this fixture cannot claim, and the original scenario wrongly did:** "each attempt carrying a distinct, non-zero usage figure" and "that sum is strictly greater than the last attempt's own figure" — both false of any reachable fixture, not merely unbuilt. `retryDecision`'s G1–G5 gates (`retry_policy.go`) only retry (G4) a `Turn` error that IS a retryable, no-partial-output `*ai.Failure`, and every such failure is necessarily a pre-stream or mid-stream-before-any-`Completion` path — neither of which ever reaches `finalize()` (`loop.go`), the ONLY site that emits `cost_turn`. A genuinely retried attempt therefore **structurally contributes zero** `cost_turn` events; only the logical turn's single completing attempt, if any, ever does. The sum this scenario proves accordingly **equals** the one completing attempt's own figure, never a multiple and never a sum strictly exceeding it — the property this fixture DOES prove is that retried attempts are counted (attempt count 3) without their zero contribution being misread as a missing report, not that multiple attempts each contribute their own nonzero figure to the sum.
...
| Removing `TurnOptions.PreRequestHook` | **Nobody in Layer 2 — DECLINED at AG-23**, the milestone that freezes the v1 surface: removing an exported member is a breaking change, and the surface-freezing milestone is the worst place to take one. Kept and superseded here, never broken; frozen into v1 as frozen-and-superseded with a **post-v1** removal path (`R-HKS-010` consequence 2) |
...
| `S-HKS-053` (d) | skip the post-hook cut re-resolution | `S-HKS-008` | compaction fails typed: `hist.markSpan` rejects the non-mark cut, zero `compaction_finished` events, exactly one `compaction_failed` (corrected by `sdd-verify` MAJOR-6 from an unreachable "committed prefix splits a call/result pair" claim — pairing safety is three-layered, and this sabotage alone cannot reach a split; see the scenario text below) |
