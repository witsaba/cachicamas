...
> **Milestone**: AI-11 — Add cache boundary markers · **Nodes**: AI-11.1, AI-11.2, AI-11.3, all `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-cache-breakpoints/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Closes**: the Layer 1 half of **G4** — cache breakpoints (with AI-10.2's segments from birth) — and the Layer 1 half of leakage register **row 9**. Wire rendering is AI-26.2 (Wave 2)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-ACB-0NN` · **Scenario IDs**: `S-ACB-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-REQ-14`, `V-REQ-19`, `V-REQ-20`, `V-REQ-22`, `V-REQ-23`, `V-REQ-24`, `V-REQ-25`, `V-MET-09`, `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13`, `V-FAIL-17`; every Layer 1 noun below is one of its rows, cited by identifier
> **Binding predecessor**: [the normalized request core](../ai-model-request/spec.md) — the three carriers, the rule order and the documented request equality are AI-10's; this milestone amends them rather than re-deciding them
> **Binding predecessor**: [the validation error taxonomy](../ai-validation-errors/spec.md) — the cap reports through AI-04's failure value and its landed rule classes
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-11--add-cache-boundary-markers) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)

---

## Purpose

Constrain the runtime behavior of `V-REQ-23` **cache-boundary marker**, `V-REQ-24` **breakpoint cap** and `V-REQ-25` **invalidation cascade** on the normalized request. This spec covers all three leaves of AI-11, all of which landed in Wave 1.

Requirement count: **10**.

## Status — this file is the canonical home of the contract

`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The marker, the cap, the cascade order and the advisory posture therefore live here, in their own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-cache-breakpoints/`](../../changes/archive/2026-08-01-cachicamas-ai-cache-breakpoints/) is the historical record of how AI-11 was explored, proposed, designed, applied and verified.

`R-ACB-008`'s advisory posture is what AI-26.2 inherits: an adapter that ignores every marker is conformant. A Wave 2 milestone that renders markers to a wire format amends **this file** if it needs to change what a marker means; it may not make a marker load-bearing for correctness.
