...
> **Amended 2026-08-21 (AG-23, `cachicamas-agent-layer3-handoff`)** by the archive executor, from [`openspec/changes/archive/2026-08-21-cachicamas-agent-layer3-handoff/`](../../changes/archive/2026-08-21-cachicamas-agent-layer3-handoff/) (**AG-23** — Layer 3 readiness handoff, Wave 6, the **last** milestone of Layer 2): `R-AGS-016` MODIFIED and one scenario appended (`S-AGS-069`; the allocated `S-AGS-` range grows to `S-AGS-069`, stated as a range and never a total, with `S-AGS-064` still permanently allocated-and-unreused per the defect record under `R-AGS-015`). Two edits, both back-annotation rather than new scope: the SEAM 1 paragraph's forward assignment of the singular pre-request field's removal to AG-23 is **RESOLVED by DECLINING the removal** — the field is kept, unamended in behaviour, carries no deprecation marker, and freezes into v1 as frozen-and-superseded with a **post-v1** removal path, so no requirement may continue to name AG-23 as that removal's holder (`agent-layer3-handoff`'s `R-L3H-009`); and an AG-23 back-annotation records Layer 2's exit — the exported surface gains and loses nothing, the unrecovered-panic posture is **KEPT** while ceasing to be able to crash a goroutine the caller does not own (`agent-run-driver`'s `R-RUN-014`), and `R-AGS-015`'s Layer 3 vocabulary discipline is made mechanical for the first time (`R-L3H-010`). No other requirement is modified, `S-AGS-048`'s G11 row still names CO-24.1 / CO-24.2, and no existing identifier is renumbered.
