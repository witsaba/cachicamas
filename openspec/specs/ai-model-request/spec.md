...
> **Milestone**: AI-10 — Define the normalized request core · **Nodes**: AI-10.1 … AI-10.6, all `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-model-request/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Closes**: the Layer 1 half of leakage register **row 8** (AI-10.2, system-instruction segments from birth) and the last node of the Layer 1 half of **C2** — content unreadable from another package (AI-10.5)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-AMR-0NN` · **Scenario IDs**: `S-AMR-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-REQ-01` … `V-REQ-22`, `V-REQ-26`, `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13`, `V-FAIL-17`; every Layer 1 noun below is one of its rows, cited by identifier
> **Binding predecessor**: [the validation error taxonomy](../ai-validation-errors/spec.md) — every rule violation here reports through AI-04's failure value and its landed rule classes; `ErrMisplaced` was appended there under AI-04's own append rule
> **Binding predecessor**: [roles and message identity](../ai-message-roles/spec.md) — messages, roles and content order are AI-05's; there is **no system role**
> **Binding predecessor**: [content parts, readable and sealed](../ai-content-parts/spec.md) and its [`decision.md`](../../changes/archive/2026-08-01-cachicamas-ai-content-parts/decision.md) § 7.2 — the per-kind content rules are called at request depth, never reimplemented
> **Binding predecessor**: [tool declarations](../ai-tool-declarations/spec.md) — AI-08.3's cross-validation runs unchanged at the request boundary · [tool calls and tool results](../ai-tool-messages/spec.md) — the correlation checked by `R-AMR-012`
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-10--define-the-normalized-request-core) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)

---

## Purpose

Constrain the runtime behavior of `V-REQ-20` — the normalized request: the one value a provider receives. This spec covers all six leaves of AI-10, all of which landed in Wave 1.

Requirement count: **17**. The archived `proposal.md` forecast 16; the redaction posture was split out as `R-AMR-017` rather than folded into the readability requirement, because it is the opposite property and a reader must be able to cite one without the other.

## Status — this file is the canonical home of the contract

`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The request's regions, its rule order, the role-versus-kind table and its equality definition therefore live here, in their own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-model-request/`](../../changes/archive/2026-08-01-cachicamas-ai-model-request/) is the historical record of how AI-10 was explored, proposed, designed, applied and verified.

AI-11 adds cache boundary markers to the regions this file defines, and AI-12 adds per-request options, the provider escape hatch and rebuild. Both amend **this file** where they change a region, per `R-AMR-004`'s neutrality test. AI-26 is the first consumer of `R-AMR-015`'s round trip.

> **Known limitation, owed to Wave 2.** The region-exhaustiveness guard is one-directional: it asserts the region count and therefore catches a **deleted** table row, but it does not catch a **field added** to the request draft — the failure that recurred at AI-10.6, AI-11.1 and AI-12.3. Doc 0002's traceability spine carries this as a Wave 2 obligation; the first Wave 2 milestone that adds a region owns the fix.

## Requirement ownership by leaf

| Leaf | Requirements |
| --- | --- |
| AI-10.1 | `R-AMR-001`, `R-AMR-002`, `R-AMR-003`, `R-AMR-004`, `R-AMR-017` |
| AI-10.2 | `R-AMR-005`, `R-AMR-006`, `R-AMR-007` |
| AI-10.3 | `R-AMR-008`, `R-AMR-009`, `R-AMR-010`, `R-AMR-011`, `R-AMR-012` |
| AI-10.4 | `R-AMR-013`, `R-AMR-014` |
| AI-10.5 | `R-AMR-015` |
| AI-10.6 | `R-AMR-016` |
...
## R-AMR-008 — Message order and intra-message content order are preserved *(AI-10.3)*
...
## R-AMR-009 — The content rules are AI-06's, called at request depth *(AI-10.3)*
...
## R-AMR-010 — Tool set and tool choice attach, and are cross-validated at the request boundary *(AI-10.3)*
...
## R-AMR-011 — Role versus content kind is enforced from a documented table *(AI-10.3)*
...
`ErrMisplaced` is a rule class **appended** to AI-04's set under AI-04's own append rule, because none of the landed classes described a value that is well-formed and wrongly placed. Appending it obliged this change to update both registry mirrors in the same commit.
...
## R-AMR-012 — Tool-call and tool-result correlation is checked across the request *(AI-10.3)*
...
## R-AMR-013 — Validation happens once, before any I/O *(AI-10.4)*
...
## R-AMR-014 — The first failure is deterministic and follows a documented order *(AI-10.4)*
...
## R-AMR-015 — A whole request round-trips, exhaustively over the registered kinds *(AI-10.5)*
...
## R-AMR-016 — A request is immutable, and equality is documented *(AI-10.6)*
