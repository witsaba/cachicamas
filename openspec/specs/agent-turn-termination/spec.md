...
>
> **Amended 2026-08-20 (AG-21 archive)**: Explicit non-requirements list MODIFIED (back-annotation only, no new identifier minted). The final bullet's clause "Multi-turn state beyond a run remains AG-21's" closes, discharged by the enumerated inventory on `agent-run-driver`'s `R-RUN-001`. See delta spec at `openspec/changes/cachicamas-agent-concurrency-hardening/specs/agent-turn-termination/spec.md`.
