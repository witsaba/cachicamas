...
> **NEW capability**, minted by this change (AG-16). This file is the normative text, promoted to `openspec/specs/agent-cost-events/spec.md` at archive. IDs `R-CST-0NN` / `S-CST-0NN`; the allocated range is `R-CST-001`…`R-CST-007` and `S-CST-001`…`S-CST-014` plus the bites `S-CST-020`, `S-CST-021`, `S-CST-022`, and AG-19's `S-CST-023`, `S-CST-024`. **The prefix `CST` was verified collision-free** (zero `[RSN]-(CST|COS|CEV|CSE)-[0-9]` matches repo-wide). **No total is stated**: a count goes silently false the moment a later milestone appends (`S-LSK-020`).
