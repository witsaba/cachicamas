...
>
> **Corrected 2026-08-10 — the hazard is one identifier wider than this note recorded: `R-CNF-019` is double-booked, not merely the last promoted number.** Both archived amendments above ALSO define an `R-CNF-019` of their own ("every exact count is derived from its own amended script"), and the shipped tree cites that archived meaning — `src/agenttest/conformance_lifecycle.go` names `R-CNF-019`…`R-CNF-021` in its header and `R-CNF-020` in its error strings — while this file's `R-CNF-019` (added 2026-08-07 by AI-35) is the unrelated CapRetry auto-retry requirement. A reader following `R-CNF-019` from `conformance_lifecycle.go` into this file lands on the wrong requirement. Disposition: the canonical `R-CNF-019` stays CapRetry (three shipped parity guards pin its wording); the lifecycle citations are annotated in-source as archived-amendment numbering; and the un-promoted set to restore is `R-CNF-019(archived meaning)` … `R-CNF-026`, one wider than the seven this note originally counted. The same restore owes a decision on the collision — renumber the archived meaning on promotion, never the canonical one. Found by the 2026-08-10 whole-layer re-verification.
>
> **Scenario-numbering note (recorded 2026-08-10).** The scenario space runs `S-CNF-001` … `S-CNF-087` with **`S-CNF-068` never defined** — 86 scenarios total, matching every stated count. The skip was consumed-and-dropped during AI-38's promotion (its archive report records it); it is recorded here so a future count reconciliation does not read the hole as a lost scenario. Identifiers stay append-only: `S-CNF-068` remains permanently unused.
