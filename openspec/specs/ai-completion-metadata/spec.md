...
> **Milestone**: AI-13 — Define finish reasons and usage · **Nodes**: AI-13.1 `[leaf]`, AI-13.2 `[leaf]`, AI-13.3 `[leaf]`, AI-13.4 `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-completion-metadata/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Closes**: **G12(c)** — refusal and pause finish reasons, landed with all seven values. It is also the mechanism doc 0002 records as meeting **G10**, cost as first-class events (AI-13.3, AI-13.4). Wire mapping is AI-31.1 (Wave 2)
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-ACM-0NN` · **Scenario IDs**: `S-ACM-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-MET-01` … `V-MET-12`, `V-OUT-07`, `V-OUT-08`, `V-OUT-10`; every Layer 1 noun below is one of its rows, cited by identifier
> **Binding predecessor**: [the v1 capability set](../ai-minimum-capabilities/spec.md) — `CAP-R-03`, whose clause 2 makes a usage record with no count present valid
...
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-13--define-finish-reasons-and-usage) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The seven-value finish-reason vocabulary, the normalization rule, the usage record and its cost formula therefore live here, in their own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-completion-metadata/`](../../changes/archive/2026-08-01-cachicamas-ai-completion-metadata/) is the historical record of how AI-13 was explored, proposed, designed, applied and verified.

The vocabulary is closed at seven and complete from birth. AI-31.1 maps a vendor's stop values onto it and may not extend it locally; a value added later arrives by amendment **here**, together with its string form and its normalization entry, or `R-ACM-006`'s pin fails. `R-ACM-011`'s inclusive/exclusive semantics bind every later field added to the usage record.
...
- **A provider stop value** — the raw string a vendor reports for its stop condition. `V-MET-08` calls this "a provider stop condition"; the register defines no term for the string itself (the archived `explore.md` § 5).
...
No exported function or method of this contract MUST panic for any input, including the zero value, a value outside the vocabulary, an empty string, an all-whitespace string and an over-long string.
...
2. Every vendor string in the archived `explore.md` § 4 normalizes to its value; every unrecognised string becomes unknown without error.
