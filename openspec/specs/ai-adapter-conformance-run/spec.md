...
> **Requirement IDs**: `R-ACR-0NN` · **Scenario IDs**: `S-ACR-0NN`
> **Format**: RFC 2119 + Given/When/Then, per `openspec/config.yaml` `rules.specs`. Every scenario is independently verifiable.
> **Composes (cited, not modified here)**: [`ai-provider-conformance-suite`](../ai-provider-conformance-suite/spec.md) (`R-CNF-001`…`R-CNF-019`, `R-CNF-027`, `R-CNF-028`) · [`ai-openrouter-first-provider`](../ai-openrouter-first-provider/spec.md) (`R-OR-05`, `R-OR-06`) · [`ai-provider-error-mapping`](../ai-provider-error-mapping/spec.md) (`R-AEM-014`) · [`ai-minimum-capabilities`](../ai-minimum-capabilities/spec.md) (`CAP-R-01…05`, `CAP-O-01…04`)
> **Maintainer decisions binding this spec**: Engram `#2763`
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md) § AI-38

---

## Status — this file is the canonical home of the contract

`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. This capability's six requirements plus one non-functional requirement — the unscoped-run acceptance gate, the named-side mismatch-resolution rule, the regenerable-transcript obligation, the generated capability record, the adversarial boundary-replay sweep, and adapter-factory retry parity — live here as the first artifact judging the shipped OpenRouter adapter against the full AI-23 conformance suite, unscoped, with no skip and no waiver.
...
## Out of scope — recorded follow-ups, none blocking

Four open items, none of which blocks any requirement above, recorded by name at this promotion
step per `verify-report.md` (Engram `sdd/cachicamas-ai-adapter-conformance/verify-report`, observation
**#2773**, ROUND 3):

1. **R3-1** — the post-cancel-close event bound is not tightened to exclude a specific class of
   late-arriving frames beyond what `R-CNF-011`/`R-CNF-012` already admit. Owner: a later round.
2. **R3-2** — the retry-declaration-parity guard (`R-ACR-006`) relies on a raw-bytes literal scan at
   the third conformance-factory call site because an import cycle blocks a direct reference; a
   negative-literal-mutation scan of that specific site was not independently re-verified beyond the
   two guards already defeated in round 3. Owner: a later round.
3. **R3-3** — `DialectConstraints.UnreachableFinishReasons` is asserted as a superset check, not a
   strict subset-enforcement against the closed seven-value vocabulary. Owner: a later round.
4. **R3-4** — the retry-literal pattern guard (`retryOfferedLiteralPattern`) uses `FindSubmatch`
   (first match only) rather than `FindAllSubmatch` with an all-must-agree rule; this narrows its
   reach at the one call site where the import cycle leaves it as the sole mechanism. Owner: a later
   round.

Two prior scenario narrowings were audited for honesty, not convenience, and closed at round 3, not
carried forward as open items: `S-ACR-007`'s "every transcript" narrowed to "every transcript with a
neutral-event preimage" (bounded by the structural absence of an `ai.Event` preimage, names the
excluded fixture, adds a positive recorder-test obligation rather than only deleting one); and
`ai-openrouter-first-provider` `R-OR-06`'s identical narrowing, both carrying a `(Previously: …)`
clause citing the verify-report's own SUG-1.
