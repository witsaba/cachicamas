...
> **Milestone**: AI-06 — Define content parts, readable and sealed · **Nodes**: AI-06.1 `[decision]`, AI-06.2 `[leaf]`, AI-06.3 `[leaf]`, AI-06.4 `[guard]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-content-parts/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Closes**: **C1** — an unconstructed content part passes validation — in full; and the Layer 1 half of **C2** — content unreadable from another package
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml`
> **Requirement IDs**: `R-ACP-0NN` · **Scenario IDs**: `S-ACP-0NN`
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-REQ-04` … `V-REQ-08`, `V-REQ-02`, `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13`; every Layer 1 noun below is one of its rows, cited by identifier
...
> **Binding predecessor**: [roles and message identity](../ai-message-roles/spec.md) — the message and its ordered content are AI-05's, and `R-ACP-006` supersedes exactly one of its scenarios (see below)
> **Decision record**: [`decision.md`](../../changes/archive/2026-08-01-cachicamas-ai-content-parts/decision.md) — AI-06.1's joint readability/sealing strategy, immutable
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-06--define-content-parts-readable-and-sealed) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The content-part contract therefore lives here, in its own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-content-parts/`](../../changes/archive/2026-08-01-cachicamas-ai-content-parts/) is the historical record of how AI-06 was explored, proposed, decided, designed, applied and verified — including AI-06.4's recorded guard bites.

AI-07 (reasoning), AI-09 (tool call and tool result) and AI-11 (cache markers) add kinds to the vocabulary this file closes. Each of them adds its kind **here and in the registration table**, under `R-ACP-004` and `R-ACP-010`; neither may introduce a second part type. Requirement and scenario identifiers are stable and append-only.
