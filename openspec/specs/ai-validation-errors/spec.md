...
> **Milestone**: AI-04 — Define the validation error taxonomy · **Nodes**: AI-04.1 `[decision]`, AI-04.2 `[leaf]`, AI-04.3 `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-validation-errors/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-FAIL-01` … `V-FAIL-04`, `V-FAIL-13`, `V-FAIL-16`, `V-FAIL-17`, `V-REQ-22`; every Layer 1 noun below is one of its rows, cited by identifier
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-04--define-the-validation-error-taxonomy) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The validation-error taxonomy therefore lives here, in its own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-validation-errors/`](../../changes/archive/2026-08-01-cachicamas-ai-validation-errors/) is the historical record of how AI-04 was explored, proposed, decided, designed, applied and verified.

AI-05 … AI-13 already report through this taxonomy; AI-19 owns the complement (`R-AIE-010`). A later milestone that needs a rule class no landed class describes appends it **here and in the package registry**, in the pull request that needs it, per `R-AIE-003` and `S-AIE-034`. Requirement and scenario identifiers are stable and append-only, because downstream artifacts cite them by name.
...
>
> **Amended 2026-07-31** — `S-AIE-035` was appended for the second gap the same `ErrDuplicate` append exposed: **nothing checked the closure it asserts**. The registry is unexported, so the test suite carried a hand-written copy of it, and no check compared the two — a class appended to the registry stayed outside `S-AIE-008`'s coverage until somebody edited the copy, and a sentinel declared but never registered was covered by nothing. `S-AIE-035` makes both mechanical. `R-AIE-003`'s original sentences are unchanged.
...
- **S-AIE-035** — Given a sentinel declared in the package source, when the test suite runs, then the suite fails unless that sentinel is listed in the package's own rule-class registry AND any list of the set held outside the package holds exactly the registry's members in the registry's order. *(Appended 2026-07-31 after AI-08. The check is mechanical — it scans both sources rather than trusting a list, and it compares what it read against the registry the package iterates at run time — because a list written out by hand agrees with itself no matter what is appended to the registry.)*
