...
> **Milestone**: AI-05 — Define roles and message identity · **Nodes**: AI-05.1 `[leaf]`, AI-05.2 `[leaf]`, AI-05.3 `[leaf]`
> **Introduced by**: `openspec/changes/archive/2026-08-01-cachicamas-ai-message-roles/`, in PR #101 (open for review at archive time, base `main` @ `efdedc4`)
...
> **Binding vocabulary**: [Layer 1 contract vocabulary](../ai-contract-vocabulary/spec.md) — `V-REQ-01`, `V-REQ-02`, `V-REQ-03`, `V-REQ-04`, `V-REQ-22`, `V-FAIL-01` … `V-FAIL-04`, `V-OUT-02`; every Layer 1 noun below is one of its rows, cited by identifier
> **Binding predecessor**: [the validation error taxonomy](../ai-validation-errors/spec.md) — every rule violation here reports through AI-04's single failure value and its landed rule classes, and is cited rather than re-decided
> **Sources**: [doc 0002 — Layer 1 task graph](../../../docs/architecture/milestones/0002-cachicamas-ai-layer-1-task-graph.md#ai-05--define-roles-and-message-identity) · [doc 0001 — agent stack v2](../../../docs/architecture/0001-cachicamas-agent-stack-v2.md) · [ADR 0004](../../../docs/adr/0004-adopt-tau-3-layer-agentic-architecture.md) · [ADR 0005 § D4](../../../docs/adr/0005-promote-agent-stack-to-own-module.md)
...
One requirement — `R-AMSG-009` — constrains the **shape of the exported surface** rather than its behavior, because the property it protects (that AI-06.1 inherits an undecided question) is not observable at runtime. It is marked where it appears and was verified by inspection in the archived `tasks.md` verification pass.

## Status — this file is the canonical home of the contract

`openspec/AGENTS.md` calls `openspec/specs/` the *source of truth (main specs)*. The role vocabulary, message identity, content ordering and copy semantics therefore live here, in their own text, and not only as a pointer into the archive. The archived change folder at [`openspec/changes/archive/2026-08-01-cachicamas-ai-message-roles/`](../../changes/archive/2026-08-01-cachicamas-ai-message-roles/) is the historical record of how AI-05 was explored, proposed, designed, applied and verified.

> **Identifier note.** This contract's identifiers are `R-AMSG-0NN` / `S-AMSG-0NN`. They were renamed from `R-AMR-` / `S-AMR-` during Wave 1 to end a collision with AI-10, which keeps `R-AMR-` for the normalized request core. The `AMSG` prefix is the live one; a citation of `R-AMR-0NN` resolves to [the normalized request core](../ai-model-request/spec.md), never to this file.

AI-06 owns what a content part *is* (`R-AMSG-009` deliberately leaves that seam open). AI-10 composes messages into the normalized request core. Requirement and scenario identifiers are stable and append-only.
