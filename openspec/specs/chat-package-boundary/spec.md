...
`backend/agent/src/chat/` MUST exist as a Go package named `chat`, carrying package documentation and **nothing else** — no type, no constant, no function, no variable — at CH-01's own merge state; CH-02 gives the package its first declarations (`cachicamas-chat-conversation`, Engram-only artifact store).
...
- **S-CPB-002** — Given `backend/agent/src/chat/` **at CH-01's own merge state**, when its Go declarations are enumerated (for example `go doc github.com/cachicamas/backend/agent/src/chat`), then the package exports nothing and declares nothing beyond the package clause and its documentation comment; CH-02 gives the package its first declarations.
...
- **S-CPB-004** — Given `backend/agent/src/chat/doc.go`, when its comment is read, then it states that this archetype occupies the Layer 3 **position** (ADR 0009 § D2), names CH-01.1 of doc 0005 as its creator, enumerates the position's forbidden closure **and its permitted OpenTelemetry surface** — the API packages and the `otelslog` bridge (ADR 0005 § D3, `adr:243`, under the ADR 0009 § D2 substitution) — and names the guard in `backend/agent/src/agent/import_boundary_test.go` as the deny-by-default enforcement. **Amended by CH-02** (`cachicamas-chat-conversation`): the comment no longer states that the package ships empty of policy — it instead states that CH-02 filled the package with its conversation/projection policy and names the file split (`conversation.go`, `wire.go`, `projection.go`, `errortext.go`).
...
- **S-CPB-021** — Given the merged tree **at CH-01's own merge state**, when that same listing is filtered for a `_test.go` suffix, then it is empty; CH-02 adds the archetype's first test files (`cachicamas-chat-conversation`).
...
**Corrected by CH-02** (`cachicamas-chat-conversation`), which discovered this clause was itself wrong rather than merely unfinished: `…/src/layer3handoff` ships only `doc.go` plus two `_test.go` files and therefore has **no importable surface at all**, so admitting it to any closure would grant nothing. It MUST instead remain **denied to both the production and test closures**. The packages `0005:66` (R-10) actually requires every harness consumer to test against — the AG-23 scripted kit — are `…/src/apptest` and `…/src/agenttest`, named nowhere in this clause's original text; CH-02's test closure admits both instead. The asymmetry this clause originally described is corrected in place, not silently replaced, in the guard's own tables.
...
- **S-CPB-034** — **Corrected by CH-02** (`cachicamas-chat-conversation`): given the guard's archetype tables, when the `…/src/layer3handoff` row is read, then a comment in place records that the path is denied to **both** the production and test closures — it has no importable surface at all — and names `…/src/apptest` and `…/src/agenttest` as the packages `0005:66` actually requires for the test closure, with CH-02 named as the milestone that corrected this row's original, wrong asymmetry claim.
