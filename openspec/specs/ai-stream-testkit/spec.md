...
> **Milestone**: AI-22 — Build stream recording and assertion helpers (Wave 3 "Prove", the wave's **second** milestone) · **Nodes**: AI-22.1 `[leaf]` timeout-safe drain and record · AI-22.2 `[leaf]` readable event diffs · AI-22.3 `[leaf]` ordering and gap assertions · AI-22.4 `[decision]` leak-detection mechanism · AI-22.5 `[leaf]` carrier view
> **Introduced by**: `openspec/changes/archive/2026-08-03-cachicamas-ai-stream-testkit/`, merged to `main` in PR #107 (commit `0d5fd91`)
...
> **Closes**: doc 0002's **AI-22** milestone in full, and **G13**'s carrier-ergonomics half — `V-STR-22`'s carrier view is the ergonomic convenience AI-02.1 delegated here, over the channel AI-20.4 already pinned mechanically
> **Requirement IDs**: `R-STK-0NN` · **Scenario IDs**: `S-STK-0NN`
> **Format**: RFC 2119 keywords + Given/When/Then, per `openspec/config.yaml` `rules.specs`. Every scenario is independently verifiable.
> **Vocabulary owned by this milestone**: [`ai-contract-vocabulary`](../ai-contract-vocabulary/spec.md) `V-PRV-11` **stream test kit** (AI-22) and `V-STR-22` **carrier view** (appended by AI-02.1, ergonomics delegated to AI-22.5). Both terms are used as the register defines them; neither is renamed and no alternate name is coined.
> **Binding predecessors, cited by identifier and never modified**:
> [`ai-model-provider`](../ai-model-provider/spec.md) (AI-20) — `R-AMP-009` … `R-AMP-012` mid-stream physics, AI-20.4's signature guard;
> [`ai-stream-lifecycle`](../ai-stream-lifecycle/spec.md) — § 3 carrier, § 4 ownership, § 5 cancellation (the abandoned-never-cancelled path it rules untestable), § 6 buffering, § 7 failure delivery;
> [`ai-event-envelope`](../ai-event-envelope/spec.md) (AI-14) — the envelope, its kind vocabulary, `CheckStream`'s ordering invariants (AI-14.4) and per-stream sequencing;
> [`ai-fake-provider`](../ai-fake-provider/spec.md) (AI-21) — the producer these helpers are proved against
...
> **Resolved at archive 2026-08-03.** This section originally left three items to design, deliberately undecided at spec time. All three are settled and shipped.

`R-STK-007`'s leak helper repeats a scenario **50 times** and compares live-goroutine counts before and after, tolerant of growth up to half the repeat count — real per-iteration leaks grow linearly with repeats, background jitter does not. `R-STK-004`'s per-kind exhaustiveness is enforced by a table-keys-versus-registry test (`stream_kit_diff.go`), proven by mutation testing to fail both when a kind is added without a summary entry and when one is removed. `R-STK-011`'s carrier view is `NewIter(ch <-chan ai.Event) *Iter`, exposing `(*Iter).Events(ctx) iter.Seq[ai.Event]` (Go 1.26's range-over-func) with a post-loop `(*Iter).Err()` surfacing the terminal error or `ctx.Err()` — `bufio.Scanner`-style, since range-over-func cannot itself return a value.

The conformance suite that consumes these helpers is AI-23 (`ai-provider-conformance-suite`, promoted alongside this file) — a sibling in this wave, a separate capability, a consumer of this one rather than part of it.
