...
### Identity persistence (`events.signIn`)

Every successful GitHub sign-in runs the `events.signIn` callback
on the Auth.js config (`frontend/src/routes/plugin@auth.ts`). The
callback persists the GitHub identity to `identity.user` +
`identity.account` via:

1. `frontend/src/lib/db.ts` — `getSql()` returns a singleton
   porsager `postgres` client backed by `process.env.IDENTITY_DATABASE_URL`.
   The driver is dynamically imported behind `import.meta.env.SSR` so
   it never reaches the browser bundle.
2. `frontend/src/lib/sign-in-callback.ts` — `handleSignIn(sql, event)`
   runs the auto-link UPSERT. The 6-test suite in
   `sign-in-callback.test.ts` covers the happy path, miss path,
   triangulation, and email-missing denial.

Failure mode: a transient SQL error is logged to `console.error`
with the `[cachicamas]` prefix and swallowed so the OAuth roundtrip
still completes and the JWE cookie is minted. Losing one persisted
row is strictly better than refusing a successful GitHub sign-in.

The relevant test files:

- `frontend/src/routes/plugin@auth.test.ts` (9 unit tests): the 5
  pre-existing config-shape tests + 4 new tests covering the
  `events.signIn` wiring, delegation to `handleSignIn(getSql(), …)`,
  best-effort error swallowing, and `null`-account guard.
- `frontend/src/lib/db.test.ts` (4 unit tests): the factory's
  fail-fast on missing `IDENTITY_DATABASE_URL`, singleton semantics,
  `resetSqlForTest()` cache clear, and option forwarding (`max: 4`,
  `prepare: false`).

#### Configuring `IDENTITY_DATABASE_URL` outside compose

In compose the URL is interpolated from `QUEEN_USER` /
`QUEEN_PASSWORD` / `POSTGRES_DB` / `POSTGRES_PORT` (see the
`frontend` service env block). Outside compose (e.g. `pnpm dev`
against a local Postgres), set the var in `.env`:

```bash
IDENTITY_DATABASE_URL=postgres://queen:<QUEEN_PASSWORD>@localhost:5432/cachicamas_pg
```

If unset, `events.signIn` throws on the first sign-in (the auth
flow itself still works because the wiring is best-effort, but
`identity.user` / `identity.account` rows will NOT be persisted).
