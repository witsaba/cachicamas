...
export const ScreenTitle = component$<ScreenTitleProps>((props) => {
  // On `/chat` the code and the title are both "Chat"; rendering both puts an
  // eyebrow with no payload above the heading. The chip earns its place only
  // when it teaches something the title does not already say.
  const showCode = props.code.toLowerCase() !== props.title.toLowerCase();
  return (
    <div
    ...
    >
      <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        {showCode ? (
          <span class="border-rule-strong text-legend text-fg-dim border px-1.5 py-px tracking-[0.18em] uppercase">
            {props.code}
          </span>
        ) : null}
