...
/**
 * Create the install's single organization.
 *
 * The ownboarding form maps the discriminated result straight back
 * onto its own `FormActionResult`:
 *   - ok               -> navigate on to the desktop
 *   - validation       -> render per-field errors (or a generic one)
 *   - conflict         -> render the identification-taken message
